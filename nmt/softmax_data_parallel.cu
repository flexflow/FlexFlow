...
SoftmaxDP::SoftmaxDP(RnnConfig config, Tensor logit, Tensor _label, ParallelConfig pc)
...
void SoftmaxDP::init(const RnnModel& model)
{
  ...
  for (PointInRectIterator<1> it(part_rect); it(); it++, idx++) {
    ...
    {
      LogicalRegion x =
        runtime->get_logical_subregion_by_color(logit_lp, dp);
                             ...
                             Context ctx, Runtime *runtime)
{
  ...
  checkCUDNN(cudnnSoftmaxForward(m->handle.dnn,
                                 ...
                                 &alpha, m->inputTensor, x_ptr,
                                 &beta, m->inputTensor, y_ptr));
  if (m->profiling_runtime) {
...
__global__ void SoftmaxLossBackprop(float *input, const int *label,
                              ...
                              Context ctx, Runtime *runtime)
{
  ...
  checkCUDA(cudaMemcpyAsync(x_grad_ptr, y_ptr,
