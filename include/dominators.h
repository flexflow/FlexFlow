...
namespace flexflow::graph {
  ...
  template <typename G, typename Structure = GraphStructure<G>>
  std::unordered_set<typename Structure::vertex_type> successors(
      G const &g,
      typename Structure::vertex_type const &node
  ) {
    //using N = typename Structure::vertex_type;
  ...
  template <typename G, typename Structure = GraphStructure<G>>
  std::unordered_set<typename Structure::vertex_type> predecessors(
      G const &g,
      typename Structure::vertex_type const &node
  ) {
    //using N = typename Structure::vertex_type;
  ...
  BasicGraph<typename Structure::vertex_type> transitive_reduction(G const &g) {
    ...
    auto dfs_visitor = [&](N const &src, G const &gg, Structure const &ss, N const &dfs_src, N const &nn) {
      if (nn != dfs_src && to_delete.find({src, nn}) == to_delete.end() && has_edge<G, Structure>(gg, src, nn)) {
    ...
    for (N const &n : nodes) {
      ...
      auto n_dfs_visitor = [&](G const &gg, Structure const &ss, N const &dfs_src, N const &nn) {
  ...
  void inplace_transitive_reduction(BasicGraph<N> &g) {
    ...
    auto dfs_visitor = [&](N const &src, G const &gg, Structure const &ss, N const &dfs_src, N const &nn) {
      if (nn != dfs_src && to_delete.find({src, nn}) == to_delete.end() && has_edge(gg, src, nn)) {
        to_delete.insert({src, nn});
      }
    };
