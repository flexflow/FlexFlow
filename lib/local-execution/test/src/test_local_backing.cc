...
LocalTrainingBacking get_test_local_backing(
  ComputationGraphOpAttrs const & attrs,
  std::vector<TensorShape> const & input_shapes,
  std::vector<TensorAttrs> const & weight_attrs,
  std::vector<TensorAttrs> const & output_attrs
) {
  ...
  cg_builder.add_layer(LayerAttrs{attrs, std::nullopt},
                        input_tensor_ids,
                        weight_attrs,
                        output_attrs);
...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("Local Backing Single Operator -- Cast") {
    ...
    TensorAttrs output = TensorAttrs{
      TensorShape{
        TensorDims{FFOrdered<size_t>{d1, d2}},
        output_datatype,
      }, std::nullopt, std::nullopt, CreateGrad::YES
    };

    CastAttrs attrs = CastAttrs{output_datatype};
    LocalTrainingBacking local_backing = get_test_local_backing(
      ComputationGraphOpAttrs{attrs},
      std::vector<TensorShape>{input},
      {},
      std::vector<TensorAttrs>{output}
    );

    ComputationGraph const cg = local_backing.get_computation_graph();
    layer_guid_t layer_guid = get_only(topological_ordering(cg));
    ...
    std::vector<tensor_guid_t> input_tensor_guids = get_incoming_tensors(cg, layer_guid);
    std::vector<tensor_guid_t> output_tensor_guids = get_incoming_tensors(cg, layer_guid);

    SUBCASE("Tensor Slot Insertion") {
      LocalSlotsBacking const local_slots_backing = local_backing.get_local_slots_backing();
      std::unordered_map<layer_guid_t, std::vector<tensor_guid_t>> correct_input_tensor_slots;
      std::unordered_map<layer_guid_t, std::vector<tensor_guid_t>> correct_output_tensor_slots;
      std::unordered_map<layer_guid_t, std::vector<tensor_guid_t>> correct_weight_tensor_slots;

      correct_input_tensor_slots.insert({layer_guid, input_tensor_guids});
      correct_input_tensor_slots.insert({layer_guid, output_tensor_guids});

      CHECK(correct_input_tensor_slots == local_slots_backing.input_tensor_slots);
      CHECK(correct_output_tensor_slots == local_slots_backing.output_tensor_slots);
      CHECK(correct_weight_tensor_slots == local_slots_backing.weight_tensor_slots);
    }
    ...
    SUBCASE("Tensor Allocation") {
      GenericTensorAccessorW input_tensor_backing = local_slots_backing.tensor_mapping.at(input_tensor_guid);
      GenericTensorAccessorW output_tensor_backing = local_slots_backing.tensor_mapping.at(output_tensor_guid);
      ...
      // gradient tensors
      GenericTensorAccessorW input_grad_tensor_backing = local_slots_backing.gradient_tensor_mapping.at(input_tensor_guid);
      GenericTensorAccessorW output_grad_tensor_backing = local_slots_backing.gradient_tensor_mapping.at(output_tensor_guid);
    ...
    SUBCASE("Forward Task Argument Accessor") {
      ...
      // get accessor
      TaskArgumentAccessor acc = local_backing.get_task_arg_accessor(fwd_invocation, layer_guid);
      ProfilingSettings result_settings = acc.get_argument<ProfilingSettings>(PROFILING);
      CastAttrs result_attrs = acc.get_argument<CastAttrs>(ATTRS);
      GenericTensorAccessorR result_input_tensor_backing = acc.get_tensor<Permissions::RO>(INPUT);
      GenericTensorAccessorW result_output_tensor_backing = acc.get_tensor<Permissions::WO>(OUTPUT);
      ...
      LocalSlotsBacking const local_slots_backing = local_backing.get_local_slots_backing();
      GenericTensorAccessorW slots_input_tensor_backing = local_slots_backing.tensor_mapping.at(input_tensor_guid);
      GenericTensorAccessorW slots_output_tensor_backing = local_slots_backing.tensor_mapping.at(output_tensor_guid);
      CHECK(result_input_tensor_backing == read_only_accessor_from_write_accessor(slots_input_tensor_backing));
    ...
    SUBCASE("Backward Task Argument Accessor") {
      ...
      // get acc
      TaskArgumentAccessor acc = local_backing.get_task_arg_accessor(bwd_invocation, layer_guid);
      ProfilingSettings result_settings = acc.get_argument<ProfilingSettings>(PROFILING);
      CastAttrs result_attrs = acc.get_argument<CastAttrs>(ATTRS);
      GenericTensorAccessorR result_input_tensor_backing = acc.get_tensor<Permissions::RO>(INPUT);
      GenericTensorAccessorR result_input_grad_tensor_backing = acc.get_tensor_grad<Permissions::RO>(INPUT);
      GenericTensorAccessorW result_output_grad_tensor_backing = acc.get_tensor_grad<Permissions::WO>(OUTPUT);

      CHECK(correct_settings == result_settings);
      CHECK(attrs == result_attrs);
      CHECK(correct_input_array_shape == result_input_tensor_backing.shape);
      CHECK(correct_input_array_shape == result_input_grad_tensor_backing.shape);
      CHECK(input_datatype == result_input_tensor_backing.data_type);
      CHECK(input_datatype == result_input_grad_tensor_backing.data_type);
      CHECK(correct_output_array_shape == result_output_grad_tensor_backing.shape);
      CHECK(output_datatype == result_output_grad_tensor_backing.data_type);

      // check against slots backing
      LocalSlotsBacking const local_slots_backing = local_backing.get_local_slots_backing();
      GenericTensorAccessorW slots_input_tensor_backing = local_slots_backing.tensor_mapping.at(input_tensor_guid);
      GenericTensorAccessorW slots_input_grad_tensor_backing = local_slots_backing.gradient_tensor_mapping.at(input_tensor_guid);
      GenericTensorAccessorW slots_output_grad_tensor_backing = local_slots_backing.gradient_tensor_mapping.at(output_tensor_guid);
      
      CHECK(result_input_tensor_backing == read_only_accessor_from_write_accessor(slots_input_tensor_backing));
      CHECK(result_input_grad_tensor_backing == read_only_accessor_from_write_accessor(slots_input_grad_tensor_backing));
      CHECK(result_output_grad_tensor_backing == slots_output_grad_tensor_backing);
    }
