...
static std::vector<TensorAttrs> get_piece_attrs(std::vector<ParallelTensorAttrs> const & parallel_attrs) {
  return transform(parallel_attrs, 
    [](ParallelTensorAttrs const &p) { return get_piece_attrs(p); });
}

static TensorAttrs get_piece_attrs(ParallelTensorAttrs const & parallel_attrs) {
  return {get_piece_shape(parallel_attrs.shape), parallel_attrs.initializer, parallel_attrs.sync_type, parallel_attrs.create_gradients};
}

static LayerAttrs get_layer_attrs_from_pcg_op_attrs(PCGOperatorAttrs const &op) {
  assert (!is_parallel_op(op));
  return std::visit([](auto&& arg) -> LayerAttrs {
        return LayerAttrs{ComputationGraphOpAttrs{std::forward<decltype(arg)>(arg)}};
    }, op);
}

float LocalCostEstimator::estimate_cost(PCGOperatorAttrs const &op,
  ...
  for (ParallelTensorShape const & input: inputs) {
    ...
    GenericTensorAccessorW const tensor_backing = allocator.allocate_tensor(tensor_shape);
    tensor_backing_map.insert({tensor_id, tensor_backing});
    input_tensor_ids.push_back(tensor_id);
  }
  
  // add operator
  std::vector<tensor_guid_t> output_tensor_ids = cg_builder.add_layer(
    layer_attrs,
    input_tensor_ids,
  ...
  RuntimeArgConfig runtime_arg_config = {
    DeviceSpecific<PerDeviceFFHandle>::create(get_ff_handle()),  // fill in from #1384
    EnableProfiling::YES,
    ProfilingSettings{warmup_iters, measure_iters}
  };

  LocalTrainingBacking local_backing (allocator,
    cg_builder.computation_graph, tensor_backing_map, runtime_arg_config);
