...
/**
 * @brief Takes a SubParallelComputationGraph but without ParallelTensorShape annotations on its OpenDataflowValue%s 
 * and uses shape inference to fill them in.
 *
 ...
 *
 * Exists only to enable apply_substitution(SubParallelComputationGraph const &, Substitution const &, PCGPatternMatch const &)
 */
