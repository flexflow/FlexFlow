...
TEST_CASE("apply_substitution") {
  ...
  auto ig =
      OutputLabelledOpenMultiDiGraph<OperatorPattern, ParallelTensorPattern>::create<
  ...
  OperatorAttrAssignment op_ass_n2 {
    ...
    {OperatorAttributeKey::OUT_CHANNELS, OperatorAttrAccess{n0, OperatorAttributeKey::OUT_CHANNELS}},
    {OperatorAttributeKey::USE_BIAS, OperatorAttrAccess{n0, OperatorAttributeKey::USE_BIAS}},
    {OperatorAttributeKey::DATA_TYPE, OperatorAttrAccess{n0, OperatorAttributeKey::DATA_TYPE}},
    {OperatorAttributeKey::ACTIVATION, OperatorAttrAccess{n0, OperatorAttributeKey::ACTIVATION}},
    {OperatorAttributeKey::REGULARIZER, OperatorAttrAccess{n0, OperatorAttributeKey::REGULARIZER}}
    }
  };
  ...
  Substitution substitution{input_graph, output_graph_expr, input_mapping, output_mapping};
  ...
  Node n5 = pcg.add_node(Operator{LinearAttrs{1, false, DataType::FLOAT, Activation::RELU, nullopt}, "linear"});
  NodePort p4 = pcg.add_node_port();
  NodePort p5 = pcg.add_node_port();

  MultiDiEdge e4{n5, p5, n4, p4};
  pcg.add_edge(e4);
  pcg.add_label(e4, ParallelTensor(ParallelTensorDims({2, 1}), DataType::FLOAT, CreateGrad::YES));

  MatchAdditionalCriterion criterion {
    [&](Node const &pattern_node, Node const &graph_node) {
      return operator_satisfies(pcg.at(graph_node), input_graph.value().at(pattern_node));
    },
    [&](OpenMultiDiEdge const &pattern_edge, OpenMultiDiEdge const &graph_edge) {
      return parallel_tensor_satisfies(at(pcg, graph_edge), at(input_graph.value(), pattern_edge));
    }
  };

  RC_ASSERT(criterion.node_criterion(n0, n5));

  std::vector<MultiDiGraphPatternMatch> matches = find_pattern_matches(input_graph, pcg, criterion);

  RC_ASSERT(matches.size() == 1);

  SubParallelComputationGraph new_pcg = apply_substitution(pcg, substitution, matches[0]);
