  ...
  TEST_CASE("pattern_split (sequential)") {
    ...
    SUBCASE("apply_split") {
      ...
      SUBCASE("subpattern_1") {
        std::unordered_set<PatternNode> result = get_nodes(split_result.subpattern_1);
        std::unordered_set<PatternNode> correct = even_split.first;
        CHECK(result == correct);
      }
      SUBCASE("subpattern_2") {
        std::unordered_set<PatternNode> result = get_nodes(split_result.subpattern_2);
        std::unordered_set<PatternNode> correct = even_split.second;
        CHECK(result == correct);
      }
      SUBCASE("full_pattern_values_to_subpattern_1_inputs") {
        bidict<PatternValue, PatternInput> result = split_result.full_pattern_values_to_subpattern_1_inputs;
        bidict<PatternValue, PatternInput> correct = {};
        CHECK(result == correct);
      }
      SUBCASE("full_pattern_values_to_subpattern_2_inputs") {
        bidict<PatternValue, PatternInput> result = split_result.full_pattern_values_to_subpattern_2_inputs;
  ...
  TEST_CASE("pattern split (parallel)") {
    ...
    SUBCASE("apply_split") {
      ...
      SUBCASE("full_pattern_values_to_subpattern_1_inputs") {
        bidict<PatternValue, PatternInput> result = split_result.full_pattern_values_to_subpattern_1_inputs;
        bidict<PatternValue, PatternInput> correct = {
          { PatternValue{pi0}, get_only(get_inputs(split_result.subpattern_1))},
        };
        CHECK(result == correct);
      }
      SUBCASE("full_pattern_values_to_subpattern_2_inputs") {
        bidict<PatternValue, PatternInput> result = split_result.full_pattern_values_to_subpattern_2_inputs;
        bidict<PatternValue, PatternInput> correct = {
          { PatternValue{pi1}, get_only(get_inputs(split_result.subpattern_2))},
        };
        CHECK(result == correct);
      }
    }
