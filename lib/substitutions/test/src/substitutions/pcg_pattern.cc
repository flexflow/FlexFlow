...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("find_pattern_matches(PCGPattern, SubParallelComputationGraph)") {
    ...
    parallel_tensor_guid_t a_tensor = builder.create_input_tensor(a_shape, /*create_grad=*/true, a_name);

    int outDim = 16;
    std::string x_matmul_name = "x_matmul";
    std::string y_matmul_name = "y_matmul";
    parallel_tensor_guid_t t0 = builder.dense(a_tensor, 
                                              outDim, 
                                              ...
                                              x_matmul_name);
    parallel_tensor_guid_t t1 = builder.dense(a_tensor, 
                                              outDim, 
                                              /*activation=*/std::nullopt, 
                                              /*use_bias=*/false, 
                                              DataType::FLOAT, 
    ...
    parallel_layer_guid_t x_matmul = get_parallel_layer_by_name(pcg, x_matmul_name);
    parallel_layer_guid_t y_matmul = get_parallel_layer_by_name(pcg, y_matmul_name);
    std::vector<parallel_tensor_guid_t> x_inputs = get_layer_inputs(pcg, x_matmul);
    REQUIRE(x_inputs.size() == 2);
    parallel_tensor_guid_t x_weights = x_inputs.at(1);
    std::vector<parallel_tensor_guid_t> y_inputs = get_layer_inputs(pcg, y_matmul);
    ...
    NodeAddedResult op_pattern_1_added = g.add_node(op_pattern_1, {OpenDataflowValue{pt_a}, OpenDataflowValue{pt_b}}, {pattern_tensor_x});
    PatternNode op_pattern_1_node = PatternNode{op_pattern_1_added.node};
    OpenDataflowValue pt_x = OpenDataflowValue{get_only(op_pattern_1_added.outputs)};

    NodeAddedResult op_pattern_2_added = g.add_node(op_pattern_2, {OpenDataflowValue{pt_a}, OpenDataflowValue{pt_c}}, {pattern_tensor_y});
    PatternNode op_pattern_2_node = PatternNode{op_pattern_2_added.node};
    OpenDataflowValue pt_y = OpenDataflowValue{get_only(op_pattern_2_added.outputs)};
