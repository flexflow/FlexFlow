...
DiGraphPatternMatch narrow_match(DiGraphPatternMatch const &match, IOpenMultiDiGraphView const &pattern) {
...
std::pair<std::unique_ptr<IOpenMultiDiGraphView>, std::unique_ptr<IOpenMultiDiGraphView>> 
...
bidict<MultiDiEdge, std::pair<OutputMultiDiEdge, InputMultiDiEdge>> get_edge_splits(IOpenMultiDiGraphView const &pattern, GraphSplit const &split) {
...
MatchSplit apply_split(IOpenMultiDiGraphView const &pattern, DiGraphPatternMatch const &match, GraphSplit const &split) {
  auto prefix = split.first;
  auto postfix = split.second;
  ...
  std::function<void(OpenMultiDiEdge const &)> handle_edge = [&](OpenMultiDiEdge const &pattern_edge) -> void {
...
template <typename F>
bool pattern_matches(IOpenMultiDiGraphView const &pattern, IMultiDiGraph const &graph, DiGraphPatternMatch const &match, F const &additional_criterion) {
...
tl::optional<DiGraphPatternMatch> get_candidate_singleton_match(IOpenMultiDiGraphView const &pattern, IMultiDiGraphView const &graph, Node const &graph_node) {
  ...
  for (OpenMultiDiEdge const &pattern_edge : get_edges(pattern)) {
    ...
    if (is_input_edge(pattern_edge)) {
      ...
      match.edgeAssignment.equate(input_edge, get_only(incoming.at(input_edge.dstIdx)));
    } else {
      OutputMultiDiEdge output_edge = mpark::get<OutputMultiDiEdge>(pattern_edge);
...
tl::optional<DiGraphPatternMatch> unsplit_matches(DiGraphPatternMatch const &prefix, 
                                                  ...
                                                  bidict<MultiDiEdge, std::pair<OutputMultiDiEdge, InputMultiDiEdge>> const &edge_splits) {
  DiGraphPatternMatch result;
  ...
  for (auto const &kv : merge_maps(prefix.edgeAssignment, postfix.edgeAssignment)) {
    if (!contains(handled, kv.first)) {
      result.edgeAssignment.equate(kv.first, kv.second);
    }
  }

  result.nodeAssignment = merge_maps(prefix.nodeAssignment, postfix.nodeAssignment);
  
  return result;
}

template <typename F>
std::unordered_set<DiGraphPatternMatch> find_pattern_matches(IOpenMultiDiGraphView const &pattern, IMultiDiGraph const &graph, F const &additional_criterion) {
