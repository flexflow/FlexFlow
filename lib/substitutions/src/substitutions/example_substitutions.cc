...
struct SubstitutionBuilder {
  PatternValue add_input(std::string const &name, std::optional<TensorAttributePattern> const & = std::nullopt);
  ...
  std::vector<OutputPatternValue> add_operator_to_output(OutputOperatorAttrsAssignment const &,
                                            std::vector<OutputPatternValue> const &inputs,
                                            int num_outputs);
  OutputPatternValue add_operator_to_output(OutputOperatorAttrsAssignment const &,
                                            std::vector<OutputPatternValue> const &inputs);
...
Substitution create_replicate_linear_combine(int num_dims,
  ...
  OutputOperatorAttrsAssignment new_replicate = OutputOperatorAttrsAssignment{{
    { 
  ...
  OutputOperatorAttrsAssignment new_linear = output_operator_clone_node(b.pattern_node("linear"));
  ...
  t = b.add_operator_to_output(new_linear, {t, b.add_input_to_output(weight)});
  t = b.add_operator_to_output(new_combine, {t});

  b.unify_outputs(output, t);

  return b.get_substitution();

  // the other option is to have the user build the pattern and the output first, and only then do the mapping at the end. Not sure which is better
}
