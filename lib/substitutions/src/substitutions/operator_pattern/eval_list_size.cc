...
std::optional<OperatorAttributeValue> eval_list_size(PCGOperatorAttrs const &attrs, OperatorAttributeListSize const &acc) {
  ...
  >([&](auto const &v) -> std::optional<OperatorAttributeValue> {
    ...
    if constexpr (std::is_same_v<T, std::vector<int>> || std::is_same_v<T, std::vector<ff_dim_t>>) {
      size_t size = v.size();
