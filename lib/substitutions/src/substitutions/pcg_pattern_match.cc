...
bidict<PatternNodeOutput, parallel_tensor_guid_t> get_output_mapping_for_pcg_pattern_match(PCGPatternMatch const &match, 
  ...
  for (auto const &[pattern_node, matched_layer] : match.node_assignment) {
    std::vector<parallel_tensor_guid_t> matched_layer_output_tensors = get_layer_outputs(spcg, matched_layer);
    std::vector<PatternNodeOutput> pattern_node_outputs = get_pattern_node_outputs(pattern, pattern_node);

    assert (matched_layer_output_tensors.size() == pattern_node_outputs.size());

    bidict<PatternNodeOutput, parallel_tensor_guid_t> mapping = bidict_from_keys_and_values(pattern_node_outputs, matched_layer_output_tensors);
