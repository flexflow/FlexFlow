...
std::optional<MultiDiGraphPatternMatch> unsplit_matches(
  ...
  for (auto const &coi : as_closed_output_input_tuples(edge_splits)) {
    ...
    OpenMultiDiEdge input_graph_edge = postfix.edge_assignment.at_l(pattern_edge_from_input_edge(input_edge));
    if (output_graph_edge == input_graph_edge) {
      result.edge_assignment.equate(pattern_edge_from_closed_edge(closed_edge), output_graph_edge);
    } else {
      return std::nullopt;
    }
  }
