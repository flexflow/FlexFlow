...
flexflow_error_t flexflow_pcg_error_get_string(flexflow_pcg_error_t err,
                                               char **m_out) {
...
flexflow_error_t
    flexflow_pcg_error_get_error_code(flexflow_error_t err,
                                      flexflow_pcg_error_code_t *out) {
...
flexflow_error_t
    flexflow_computation_graph_destroy(flexflow_computation_graph_t opaque) {
...
T from_v1_string(char const *s) {
  return json::parse(s)
      .template get<decltype(to_v1(
          std::declval<opaque_to_underlying_t<T>>()))>();
}

flexflow_error_t flexflow_computation_graph_serialize_to_buffer(
    flexflow_computation_graph_t opaque, char **out) {
  return handle_errors([&] {
...
flexflow_error_t flexflow_computation_graph_deserialize_from_buffer(
  ...
  return handle_errors(out,
                       [&] { return from_v1_string<ComputationGraph>(buf); });
}

flexflow_error_t flexflow_computation_graph_serialize_to_file(
    flexflow_computation_graph_t cg, FILE *f) {
  return handle_errors([&] {
    if (f == nullptr) {
    ...
    size_t num_bytes_written =
        fwrite(json_str.c_str(), sizeof(char), json_str.size(), f);
...
flexflow_error_t flexflow_computation_graph_deserialize_from_file(
    FILE *f, flexflow_computation_graph_t *out) {
  return handle_errors(out, [&] {
...
flexflow_error_t flexflow_tensor_get_create_grad(flexflow_tensor_t opaque,
                                                 bool *out) {
  return handle_errors(out,
                       [&] { return c_deref_opaque(opaque).create_gradients; });
}

flexflow_error_t flexflow_tensor_get_initializer(flexflow_tensor_t opaque,
                                                 flexflow_initializer_t *out) {
  return handle_errors(out, [&] { return c_deref_opaque(opaque).initializer; });
}

flexflow_error_t flexflow_tensor_get_sync_type(flexflow_tensor_t opaque,
                                               flexflow_param_sync_t *out) {
  return handle_errors(out, [&] { return c_deref_opaque(opaque).sync_type; });
}

flexflow_error_t flexflow_tensor_get_datatype(flexflow_tensor_t opaque,
                                              flexflow_datatype_t *out) {
  return handle_errors(out, [&] { return c_deref_opaque(opaque).data_type; });
}

flexflow_error_t flexflow_tensor_get_num_dims(flexflow_tensor_t opaque,
                                              int *out) {
...
using BinaryFunc = std::function<Tensor(ComputationGraph &,
                                        Tensor const &,
                                        Tensor const &,
                                        optional<std::string> const &)>;
using UnaryFunc = std::function<Tensor(
    ComputationGraph &, Tensor const &, optional<std::string>)>;
using ScalarUnaryFunc = std::function<Tensor(
    ComputationGraph &, Tensor const &, float, optional<std::string>)>;

#define BINARY_OP(insert_func_layer)                                           \
  [](ComputationGraph &cg,                                                     \
     Tensor const &lhs,                                                        \
     Tensor const &rhs,                                                        \
     optional<std::string> const &name) -> Tensor {                            \
    return func(cg, lhs, rhs, name);                                           \
  }

#define BINARY_OP(insert_func_layer)                                           \
  [](ComputationGraph &cg, Tensor const &t, optional<std::string> const &name) \
      -> Tensor { return func(cg, t, name); }

#define SCALAR_BINARY_OP(insert_func_layer)                                    \
  [](ComputationGraph &cg,                                                     \
     Tensor const &t,                                                          \
     float scalar,                                                             \
     optional<std::string> const &name) -> Tensor {                            \
    return func(cg, t, scalar, name);                                          \
  }
...
flexflow_error_t add_op(UnaryFunc const &f,
                        ...
                        char *name) {
  return handle_errors(out, [&] {
    return f(deref_opaque(cg), c_deref_opaque(input), maybe_string(name));
  });
}
...
flexflow_error_t flexflow_computation_graph_add_op_exp(
    flexflow_computation_graph_t opaque_cg,
    flexflow_tensor_t opaque_input,
    flexflow_tensor_t *out,
    char *name) {
  return add_op(
      BINARY_OP(insert_exp_layer), opaque_cg, opaque_input, out, name);
}

flexflow_error_t flexflow_computation_graph_add_op_add(
    flexflow_computation_graph_t opaque_cg,
    flexflow_tensor_t opaque_lhs,
    flexflow_tensor_t opaque_rhs,
    flexflow_tensor_t *out,
    char *name) {
  return add_op(BINARY_OP(insert_add_layer),
                opaque_cg,
                opaque_lhs,
                opaque_rhs,
                out,
...
flexflow_error_t
    flexflow_computation_graph_add_op_subtract(flexflow_computation_graph_t cg,
                                               flexflow_tensor_t lhs,
                                               flexflow_tensor_t rhs,
                                               flexflow_tensor_t *out,
                                               char *name) {
  return add_op(BINARY_OP(insert_subtract_layer), cg, lhs, rhs, out, name);
}

flexflow_error_t
    flexflow_computation_graph_add_op_multiply(flexflow_computation_graph_t cg,
                                               flexflow_tensor_t lhs,
                                               flexflow_tensor_t rhs,
                                               flexflow_tensor_t *out,
                                               char *name) {
  return add_op(BINARY_OP(insert_multiply_layer), cg, lhs, rhs, out, name);
}

flexflow_error_t
    flexflow_computation_graph_add_op_divide(flexflow_computation_graph_t cg,
                                             flexflow_tensor_t lhs,
                                             flexflow_tensor_t rhs,
                                             flexflow_tensor_t *out,
                                             char *name) {
  return add_op(BINARY_OP(insert_divide_layer), cg, lhs, rhs, out, name);
}

flexflow_error_t
    flexflow_computation_graph_add_op_max(flexflow_computation_graph_t cg,
                                          flexflow_tensor_t lhs,
                                          flexflow_tensor_t rhs,
                                          flexflow_tensor_t *out,
                                          char *name) {
  return add_op(BINARY_OP(insert_max_layer), cg, lhs, rhs, out, name);
}

flexflow_error_t
    flexflow_computation_graph_add_op_min(flexflow_computation_graph_t cg,
                                          flexflow_tensor_t lhs,
                                          flexflow_tensor_t rhs,
                                          flexflow_tensor_t *out,
                                          char *name) {
  return add_op(BINARY_OP(insert_min_layer), cg, lhs, rhs, out, name);
}

flexflow_error_t
    flexflow_computation_graph_add_op_rsqrt(flexflow_computation_graph_t cg,
                                            flexflow_tensor_t input,
                                            flexflow_tensor_t *out,
                                            char *name) {
  return add_op(BINARY_OP(insert_rsqrt_layer), cg, input, out, name);
}
...
flexflow_error_t flexflow_computation_graph_add_op_scalar_multiply(
    ...
    flexflow_tensor_t *out,
    char *name) {
  return add_op(SCALAR_BINARY_OP(insert_scalar_multiply_layer),
                cg,
                input,
                scalar,
                out,
...
flexflow_error_t flexflow_computation_graph_add_op_scalar_add(
    flexflow_computation_graph_t cg,
    flexflow_tensor_t input,
    float scalar,
    flexflow_tensor_t *out,
    char *name) {
  return add_op(
      SCALAR_BINARY_OP(insert_scalar_add_layer), cg, input, scalar, out, name);
}

flexflow_error_t flexflow_computation_graph_add_op_scalar_sub(
    flexflow_computation_graph_t cg,
    flexflow_tensor_t input,
    float scalar,
    flexflow_tensor_t *out,
    char *name) {
  return add_op(
      SCALAR_BINARY_OP(insert_scalar_sub_layer), cg, input, scalar, out, name);
}
...
flexflow_error_t flexflow_computation_graph_add_op_scalar_truediv(
    ...
    flexflow_tensor_t *out,
    char *name) {
  return add_op(SCALAR_BINARY_OP(insert_scalar_truediv_layer),
                cg,
                input,
                scalar,
                out,
...
flexflow_error_t
    flexflow_computation_graph_add_op_sin(flexflow_computation_graph_t cg,
                                          flexflow_tensor_t input,
                                          flexflow_tensor_t *out,
                                          char *name) {
  return add_op(BINARY_OP(insert_sin_layer), cg, input, out, name);
}

flexflow_error_t
    flexflow_computation_graph_add_op_cos(flexflow_computation_graph_t cg,
                                          flexflow_tensor_t input,
                                          flexflow_tensor_t *out,
                                          char *name) {
  return add_op(BINARY_OP(insert_cos_layer), cg, input, out, name);
}

flexflow_error_t
    flexflow_computation_graph_add_op_relu(flexflow_computation_graph_t cg,
                                           flexflow_tensor_t input,
                                           flexflow_tensor_t *out,
                                           char *name) {
  return add_op(BINARY_OP(insert_relu_layer), cg, input, out, name);
}

flexflow_error_t
    flexflow_computation_graph_add_op_identity(flexflow_computation_graph_t cg,
                                               flexflow_tensor_t input,
                                               flexflow_tensor_t *out,
                                               char *name) {
  return add_op(BINARY_OP(insert_identity_layer), cg, input, out, name);
}

flexflow_error_t
    flexflow_computation_graph_add_op_gelu(flexflow_computation_graph_t cg,
                                           flexflow_tensor_t input,
                                           flexflow_tensor_t *out,
                                           char *name) {
  return add_op(BINARY_OP(insert_gelu_layer), cg, input, out, name);
}

flexflow_error_t
    flexflow_computation_graph_add_op_sigmoid(flexflow_computation_graph_t cg,
                                              flexflow_tensor_t input,
                                              flexflow_tensor_t *out,
                                              char *name) {
  return add_op(BINARY_OP(insert_sigmoid_layer), cg, input, out, name);
}

flexflow_error_t
    flexflow_computation_graph_add_op_tanh(flexflow_computation_graph_t cg,
                                           flexflow_tensor_t input,
                                           flexflow_tensor_t *out,
                                           char *name) {
  return add_op(BINARY_OP(insert_tanh_layer), cg, input, out, name);
}

flexflow_error_t
    flexflow_computation_graph_add_op_elu(flexflow_computation_graph_t cg,
                                          flexflow_tensor_t input,
                                          flexflow_tensor_t *out,
                                          char *name) {
  return add_op(BINARY_OP(insert_tanh_layer), cg, input, out, name);
...
flexflow_error_t
    flexflow_computation_graph_add_op_dropout(flexflow_computation_graph_t cg,
                                              flexflow_tensor_t input,
                                              flexflow_tensor_t *out,
                                              float rate,
...
flexflow_error_t
    flexflow_computation_graph_add_op_gather(flexflow_computation_graph_t cg,
                                             flexflow_tensor_t input,
...
flexflow_error_t
    flexflow_computation_graph_add_op_group_by(flexflow_computation_graph_t cg,
...
flexflow_error_t
    flexflow_computation_graph_add_op_pool2d(flexflow_computation_graph_t cg,
                                             flexflow_tensor_t input,
                                             flexflow_tensor_t *out,
                                             int kernelH,
                                             int kernelW,
...
flexflow_error_t flexcflow_computation_graph_add_op_layer_norm(
    flexflow_computation_graph_t cg,
    flexflow_tensor_t input,
    flexflow_tensor_t *out,
    int *axes,
...
flexflow_error_t flexflow_computation_graph_add_op_batch_norm(
    flexflow_computation_graph_t cg,
    flexflow_tensor_t input,
    flexflow_tensor_t *out,
    bool relu,
  ...
  return handle_errors(out, [&] {
    return batch_norm(
        deref_opaque(cg), c_deref_opaque(input), relu, maybe_string(name));
  });
}

flexflow_error_t flexflow_computation_graph_add_op_batch_matmul(
    flexflow_computation_graph_t cg,
    flexflow_tensor_t lhs,
    flexflow_tensor_t rhs,
    flexflow_tensor_t *out,
...
flexflow_error_t
    flexflow_computation_graph_add_op_cast(flexflow_computation_graph_t cg,
                                           flexflow_tensor_t input,
                                           flexflow_tensor_t *out,
                                           flexflow_datatype_t out_type,
                                           char *name) {
  return handle_errors(out, [&] {
    return cast(deref_opaque(cg),
                c_deref_opaque(input),
...
flexflow_error_t
    flexflow_computation_graph_add_op_concat(flexflow_computation_graph_t cg,
                                             flexflow_tensor_t *inputs,
                                             flexflow_tensor_t *out,
                                             int num_inputs,
                                             int axis,
...
flexflow_error_t
    flexflow_computation_graph_add_op_mean(flexflow_computation_graph_t cg,
                                           flexflow_tensor_t input,
                                           flexflow_tensor_t *out,
                                           int *dims,
                                           int num_dims,
...
flexflow_error_t
    flexflow_computation_graph_add_op_moe(flexflow_computation_graph_t cg,
                                          flexflow_tensor_t input,
                                          flexflow_tensor_t *out,
                                          int num_exp,
                                          int num_select,
...
flexflow_error_t
    flexflow_computation_graph_add_op_split(flexflow_computation_graph_t cg,
                                            flexflow_tensor_t input,
                                            flexflow_tensor_list_t *out,
                                            int *splits,
                                            int num_splits,
...
flexflow_error_t
    flexflow_computation_graph_add_op_flat(flexflow_computation_graph_t cg,
                                           flexflow_tensor_t input,
                                           flexflow_tensor_t *out,
                                           char *name) {
  return handle_errors(out, [&] {
    return flat(deref_opaque(cg), c_deref_opaque(input), maybe_string(name));
  });
}

flexflow_error_t
    flexflow_computation_graph_add_op_softmax(flexflow_computation_graph_t cg,
                                              flexflow_tensor_t input,
                                              flexflow_tensor_t *out,
                                              int dim,
                                              char *name) {
  return handle_errors(out, [&] {
    return softmax(
        deref_opaque(cg), c_deref_opaque(input), dim, maybe_string(name));
  });
}

flexflow_error_t
    flexflow_computation_graph_add_op_transpose(flexflow_computation_graph_t cg,
                                                flexflow_tensor_t input,
                                                flexflow_tensor_t *out,
                                                int *permutation,
                                                ...
                                                char *name) {
  return handle_errors(out, [&] {
    return transpose(deref_opaque(cg),
                     c_deref_opaque(input),
...
flexflow_error_t flexflow_computation_graph_add_op_reduce_sum(
    flexflow_computation_graph_t cg,
    flexflow_tensor_t input,
    flexflow_tensor_t *out,
    int *axes,
    ...
    char *name) {
  return handle_errors(out, [&] {
    return reduce_sum(deref_opaque(cg),
                      c_deref_opaque(input),
                      make_vector(axes, num_axes),
                      keepdims,
                      maybe_string(name));
...
flexflow_error_t
    flexflow_computation_graph_add_op_reshape(flexflow_computation_graph_t cg,
                                              flexflow_tensor_t input,
                                              flexflow_tensor_t *out,
                                              int *shape,
                                              ...
                                              char *name) {
  return handle_errors(out, [&] {
    return reshape(deref_opaque(cg),
                   c_deref_opaque(input),
...
flexflow_error_t
    flexflow_computation_graph_add_op_reverse(flexflow_computation_graph_t cg,
                                              flexflow_tensor_t input,
                                              flexflow_tensor_t *out,
                                              int axis,
                                              char *name) {
  return handle_errors(out, [&] {
    return reverse(
        deref_opaque(cg), c_deref_opaque(input), axis, maybe_string(name));
  });
}

flexflow_error_t
    flexflow_computation_graph_add_op_topk(flexflow_computation_graph_t cg,
                                           flexflow_tensor_t input,
                                           flexflow_tensor_list_t *out,
                                           int k,
                                           bool sorted,
  ...
  return handle_errors(out, [&] {
    return top_k(
        deref_opaque(cg), c_deref_opaque(input), k, sorted, maybe_string(name));
  });
}
...
flexflow_error_t flexflow_computation_graph_add_op_multihead_attention(
    ...
    char *name) {
  return handle_errors(out, [&] {
    return multihead_attention(deref_opaque(cg),
