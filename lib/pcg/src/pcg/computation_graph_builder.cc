...
tensor_guid_t ComputationGraphBuilder::multihead_attention(
  ...
  LayerAttrs layer = LayerAttrs{ComputationGraphOpAttrs{attrs}, name};
  TensorShape output_shape = throw_if_unexpected(get_output_shape(attrs, get_shape(query), get_shape(key), get_shape(value)));

  TensorShape weights_shape = throw_if_unexpected(get_weights_shape(attrs, 
    get_shape(query), get_shape(key), get_shape(value)));
  TensorAttrs weight_attrs = make_weight_attrs(weights_shape, initializer);

  return this->add_layer(layer, std::vector<tensor_guid_t>{query, key, value}, {weight_attrs}, output_shape);
}
