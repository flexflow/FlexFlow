...
/* static V1OperatorGraph to_v1(OperatorGraphView const &g, bidict<Node, size_t>
 * const &nodes) { */
...
/*   bidict<size_t, OperatorGraphOutput> outputs_bidict =
 * enumerate(get_outputs(g)); */
/*   std::unordered_map<size_t, V1GraphOutput> outputs = */
/*       map_values(outputs_bidict, [&](OperatorGraphOutput const &o) { */
/*         return V1GraphOutput{nodes.at_r(get_node(o)),
 * size_t_from_int(get_idx(o))}; */
/*       }); */
...
template <typename NodeLabel, typename OutputLabel>
static bidict<size_t, NodePort>
    get_ports_by_idx(DataflowGraph<NodeLabel, OutputLabel> const &g) {
...
static V1JsonableGraph<NodeLabel, OutputLabel>
  ...
  V1MultiDiGraph unlabelled =
      to_v1(g.get_raw_graph(), nodes.reversed(), node_ports.reversed());
  std::unordered_map<size_t, NodeLabel> node_labels =
      map_values(nodes, [&](Node const &n) { return g.at(n); });

  bidict<size_t, MultiDiOutput> outputs_bidict =
      enumerate(get_outputs(g.get_raw_graph()));
  std::unordered_map<size_t, V1GraphOutput> outputs =
