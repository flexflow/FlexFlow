...
V1MultiDiGraph to_v1(MultiDiGraphView const &g) {
  return to_v1(g, enumerate(get_nodes(g)).reversed(), enumerate(get_node_ports(g)).reversed());
}

V1MultiDiGraph to_v1(MultiDiGraphView const &g, std::unordered_map<Node, size_t> const &nodes, std::unordered_map<NodePort, size_t> const &node_ports) {
...
V1JsonableGraph<
  decltype(to_v1(std::declval<NodeLabel>())), 
...
> to_v1(OutputLabelledMultiDiGraph<NodeLabel, OutputLabel> const &g) {
  ...
  std::unordered_map<size_t, V1NodeLabel> node_labels = map_values(nodes, [&](Node const &n) { return to_v1(g.at(n)); });
  bidict<size_t, MultiDiOutput> outputs_bidict = enumerate(get_outputs(g));
  std::unordered_map<size_t, V1GraphOutput> outputs = map_values(outputs_bidict, [&](MultiDiOutput const &o) { return V1GraphOutput{ nodes.at_r(o.node), node_ports.at_r(o.idx) }; });
  std::unordered_map<size_t, V1OutputLabel> output_labels = map_values(outputs_bidict, [&](MultiDiOutput const &o) { return to_v1(g.at(o)); });
