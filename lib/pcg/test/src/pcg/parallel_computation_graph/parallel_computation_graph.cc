...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("topological_ordering") {
    // TODO(@lockshaw) should probably be replaced with a rapidcheck test that compares
    // ParallelComputationGraph to DataflowGraph, but since we currently don't have rapidcheck
    // generation for DataflowGraph this will have to do for now
    ...
    ParallelLayerAddedResult layer1_added = add_parallel_layer(pcg, layer_label, {}, {tensor_label});
    // parallel_layer_guid_t layer1 = layer1_added.parallel_layer;
    // parallel_tensor_guid_t tensor1 = get_only(layer1_added.outputs);
    //
    // ParallelLayerAddedResult layer2_added = add_parallel_layer(pcg, layer_label, {tensor1}, {tensor_label});
    // parallel_layer_guid_t layer2 = layer2_added.parallel_layer;
    // parallel_tensor_guid_t tensor2 = get_only(layer2_added.outputs);
    //
    // ParallelLayerAddedResult layer3_added = add_parallel_layer(pcg, layer_label, {tensor2}, {tensor_label});
