...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("MachineView general util functions") {
    StridedRectangle rect{{
      StridedRectangleSide{num_points_t{7}, 5},
  ...
  TEST_CASE("MachineView make_1d_machine_view") {
    ...
    CHECK(make_1d_machine_view(start_gpu, get_side_size(rect.sides.at(ff_dim_t{0})), 5) == gpu_mv);

    CHECK(make_1d_machine_view(start_cpu, cpu_id_t(3+7*5), 5) == cpu_mv);
    CHECK(make_1d_machine_view(start_cpu, num_points_t{7}, 5) == cpu_mv);
    CHECK(make_1d_machine_view(start_cpu, get_side_size(rect.sides.at(ff_dim_t{0})), 5) == cpu_mv);
