...
// public:
//   OpenDataflowGraph()
//     : g(OutputLabelledOpenMultiDiGraph<NodeLabel, OutputLabel>::template
//     create<
...
//   }
//
//   std::vector<MultiDiOutput> add_operator(NodeLabel const &func,
//   std::vector<DataflowInput> const &inputs, std::vector<OutputLabel> const
//   &outputs) {
//     Node n = this->g.add_node(func);
//     for (auto const &[idx, input] : enumerate_vector(inputs)) {
//       this->g.add_edge(MultiDiEdge{input.src, input.src_idx, n,
//       this->make_port_for_idx(idx)});
//     }
//
