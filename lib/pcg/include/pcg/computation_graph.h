...
struct ComputationGraph
  ...
  std::vector<operator_guid_t> traverse_reverse_order() {
    std::vector<Node> layers = reversed<std::vector<Node>>(get_topological_ordering(this->value()));
    return transform(layers, [&](Node const &e) -> operator_guid_t {
      return operator_guid_t{e};
    });
  }
  ...
  std::vector<tensor_guid_t> sort_edge_set(std::unordered_set<MultiDiEdge> edges) {
    std::unordered_set<MultiDiOutput> outputs = transform(edges, [&](MultiDiEdge const &e) -> MultiDiOutput {
    ...
    return transform(sorted_outputs, [&](MultiDiOutput const &e) -> tensor_guid_t {
  ...
  tensor_guid_t create_outgoing_edge_with_label(operator_guid_t node, int idx, Tensor tensor) {
    tensor_guid_t tensor_guid = create_outgoing_edge(node, idx);
    add_output(tensor_guid, tensor);
    return tensor_guid;
  }

  void add_incoming_edges(std::vector<tensor_guid_t> const & incoming_edges, operator_guid_t node) {
