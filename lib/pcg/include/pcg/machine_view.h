...
MachineView make_1d_machine_view(device_id_t start, device_id_t stop, int stride = 1);

MachineView make_1d_machine_view(gpu_id_t start, num_points_t num_points, int stride = 1);
MachineView make_1d_machine_view(cpu_id_t start, num_points_t num_points, int stride = 1);
MachineView make_1d_machine_view(device_id_t start, num_points_t num_points, int stride = 1);

MachineView make_1d_machine_view(gpu_id_t start, side_size_t interval_size, int stride = 1);
MachineView make_1d_machine_view(cpu_id_t start, side_size_t interval_size, int stride = 1);
