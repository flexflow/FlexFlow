...
public:
  ...
  parallel_tensor_guid_t create_input_tensor(ParallelTensorShape const &shape, 
                                             bool create_grad = true,
                                             std::optional<std::string> const &name = std::nullopt);

  parallel_tensor_guid_t add(parallel_tensor_guid_t const &lhs,
  ...
  parallel_tensor_guid_t cast(parallel_tensor_guid_t const &input,
  ...
  parallel_tensor_guid_t relu(parallel_tensor_guid_t const &x,
                     std::optional<std::string> const &name = std::nullopt);

  parallel_tensor_guid_t parallel_partition(parallel_tensor_guid_t const &x,
                                     ff_dim_t dim, 
                                     int degree,
                                     std::optional<std::string> const &name = std::nullopt);
  parallel_tensor_guid_t parallel_combine(parallel_tensor_guid_t const &x,
                                 ff_dim_t dim,
                                 int degree, 
                                 std::optional<std::string> const &name = std::nullopt);
  parallel_tensor_guid_t parallel_replicate(parallel_tensor_guid_t const &x,
                                            int degree,
                                            std::optional<std::string> const &name = std::nullopt);
  parallel_tensor_guid_t parallel_reduce(parallel_tensor_guid_t const &x,
                                         int degree,
                                         std::optional<std::string> const &name = std::nullopt);

private:
  parallel_tensor_guid_t as_type(parallel_tensor_guid_t const &, DataType, std::string const &name);
private:
  ParallelTensorShape get_shape(parallel_tensor_guid_t const &) const;
private:
  std::vector<parallel_tensor_guid_t> add_layer(ParallelLayerAttrs const &layer,
  ...
  parallel_tensor_guid_t add_layer(ParallelLayerAttrs const &layer,
                          std::vector<parallel_tensor_guid_t> const &inputs,
                          std::vector<ParallelTensorAttrs> const &weights,
