...
struct PerfMetrics : public use_visitable_cmp<PerfMetrics> {
  ...
  PerfMetrics(int train_all, optional<int> train_correct,
              optional<float> cce_loss, optional<float> sparse_cce_loss,
              optional<float> mse_loss, optional<float> rmse_loss,
              optional<float> mae_loss, double start_time_micro,
...
VISITABLE_STRUCT(::FlexFlow::PerfMetrics, train_all, train_correct, cce_loss,
                 sparse_cce_loss, mse_loss, rmse_loss, mae_loss, start_time);

namespace fmt {

template <> struct formatter<::FlexFlow::PerfMetrics> : formatter<std::string> {
  ...
  auto format(::FlexFlow::PerfMetrics const &m, FormatContext &ctx)
    ...
    if (m.train_correct.has_value()) {
      fmt::format_to(std::back_inserter(out), " accuracy={:.2f}%",
                     100.0 * get_accuracy(m));
    }
    if (m.cce_loss.has_value()) {
      fmt::format_to(std::back_inserter(out), " cce={:.2f}",
                     m.cce_loss.value());
    }
    if (m.sparse_cce_loss.has_value()) {
      fmt::format_to(std::back_inserter(out), " sparse_cce={:.2f}",
                     m.sparse_cce_loss.value());
    }
    if (m.mse_loss.has_value()) {
      fmt::format_to(std::back_inserter(out), " mse={:.2f}",
                     m.mse_loss.value());
    }
    if (m.rmse_loss.has_value()) {
      fmt::format_to(std::back_inserter(out), " rmse={:.2f}",
                     m.rmse_loss.value());
    }
    if (m.mae_loss.has_value()) {
      fmt::format_to(std::back_inserter(out), " mae={:.2f}",
                     m.mae_loss.value());
    }
    fmt::format_to(std::back_inserter(out), "throughput={:.2f}",
                   get_throughput(m));
