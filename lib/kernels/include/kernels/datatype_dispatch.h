...
template <template <DataType> class F, 
          typename ...Args,
          typename Out = decltype(std::declval<F<DataType::FLOAT>>()(std::declval<Args>()...))
         >
...
struct DataTypeDispatch1 {
  ...
  struct Type1Dispatch {
    template <typename ...Args, typename Out = decltype(std::declval<F<DataType::FLOAT>>()(std::declval<Args>()...))>
