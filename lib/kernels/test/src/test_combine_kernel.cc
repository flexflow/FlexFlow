...
TEST_SUITE(FF_TEST_SUITE) {
    ...
    TEST_CASE("Test combine kernel backward") {
        ...
        Allocator allocator = get_local_memory_allocator();
        void* grad_output_data_ptr = allocator.allocate(100 * 100 * 
                                                        sizeof(float));
        void* grad_input_data_ptr = allocator.allocate(100 * 100 * 
                                                       sizeof(float));
        ...
        Kernels::Combine::backward_kernel(stream, accessorRGrad, 
                                          accessorWGrad);
