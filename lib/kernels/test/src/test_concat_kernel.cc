...
TEST_SUITE(FF_TEST_SUITE) {
    TEST_CASE("Test concat kernel forward and backward") {
        ...
        for (int i = 0; i < num_inputs; i++) {
            void* input_data_ptr = allocator.allocate(size_per_input * 
                                                      sizeof(float));
            input_ptrs.push_back(input_data_ptr);
            GenericTensorAccessorR accessor{ DataType::FLOAT, shape, 
                                            input_data_ptr };
        ...
        void* output_data_ptr = allocator.allocate(num_inputs * 
                                                   size_per_input * 
                                                   sizeof(float));
        const GenericTensorAccessorW output_accessor{ DataType::FLOAT,
                                                      shape, 
                                                      output_data_ptr};
        ...
        Kernels::Concat::forward_kernel(stream, output_accessor, 
                                        input_accessors, concat_axis);
        ...
        for (int i = 0; i < num_inputs; i++) {
            void* grad_input_data_ptr = allocator.allocate(size_per_input * 
                                                           sizeof(float));
            ...
            cudaMemset(grad_input_data_ptr, 0, 
                       size_per_input * sizeof(float));
        }

        void* grad_output_data_ptr = allocator.allocate(num_inputs * 
                                                        size_per_input * 
                                                        sizeof(float));
        checkCUDA(cudaMemcpy(grad_output_data_ptr, host_output_data.data(),
                             host_output_data.size() * sizeof(float), 
                             cudaMemcpyHostToDevice));
        const GenericTensorAccessorR 
            grad_output_accessor{ DataType::FLOAT, shape, 
        ...
        for (int i = 0; i < num_inputs; i++) {
            ...
            for (int j = 0; j < size_per_input; j++) {
                REQUIRE(
                    host_grad_input[j] == host_output_data[i * 
                                                           size_per_input +j]);
            }
        }
