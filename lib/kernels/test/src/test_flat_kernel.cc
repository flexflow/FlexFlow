...
TEST_SUITE(FF_TEST_SUITE) {
    TEST_CASE("Test Flat Kernel Forward and Backward") {
        ...
        float* add_data = static_cast<float*>(allocator.allocate(num_elements * sizeof(float)));
        checkCUDA(cudaMemcpy(add_data, host_output_data.data(), num_elements * sizeof(float), cudaMemcpyHostToDevice));
        const GenericTensorAccessorR data_accessor{DataType::FLOAT, shape, add_data};

        Kernels::Flat::backward_kernel(stream, input_accessor, output_data, add_data);

        std::vector<float> backward_output_data(num_elements);
        checkCUDA(cudaMemcpy(backward_output_data.data(), output_data, num_elements * sizeof(float), cudaMemcpyDeviceToHost));
