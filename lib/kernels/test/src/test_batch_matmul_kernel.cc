...
void allocate_ptrs(std::vector<float **> &gpu_data_ptrs,
  ...
  for (size_t i = 0; i < gpu_data_ptrs.size(); ++i) {
    *gpu_data_ptrs[i] = static_cast<float *>(allocator.allocate(num_elements[i] * sizeof(float)));
  }
}
...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("Test BatchMatmul Forward and Backward Kernel") {
    ...
    std::vector<size_t> sizes = {num_elements_a, num_elements_b,
                                 num_elements_output};
    ...
    Kernels::BatchMatmul::forward_kernel(
        stream, handle, output, a_input, b_input, m, n, k, batch,
        a_seq_length_dim, b_seq_length_dim, seq_length);

    std::vector<float> host_output(num_elements_output);
    cudaMemcpy(host_output.data(), output, num_elements_output * sizeof(float),
    ...
    Kernels::BatchMatmul::backward_kernel(stream, handle, output, o_grad,
                                          a_input, a_grad, b_input, b_grad, m,
                                          n, k, batch);
