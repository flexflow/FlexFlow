...
struct Arbitrary<PerfMetrics> {
    static Gen<PerfMetrics> arbitrary() {
        return gen::build<PerfMetrics>(
            gen::set(&PerfMetrics::train_all),
...
RC_GTEST_PROP(PerfMetricsTests, "Throughput non-negative", ()) {
    const auto m = *rc::gen::arbitrary<PerfMetrics>();
...
RC_GTEST_PROP(PerfMetricsTests, "Update maintains non-negative values", ()) {
    const auto lhs = *rc::gen::arbitrary<PerfMetrics>();
    const auto rhs = *rc::gen::arbitrary<PerfMetrics>();
    const auto result = update(lhs, rhs);
