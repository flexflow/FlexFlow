...
TEST_SUITE(FF_TEST_SUITE) {
    TEST_CASE("Test Transpose Forward Kernel") {
        ...
        TransposePerDeviceState state = Kernels::Transpose::init_kernel(num_dims, perm);
        ...
        std::generate(host_input_data.begin(), host_input_data.end(), []() { return static_cast<float>(rand()) / RAND_MAX; });
        ...
        checkCUDA(cudaMemcpy(output_data, host_output_data.data(), num_elements * sizeof(float), cudaMemcpyHostToDevice));

        cudaStream_t stream;
        checkCUDA(cudaStreamCreate(&stream));

        const GenericTensorAccessorR input_accessor{DataType::FLOAT, shape, input_data};
        const GenericTensorAccessorW output_accessor{DataType::FLOAT, shape, output_data};

        Kernels::Transpose::forward_kernel(stream, state, input_accessor, output_accessor); 
        ...
        for (int o_idx = 0; o_idx < num_elements; ++o_idx) {
            ...
            CHECK(doctest::Approx(host_input_data[i_index]) == check_output_data[o_idx]);
    ...
    TEST_CASE("Test Transpose Backward Kernel") {
        ...
        Allocator allocator = get_local_memory_allocator();
        float* out_grad_data = static_cast<float*>(allocator.allocate(num_elements * sizeof(float)));
        float* in_grad_data = static_cast<float*>(allocator.allocate(num_elements * sizeof(float)));

        std::vector<float> host_out_grad_data(num_elements);
        std::generate(host_out_grad_data.begin(), host_out_grad_data.end(), []() { return static_cast<float>(rand()) / RAND_MAX; });
        checkCUDA(cudaMemcpy(out_grad_data, host_out_grad_data.data(), num_elements * sizeof(float), cudaMemcpyHostToDevice));

        std::vector<float> host_in_grad_data(num_elements, 0.0f);
        checkCUDA(cudaMemcpy(in_grad_data, host_in_grad_data.data(), num_elements * sizeof(float), cudaMemcpyHostToDevice));

        cudaStream_t stream;
        checkCUDA(cudaStreamCreate(&stream));

        const GenericTensorAccessorR out_grad_accessor{DataType::FLOAT, shape, out_grad_data};
        const GenericTensorAccessorW in_grad_accessor{DataType::FLOAT, shape, in_grad_data};

        Kernels::Transpose::backward_kernel(stream, state, in_grad_accessor, out_grad_accessor);

        std::vector<float> check_in_grad_data(num_elements);
        checkCUDA(cudaMemcpy(check_in_grad_data.data(), in_grad_data, num_elements * sizeof(float), cudaMemcpyDeviceToHost));
        ...
        for (int i_idx = 0; i_idx < num_elements; ++i_idx) {
            ...
            CHECK(doctest::Approx(host_out_grad_data[i_idx]) == check_in_grad_data[o_idx]);
        }
