...
TEST_SUITE(FF_TEST_SUITE) {
    TEST_CASE("Test multi-head attention forward kernel") {
        ...
        void* query_ptr = allocator.allocate(num_samples * qoSeqLength * 
                                             qSize * sizeof(float));
        void* key_ptr = allocator.allocate(num_samples * kvSeqLength * 
                                           kSize * sizeof(float));
        void* value_ptr = allocator.allocate(num_samples * kvSeqLength * 
                                             vSize * sizeof(float));
        ...
        for (auto& val : host_query) val = dist(gen);
        for (auto& val : host_key) val = dist(gen);
        for (auto& val : host_value) val = dist(gen);
        for (auto& val : host_weight) val = dist(gen);

        checkCUDA(cudaMemcpy(query_ptr, host_query.data(), host_query.size() * 
                             sizeof(float), cudaMemcpyHostToDevice));
        checkCUDA(cudaMemcpy(key_ptr, host_key.data(), host_key.size() * 
                             sizeof(float), cudaMemcpyHostToDevice));
        ...
        Kernels::MultiHeadAttention::
                    forward_kernel(stream, state,
   ...
   TEST_CASE("Test multi-head attention backward kernel") {
    ...
    void* query_grad_ptr = allocator.allocate(num_samples * qoSeqLength * 
                                              qSize * sizeof(float));
    void* key_grad_ptr = allocator.allocate(num_samples * kvSeqLength * 
                                            kSize * sizeof(float));
    void* value_grad_ptr = allocator.allocate(num_samples * kvSeqLength * 
                                              vSize * sizeof(float));
    void* weight_grad_ptr = allocator.allocate(state.weightSize);
    void* output_grad_ptr = allocator.allocate(num_samples * qoSeqLength * 
                                               oProjSize * sizeof(float));

    cudaMemset(query_grad_ptr, 0, num_samples * qoSeqLength * 
               qSize * sizeof(float));
    cudaMemset(key_grad_ptr, 0, num_samples * kvSeqLength * 
               kSize * sizeof(float));
    cudaMemset(value_grad_ptr, 0, num_samples * kvSeqLength * 
               vSize * sizeof(float));
    cudaMemset(weight_grad_ptr, 0, state.weightSize);
    cudaMemset(output_grad_ptr, 0, num_samples * qoSeqLength * 
               oProjSize * sizeof(float));
    ...
    for (auto& val : host_value) val = dist(gen);
    for (auto& val : host_weight) val = dist(gen);
    for (auto& val : host_output) val = dist(gen);
    for (auto& val : host_output_grad) val = dist(gen);
