...
void forward_kernel(cudaStream_t stream, TransposePerDeviceState const *m,
...
void backward_kernel(cudaStream_t stream, TransposePerDeviceState const *m,
  ...
  transpose_simple_kernel<<<GET_BLOCKS(in_grad_domain.get_volume()),
                            CUDA_NUM_THREADS, 0, stream>>>(
      in_grad_domain.get_volume(), output_grad_ptr, input_grad_ptr, info,
      1.0f /*beta*/);
}
