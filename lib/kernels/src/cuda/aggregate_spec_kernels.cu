...
void forward_kernel(cudaStream_t stream, AggregateSpecPerDeviceState const *m,
  ...
  // call forward kernel
  cudaMemcpy(m->dev_region_ptrs, exp_preds, n * sizeof(float *),
             cudaMemcpyHostToDevice);

  aggspec_forward_kernel<<<
      GET_BLOCKS(batch_size * k * out_dim),
      min(CUDA_NUM_THREADS, (int)(batch_size * k * out_dim)), 0, stream>>>(
      m->dev_region_ptrs, acc_gate_assign_ptr, acc_output_ptr, n, k, rows,
      batch_size, out_dim);
}
...
void backward_kernel(cudaStream_t stream, AggregateSpecPerDeviceState const *m,
  ...
  cudaMemcpy(m->dev_region_ptrs, exp_grads, n * sizeof(float *),
             cudaMemcpyHostToDevice);

  aggspec_backward_kernel<<<
      GET_BLOCKS(batch_size * k * out_dim),
      min(CUDA_NUM_THREADS, (int)(batch_size * k * out_dim)), 0, stream>>>(
