...
void init_kernel(Pool2DPerDeviceState *m, int input_w, int input_h, int input_c,
  ...
  checkCUDNN(cudnnSetPooling2dDescriptor(m->poolDesc, mode, CUDNN_PROPAGATE_NAN,
                                         kernel_h, kernel_w, pad_h, pad_w,
                                         stride_h, stride_w));
  int n, c, h, w;
  checkCUDNN(cudnnGetPooling2dForwardOutputDim(m->poolDesc, m->inputTensor, &n,
                                               &c, &h, &w));
  assert(n == output_n);
  assert(c == output_c);
  assert(h == output_h);
...
void forward_kernel(cudaStream_t stream, Pool2DPerDeviceState const *m,
  ...
  checkCUDNN(cudnnPoolingForward(m->handle.dnn, m->poolDesc, &alpha,
                                 m->inputTensor, input_ptr, &beta,
...
void backward_kernel(cudaStream_t stream, Pool2DPerDeviceState const *m,
  ...
  float alpha = 1.0f;
  checkCUDNN(cudnnPoolingBackward(m->handle.dnn, m->poolDesc, &alpha,
                                  m->outputTensor, output_ptr, m->outputTensor,
                                  output_grad_ptr, m->inputTensor, input_ptr,
