...
void init_kernel(ElementBinaryPerDeviceState *m, Domain const &input1_domain,
  ...
  checkCUDNN(miopenSetReduceTensorDescriptor(
      m->reduceAddDesc, MIOPEN_REDUCE_TENSOR_ADD, miopenFloat,
...
void forward_kernel(hipStream_t stream, ElementBinaryPerDeviceState const *m,
  ...
  if (m->broadcast_input1) {
    ...
    checkCUDNN(miopenOpTensor(m->handle.dnn, m->opDesc, &beta, m->outputTensor,
                              out_ptr, &alpha1, m->input1Tensor, in1_ptr, &beta,
                              m->outputTensor, out_ptr));
    checkCUDNN(miopenOpTensor(m->handle.dnn, m->opDesc, &beta, m->outputTensor,
                              out_ptr, &alpha2, m->input2Tensor, in2_ptr,
                              &alpha1, m->outputTensor, out_ptr));
  } else {
    checkCUDNN(miopenOpTensor(
        m->handle.dnn, m->opDesc, &alpha1, m->input1Tensor, in1_ptr, &alpha2,
        m->input2Tensor, in2_ptr, &beta, m->outputTensor, out_ptr));
  }
}

void backward_kernel(hipStream_t stream, ElementBinaryPerDeviceState const *m,
  ...
  if (m->op_type == OP_EW_ADD || m->op_type == OP_EW_SUB) {
    ...
    if (in1_grad_ptr != nullptr) {
      if (m->broadcast_input1) {
        ...
        checkCUDNN(miopenOpTensor(m->handle.dnn, miopenTensorOpAdd, &alpha,
                                  ...
                                  m->outputTensor, out_grad_ptr, &beta,
    ...
    if (in1_grad_ptr != nullptr) {
      checkCUDNN(miopenOpTensor(m->handle.dnn, m->opDesc, &alpha1,
                                m->outputTensor, out_grad_ptr, &alpha2,
    ...
    if (in2_grad_ptr != nullptr) {
      checkCUDNN(miopenOpTensor(m->handle.dnn, m->opDesc, &alpha1,
                                m->outputTensor, out_grad_ptr, &alpha2,
                                m->input2Tensor, in1_ptr, &beta,
                                m->input1Tensor, in2_grad_ptr));
    }
  } else {
