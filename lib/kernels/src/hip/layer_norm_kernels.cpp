...
template <DataType T> struct ForwardKernel {
  void operator()(hipStream_t stream, LayerNormPerDeviceState const *m,
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(RowwiseMomentsCUDAKernel<float>),
                       m->effective_batch_size, kCUDABlockReduceNumThreads, 0,
                       stream, m->effective_num_elements, m->eps,
                       input.get<T>(), m->mean_ptr, m->rstd_ptr);
    hipLaunchKernelGGL(HIP_KERNEL_NAME(LayerNormForwardCUDAKernel<float>),
                       m->effective_batch_size, kCUDANumThreads, 0, stream,
                       m->effective_num_elements, input.get<T>(), m->mean_ptr,
                       m->rstd_ptr, gamma.get<T>(), beta.get<T>(),
                       output.get<T>());
  }
}
...
struct BackwardKernel {
  void operator()(hipStream_t stream, LayerNormPerDeviceState const *m,
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(ComputeInternalGradientsCUDAKernel<T>),
                       ...
                       output_grad.get<T>(), input.get<T>(), gamma.get<T>(),
                       m->ds_ptr, m->db_ptr);
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(ComputeGradientFusedParamsCUDAKernel<T>),
                       B, kCUDANumThreads, 0, stream, M, N, m->mean_ptr,
                       m->rstd_ptr, m->ds_ptr, m->db_ptr, m->scale_ptr,
                       m->bias_ptr);
    if (gamma_grad.get<T>() != NULL || beta_grad.get<T>() != NULL) {
      if (M < 512) {
        ...
        hipLaunchKernelGGL(
            HIP_KERNEL_NAME(GammaBetaBackwardSimpleCUDAKernel<T>), B,
            kCUDANumThreads, 0, stream, M, N, output_grad.get<T>(),
            input.get<T>(), m->mean_ptr, m->rstd_ptr, gamma_grad.get<T>(),
            beta_grad.get<T>());
      } else {
        const int64_t B =
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(GammaBetaBackwardCUDAKernel<T>), B,
                           dim3(kThreadX, kThreadY), 0, stream, M, N,
                           output_grad.get<T>(), input.get<T>(), m->mean_ptr,
                           m->rstd_ptr, gamma_grad.get<T>(),
                           beta_grad.get<T>());
      }
    }
  }

  void forward_kernel(hipStream_t stream, LayerNormPerDeviceState const *m,
                      GenericTensorAccessorR const &input,
                      GenericTensorAccessorW const &output,
                      GenericTensorAccessorW const &gamma,
                      GenericTensorAccessorW const &beta) {
  ...
  void backward_kernel(hipStream_t stream, LayerNormPerDeviceState const *m,
                       GenericTensorAccessorR const &output_grad,
                       GenericTensorAccessorR const &input,
                       GenericTensorAccessorW const &input_grad,
                       GenericTensorAccessorR const &gamma,
...
#pragma unroll
  ...
  template <typename T>
  __global__ void RowwiseMomentsCUDAKernel(int64_t N, T eps, T const *X,
                                           T *mean, T *rstd) {
  ...
  template <typename T>
  __global__ void LayerNormForwardCUDAKernel(int64_t N, T const *X,
                                             T const *mean, T const *rstd,
                                             T const *gamma, T const *beta,
                                             T *Y) {
    using T_ACC = T;
    const int64_t i = blockIdx.x;
    for (int64_t j = threadIdx.x; j < N; j += blockDim.x) {
  ...
  template <typename T>
  __global__ void LayerNormBackwardCUDAKenrel(int64_t N, T const *dY,
                                              T const *X, T const *gamma,
                                              T const *a, T const *b,
                                              T const *c, T *dX) {
    using T_ACC = T;
  ...
  template <typename T>
  __global__ void
  GammaBetaBackwardSimpleCUDAKernel(int64_t M, int64_t N, T const *dY,
                                    T const *X, T const *mean, T const *rstd,
                                    T *dg, T *db) {
    using T_ACC = T;
