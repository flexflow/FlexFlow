...
__host__ void relu_backward_kernel(DataType data_type, void *output_grad_ptr,
  ...
  if (data_type == DT_FLOAT) {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(reluBackward<float>),
                       ...
                       (float *)output_grad_ptr, (float const *)output_ptr,
                       output_size);
  } else if (data_type == DT_DOUBLE) {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(reluBackward<double>),
                       GET_BLOCKS(output_size), CUDA_NUM_THREADS, 0, stream,
                       (double *)output_grad_ptr, (double const *)output_ptr,
                       output_size);
  } else {
...
__host__ void sigmoid_backward_kernel(DataType data_type, void *output_grad_ptr,
                                      ...
                                      size_t output_size, hipStream_t stream) {
  if (data_type == DT_FLOAT) {
...
__host__ void updateGAS(float *para_ptr, float const *grad_ptr,
  ...
  for (int i = 1; i < num_replica; i++) {
    ...
    hipLaunchKernelGGL(apply_add, GET_BLOCKS(replica_size), CUDA_NUM_THREADS, 0,
                       stream, (float *)grad_ptr, replica, replica_size);
  }
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(apply_add_with_scale<float>),
                     GET_BLOCKS(replica_size), CUDA_NUM_THREADS, 0, stream,
...
cudnnSetTensorDescriptorFromDomain(miopenTensorDescriptor_t tensor,
  ...
  case 2: {
    ...
    return miopenSet4dTensorDescriptor(tensor, miopenFloat, dims[1], dims[0], 1,
                                       1);
  }
  case 3: {
    Rect<3> rect = domain;
    ...
    return miopenSet4dTensorDescriptor(tensor, miopenFloat, dims[2], dims[1],
                                       dims[0], 1);
  }
  case 4: {
    Rect<4> rect = domain;
    ...
    return miopenSet4dTensorDescriptor(tensor, miopenFloat, dims[3], dims[2],
                                       dims[1], dims[0]);
  }
  case 5: {
    Rect<5> rect = domain;
...
template __global__ void add_kernel<long>(long *dst, long const *src,
                                          size_t size);
