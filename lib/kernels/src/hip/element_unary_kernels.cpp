...
template <DataType T> struct ForwardKernel {
  void operator()(ffStream_t stream, ElementUnaryPerDeviceState const *m,
    ...
    if (use_cudnn(m->op_type)) {
      ...
      checkCUDNN(miopenActivationForward(m->handle.dnn, m->actiDesc, &alpha,
                                         m->inputTensor, input.get<T>(), &beta,
                                         m->outputTensor, output.get<T>()));
    } else {
      ...
      hipLaunchKernelGGL(HIP_KERNEL_NAME(elewise_unary_forward_kernel),
                         ...
                         num_elements, (T)m->scalar, m->op_type, input.get<T>(),
                         output.get<T>());
    }
  }
}
...
struct BackwardKernel {
  void operator()(ffStream_t stream, ElementUnaryPerDeviceState const *m,
    ...
    if (use_cudnn(m->op_type)) {
      float alpha = 1.0f;
      float beta = 0.0f;
      checkCUDNN(miopenActivationBackward(
                     m->handle.dnn, m->actiDesc, &alpha, m->outputTensor,
                     output.get<T>(), m->outputTensor, output_grad.get<T>()),
                 m->inputTensor, input.get<T>(), &beta, m->inputTensor,
                 input_grad.get<T>());
    } else {
      size_t num_elements = input.shape.num_elements();
      hipLaunchKernelGGL(HIP_KERNEL_NAME(elewise_unary_backward_kernel<T>),
                         GET_BLOCKS(num_elements), CUDA_NUM_THREADS, 0, stream,
                         num_elements, m->scalar, m->op_type, output.get<T>(),
                         output_grad.get<T>(), input.get<T>(),
                         input_grad.get<T>());
    }
  }
  ...
  void backward_kernel(ffStream_t stream, ElementUnaryPerDeviceState const *m,
                       ...
                       GenericTensorAccessorW const &output,
                       GenericTensorAccessorW const &output_grad)
      DataTypeDispatch1<BackwardKernel>{}(m->data_type, stream, m, input,
                                          input_grad, output, output_grad);
}

template <typename T>
__global__ void elewise_unary_forward_kernel(coord_t volume, const T scalar,
                                             OperatorType type, T const *in,
                                             T *out) {
...
template <typename T>
__global__ void
elewise_unary_backward_kernel(coord_t volume, const T scalar, OperatorType type,
                              T const *output, T const *output_grad,
                              T const *input, T *input_grad) {
  CUDA_KERNEL_LOOP(i, volume) {
    switch (type) {
    case OP_EXP: {
