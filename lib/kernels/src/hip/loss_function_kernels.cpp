...
void Loss::sparse_categorical_crossentropy_loss_backward_kernel_wrapper(
  ...
  checkCUDA(hipMemcpy(logit_grad_ptr, logit_ptr, logit_volume * sizeof(float),
  ...
  hipLaunchKernelGGL(scale_kernel, GET_BLOCKS(logit_grad_volume),
                     CUDA_NUM_THREADS, 0, stream, logit_grad_ptr,
                     logit_grad_volume, 0, scale_factor * k);
}
...
void Loss::identity_loss_backward_kernel_wrapper(float *loss_grad_ptr,
  ...
  hipStream_t stream;
  checkCUDA(get_legion_stream(&stream));
  hipLaunchKernelGGL(identity_loss_backward, GET_BLOCKS(loss_volume),
                     CUDA_NUM_THREADS, 0, stream, loss_grad_ptr, loss_ptr,
                     loss_volume);
  // Scale logit gradients by loss->scale_factor
  hipLaunchKernelGGL(scale_kernel, GET_BLOCKS(loss_grad_volume),
                     CUDA_NUM_THREADS, 0, stream, loss_grad_ptr,
                     loss_grad_volume, 0, scale_factor);
}
