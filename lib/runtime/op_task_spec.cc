...
Legion::PrivilegeMode get_default_fwd_privs(TensorRole tensor_role,
                                            IsGrad is_grad) {
...
Legion::PrivilegeMode get_default_bwd_privs(TensorRole tensor_role,
                                            IsGrad is_grad) {
...
OpTensorSpec output_tensor(int idx) {
  return {TensorRole::OUTPUT, idx, IsTrainable::YES, IsGrad::NO};
}

OpTensorSpec param_tensor(int idx) {
  return {TensorRole::PARAM, idx, IsTrainable::YES, IsGrad::NO};
}

void OpTaskSignature::add_input_slot(slot_id, SlotType slot_type) {
  this.op_tensor_slots.insert({slot_id,
                               slot_type,
                               TensorRole::INPUT,
                               IsGrad::NO,
...
using TensorSpecCollection =
    std::unordered_map<slot_id,
                       variant<OpTensorSpec, std::vector<OpTensorSpec>>>;

static TensorSpecCollection
    collect_tensor_specs(OpTaskSignature const &signature,
  ...
  auto privs_satisfies_slot = [&](TensorSlotSpec const &slot,
                                  Legion::PrivilegeMode m2) -> bool {
    Legion::PrivilegeMode slot_privs =
        slot.get_privileges(signature.get_task_type());
...
static std::unordered_map<slot_id, TensorArgumentFormat>
    allocate_region_idxs(OpTaskSignature const &signature,
                         OpTaskBinding const &binding) {
  ...
  for (auto const &kv : collected) {
    ...
    if (holds_alternative<OpTensorSpec>(kv.second)) {
      ...
      result[slot] = std::pair<int, OpTensorSpec>{region_idx, tensor_spec};
      region_idx++;
    } else {
      ...
      for (OpTensorSpec const &tensor_spec : tensor_specs) {
        region_idx_mappings.push_back({region_idx, tensor_spec});
        region_idx++;
      }
...
static std::unordered_map<slot_id, ArgSpec>
    allocate_argument_offsets(OpTaskSignature const &signature,
                              OpTaskBinding const &binding) {
  static_assert(
      is_trivially_serializable<OpTaskArgumentFormat>,
      "OpTaskArgumentFormat must be trivially serializable for this to work");
...
TaskArgumentFormat compile_task_invocation(OpTaskSignature const &signature,
...
OpTaskArgumentAccessor::OpTaskArgumentAccessor(
    Legion::Task const *task,
    std::vector<Legion::PhysicalRegion> const &regions,
    Legion::Context ctx,
    Legion::Runtime *runtime)
    : task(task), regions(regions), ctx(ctx), runtime(runtime),
      args_fmt(*(OpTaskArgumentFormat const *)task->args){}

      /* OpTensorSpec::OpTensorSpec(TensorRole tensor_role, int idx, IsGrad
         is_grad, optional<Legion::PrivilegeMode> mode) */
      ...
      /* static bool spec_satisfies_slot_role(TensorRole slot_role, OpTensorSpec
         const &tensor_spec) { */
      /*   if (slot_role == TensorRole::INPUT || slot_role == TensorRole::PARAM)
         { */
      /*     return (tensor_spec.mode == READ_ONLY || tensor_spec.mode ==
         READ_WRITE); */
      /*   } else if (slot_role == TensorRole::OUTPUT) { */
      /*     return (tensor_spec.mode == WRITE_ONLY || tensor_spec.mode ==
         READ_WRITE); */
      /*   } */
      /* } */
      ...
      /* void OpTaskSpec::bind(std::vector<std::pair<int, OpTensorSpec>> const
         &bindings) { */
      ...
      /* TaskAccessor(Task const *task, std::vector<Legion::PhysicalRegion>
         const &regions, Legion::Context const &ctx, Legion::Runtime *runtime,
         TaskSpec const &spec) */
      /*   : task(task), regions(regions), ctx(ctx), runtime(runtime),
         spec(spec) */
      /* { } */
      ...
      /*   : TaskAccessor(task, regions, ctx, runtime, ((Op const
         *)task->args)->get_tasks_spec(task_type)) */
      /* { } */

      /* void execute_task(LegionConfig const &config, TaskID task_id,
      ...
      /*   OpTaskBinding binding =
         this->get_task_binding(signature.get_task_type()); */
