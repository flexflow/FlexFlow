...
void FFConfig::parse_args(char **argv, int argc) {
  ...
  auto dataset_pat_ref = args.add_argument("--dataset-path", "", "Path to the dataset");
  auto search_budget_ref = args.add_argument("--search-budget", 0, "Search budget");
  auto search_alpha_ref = args.add_argument("--search-alpha", 0.0f, "Search alpha");
  auto simulator_workspace_size_ref = args.add_argument(
      "--simulator-workspace-size", 0, "Simulator workspace size");
  auto only_data_parallel_ref = args.add_argument("--only-data-parallel", false, "Only use data parallelism");
  ...
  auto profiling_ref = args.add_argument("--profiling", false, "Enable profiling");
  auto allow_tensor_op_math_conversion_ref = args.add_argument("--allow-tensor-op-math-conversion",
                    false,
  ...
  auto machine_model_version_ref = args.add_argument("--machine-model-version", 0, "Machine model version");
  auto machine_model_file_ref = args.add_argument("--machine-model-file", "", "Machine model file");
  auto simulator_segment_size_ref = args.add_argument("--simulator-segment-size", 0, "Simulator segment size");
  ...
  auto search_num_nodes_ref = args.add_argument("--search-num-nodes", 0, "Search number of nodes");
  auto search_num_workers_ref= args.add_argument("--search-num-workers", 0, "Search number of workers");
  auto base_optimize_threshold_ref = args.add_argument("--base-optimize-threshold", 0, "Base optimize threshold");
  auto enable_control_replication_ref = args.add_argument(
      "--enable-control-replication", false, "Enable control replication");
  auto python_data_loader_type_ref = args.add_argument("--python-data-loader-type", 0, "Python data loader type");
  auto substitution_json_ref = args.add_argument("--substitution-json", "", "Substitution json path");
  ...
  auto ll_util_ref = args.add_argument("-ll:util", 1, "utility processors to create per process");
  auto ll_csize_ref = args.add_argument(
      "-ll:csize", 1024, "size of CPU DRAM memory per process(in MB)");
  auto ll_gsize_ref = args.add_argument("-ll:gsize", 0, "size of GPU DRAM memory per process");
  ...
  auto lg_sched_ref = args.add_argument("-lg:sched",
                    1024,
  ...
  batch_size = args.get(batch_size_ref)
  epochs = args.get(epochs_ref);
