...
std::vector<ParallelTensorUseDescription> ParallelTensorUses::at(ParallelTensor const &p_tensor) const {
  return this->at(p_tensor->parallel_tensor_guid);
}

std::vector<ParallelTensorUseDescription> ParallelTensorUses::at(ParallelTensorBase const *ptr) const {
  return this->at(ptr->parallel_tensor_guid);
}

std::vector<ParallelTensorUseDescription> ParallelTensorUses::at(size_t parallel_tensor_guid) const {
...
void ParallelTensorUses::remove(Op const &op) {
  for (auto const &k : keys(this->uses)) {
    inplace_filter(this->uses.at(k), [&](ParallelTensorUseDescription const &d) { return d.op->op_guid == op.op_guid; });
  }
}
...
void ParallelTensorUses::update(Op const &op) {
  ...
  for (int idx = 0; idx < op.outputs.size(); idx++) {
    ...
    this->uses[output->parallel_tensor_guid].push_back({ TensorUseType::OUTPUT, &op, idx });
  }
  for (int idx = 0; idx < op.weights.size(); idx++) {
    ParallelTensor weight = op.weights.at(idx);
    this->uses[weight->parallel_tensor_guid].push_back({ TensorUseType::WEIGHT, &op, idx });
  }
  for (int idx = 0; idx < op.inputs.size(); idx++) {
    ParallelTensor input = op.inputs.at(idx);
    this->uses[input->parallel_tensor_guid].push_back({ TensorUseType::INPUT, &op, idx });
  }
}
