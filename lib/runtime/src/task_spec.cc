...
Legion::PrivilegeMode get_privileges(TaskArgumentFormat const &fmt, region_idx_t region_idx) {
  return fmt.regions.at(region_idx);
}

Legion::PrivilegeMode get_privileges(TaskArgumentFormat const &fmt, ParallelTensorSpec const &t) {
  return get_privileges(fmt, get_region_idx(fmt, t));
}

DataType get_datatype(TaskArgumentFormat const &fmt, ParallelTensorSpec const &t) {
  return fmt.data_types.at(t);
}

region_idx_t get_region_idx(TaskArgumentFormat const &fmt, ParallelTensorSpec const &t) {
...
TaskArgumentFormat compile_task_invocation(TaskSignature const &signature, TaskBinding const &binding) {
...
void execute_task(LegionConfig const &config, TaskID task_id, TaskSignature const &signature, TaskBinding const &binding) {
...
TaskSignature get_signature(TaskID task_id) {
  switch (task_id) {
    ...
    default: throw mk_runtime_error("Unknown task id {}. Please report this as an issue.", task_id);
  }
}
