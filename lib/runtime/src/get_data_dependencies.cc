...
static void add_pointwise_data_dependency(DataDependencies &deps, slot_id input, slot_id output) {
  deps.add_data_dependency(input, output, [](std::vector<int> const &coords) { return coords; });
}

DataDependencies pointwise_data_dependence(std::vector<slot_id> const &input_slots,
