#ifndef _FLEXFLOW_RUNTIME_SRC_TASK_RETURN_ACCESSOR_H
#define _FLEXFLOW_RUNTIME_SRC_TASK_RETURN_ACCESSOR_H
...
struct TaskReturnAccessor {
  ...
  template <typename T> TypedFuture<T> get_returned_future() const;

  template <typename T> TypedFutureMap<T> get_returned_future_map() const;
