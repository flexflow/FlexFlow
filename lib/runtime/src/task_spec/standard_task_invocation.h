...
using StandardTypedTaskArg =
    variant<T, TypedFuture<T>, RuntimeArgRef<T>, TypedStandardTaskInvocation<T>>;

std::type_index get_type_index(StandardArgSpec);

template <typename T>
TaskInvocationSpec create_task_invocation_spec(TypedStandardTaskInvocation<T> const &);
...
FF_VISITABLE_STRUCT_NONSTANDARD_CONSTRUCTION(StandardTaskBinding, arg_bindings, bindings);
