...
TypedTaskInvocation<T> ensure_return_type(TaskInvocation const &invocation) { 
  return internal_ensure_return_type<TaskInvocation, TypedTaskInvocation<T>, T>(invocation);
}

template <typename T>
TypedIndexTaskInvocation<T> ensure_return_type(IndexTaskInvocation const &invocation) {
  return internal_ensure_return_type<IndexTaskInvocation, TypedIndexTaskInvocation<T>, T>(invocation);
}
