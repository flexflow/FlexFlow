...
void * LocalTaskArgumentAccessor::allocate(size_t size) {
        void * ptr = local_allocator.allocate(memory_size);//Note: how(when) to free this memory? 
        void * cpu_ptr = malloc(memory_size);
        memset(cpu_ptr, 0, memory_size);
        checkCUDA(cudaMemcpy(ptr, cpu_ptr, memory_size, cudaMemcpyHostToDevice)); //fill ptr
        free(cpu_ptr);
        return ptr;
}
...
template <Permissions PRIV> privilege_mode_to_accessor<PRIV> LocalTaskArgumentAccessor::get_tensor(slot_id slot) const {
    ...
    if(slot == GATE_PREDS || slot == GATE_ASSIGN ) {
        ...
        ArrayShape array_shape = {gate_preds.shape.dims.get_dims()};//gate_preds.shape.dims.get_dims() return std::vector<size_t>
        ...
        ArrayShape array_shape = {output_shape.dims.get_dims()};//output_shape.dims.get_dims() return std::vector<size_t>
        size_t shape_size = output_shape.dims.get_volume() * size_of(data_type);
        void * ptr = allocate(shape_size);
        return {shape, array_shape, ptr};
    } else {
        throw mk_runtime_error("Unknown Slot ID in LocalTaskArgumentAccessor::get_tensor");
    }
