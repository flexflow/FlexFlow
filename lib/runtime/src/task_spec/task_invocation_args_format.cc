...
std::vector<ExecutableArgSpec> process_task_invocation_args(TensorlessTaskBinding const &binding, 
  ...
  for (auto const &kv : get_args_of_type<TaskInvocationSpec>(binding)) {
    ...
    ExecutableTaskInvocation executable = resolve(spec.get_invocation(), enable_profiling, runtime_backing);
