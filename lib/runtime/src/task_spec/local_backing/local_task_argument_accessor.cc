...
LocalTaskArgumentAccessor::LocalTaskArgumentAccessor(Allocator allocator) : allocator(allocator) {};
...
privilege_mode_to_accessor<PRIV> LocalTaskArgumentAccessor::get_tensor(slot_id slot, bool is_grad = false) const {
  std::pair<slot_id, bool> slot_grad_pair = std::make_pair<slot_id, bool>(slot, is_grad);
  GenericTensorAccessorW tensor_backing = this->tensor_backing_map[slot_grad_pair];
  if (PRIV == Permissions::RO) {
    GenericTensorAccessorR readonly_tensor_backing = {tensor_backing.data_type, tensor_backing.get_shape(), tensor_backing.ptr};
