...
bool includes_tensor(ExecutableTensorSpec const &spec, parallel_tensor_guid_t const &guid) {
  if (is_variadic(spec)) {
...
std::unordered_set<slot_id> get_tensor_slots(ExecutableTaskBinding const &binding, parallel_tensor_guid_t const &guid) {
...
Permissions get_tensor_permissions(TaskSignature const &sig, ExecutableTaskBinding const &binding, parallel_tensor_guid_t const &guid) {
