...
GenericTaskLauncher compile_to_launcher(TensorlessTaskInvocation const &invocation,
  ...
  if (binding.invocation_type == InvocationType::STANDARD) {
    ...
    IndexArgsFormat index_args_format = process_index_args(binding, backing.get_domain(mv_backing.parallel_is));
    Legion::TaskArgument task_arg = as_task_argument(concrete_args_format,
                                                     future_args_format,
                                                     nullopt,
                                                     index_args_format);
    ...
    throw mk_runtime_error("Unknown invocation type {}", binding.invocation_type);
  }
}
