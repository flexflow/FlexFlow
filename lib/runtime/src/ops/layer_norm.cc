...
Tensor FFModel::layer_norm(const Tensor input, std::vector<int> const &axes,
  ...
  Layer *ln = new Layer(this, OP_LAYERNORM, DT_FLOAT, name, 1 /*inputs*/,
                        num_weights, 1 /*outputs*/, input);
  ln->outputs[0] = create_tensor_legion_ordering(input->num_dims, input->dims,
                                                 input->data_type, ln, 0,
                                                 true /*create_grad*/);
  if (num_weights == 2) {
    int M = 1;
    for (int i = 0; i < axes.size(); i++) {
...
Op *LayerNorm::create_operator_from_layer(
  ...
  return new LayerNorm(model, layer->layer_guid, inputs[0], axes,
                       elementwise_affine, eps,
                       false, // allocate_weights
                       layer->name);
}

LayerNorm::LayerNorm(FFModel &model, LayerNormParams const &params,
                     ParallelTensor const input, char const *name,
                     bool allocate_weights)
    : LayerNorm(model, params.layer_guid, input, params.axes,
                params.elementwise_affine, params.eps, allocate_weights, name) {
}

LayerNorm::LayerNorm(FFModel &model, LayerID const &_layer_guid,
                     const ParallelTensor _input, std::vector<int> const &_axes,
                     bool _elementwise_affine, float _eps,
                     bool allocate_weights, char const *name)
    : Op(model, OP_LAYERNORM, _input->data_type, name, 1 /*inputs*/,
         _elementwise_affine ? 2 : 0 /*weights*/, 1 /*outputs*/, _input),
...
void LayerNorm::forward(FFModel const &ff) {
  ...
  IndexLauncher launcher(LAYERNORM_FWD_TASK_ID, parallel_is,
  ...
  if (elementwise_affine) {
    launcher.add_region_requirement(
        RegionRequirement(weights[0]->part, 0 /*projection id*/, READ_WRITE,
                          EXCLUSIVE, weights[0]->region));
    launcher.add_field(2, FID_DATA);
    launcher.add_region_requirement(
        RegionRequirement(weights[1]->part, 0 /*projection id*/, READ_WRITE,
...
void LayerNorm::forward_task(Task const *task,
  ...
  in_ptr = helperGetTensorPointerRO<float>(regions[0], task->regions[0],
                                           FID_DATA, ctx, runtime);
  Domain out_domain = runtime->get_index_space_domain(
      ctx, task->regions[1].region.get_index_space());
  out_ptr = helperGetTensorPointerWO<float>(regions[1], task->regions[1],
                                            FID_DATA, ctx, runtime);
  ...
  if (m->elementwise_affine) {
    ...
    gamma_ptr = helperGetTensorPointerRW<float>(regions[2], task->regions[2],
                                                FID_DATA, ctx, runtime);
    Domain beta_domain = runtime->get_index_space_domain(
        ctx, task->regions[3].region.get_index_space());
    beta_ptr = helperGetTensorPointerRW<float>(regions[3], task->regions[3],
                                               FID_DATA, ctx, runtime);
...
void LayerNorm::backward(FFModel const &ff) {
  ...
  IndexLauncher launcher(LAYERNORM_BWD_TASK_ID, parallel_is,
                         TaskArgument(NULL, 0), argmap, Predicate::TRUE_PRED,
                         false /*must*/, 0 /*mapper_id*/,
                         outputs[0]->machine_view.hash());
...
void LayerNorm::backward_task(Task const *task,
  ...
  Domain out_grad_domain = runtime->get_index_space_domain(
      ctx, task->regions[0].region.get_index_space());
  out_grad_ptr = helperGetTensorPointerRO<float>(regions[0], task->regions[0],
                                                 FID_DATA, ctx, runtime);
  Domain in_domain = runtime->get_index_space_domain(
      ctx, task->regions[1].region.get_index_space());
  in_ptr = helperGetTensorPointerRO<float>(regions[1], task->regions[1],
                                           FID_DATA, ctx, runtime);
  Domain in_grad_domain = runtime->get_index_space_domain(
      ctx, task->regions[2].region.get_index_space());
  in_grad_ptr = helperGetTensorPointerRW<float>(regions[2], task->regions[2],
                                                FID_DATA, ctx, runtime);
  ...
  if (m->elementwise_affine) {
    ...
    Domain gamma_domain = runtime->get_index_space_domain(
        ctx, task->regions[3].region.get_index_space());
    gamma_ptr = helperGetTensorPointerRO<float>(regions[3], task->regions[3],
                                                FID_DATA, ctx, runtime);
  ...
  backward_kernel_wrapper<float>(m, out_grad_ptr, in_ptr, in_grad_ptr,
                                 gamma_ptr, gamma_grad_ptr, beta_grad_ptr);
}

bool LayerNorm::measure_operator_cost(Simulator *sim, MachineView const &mv,
...
using PCG::Node;
/*static*/
Node LayerNorm::deserialize(FFModel &ff, Legion::Deserializer &dez,
...
Op *LayerNorm::materialize(FFModel &ff, ParallelTensor inputs[],
                           int num_inputs) const {
  LayerNormParams params = get_params();
  return new LayerNorm(ff, params, inputs[0], this->name,
