...
void add_tensor_requirements(Legion::TaskLauncher const &, TensorArgsFormat const &);
void add_tensor_requirements(Legion::IndexTaskLauncher const &, TensorArgsFormat const &);

bool includes_tensor(ExecutableTensorSpec const &, parallel_tensor_guid_t const &);
std::unordered_set<slot_id> get_tensor_slots(ExecutableTaskBinding const &, parallel_tensor_guid_t const &);
Permissions get_tensor_permissions(TaskSignature const &, ExecutableTaskBinding const &, parallel_tensor_guid_t const &);
std::vector<parallel_tensor_guid_t> as_vector(ExecutableTensorSpec const &);

TensorArgsFormat process_tensor_args(TaskSignature const &, ParallelComputationGraph const &, ExecutableTaskBinding const &);
