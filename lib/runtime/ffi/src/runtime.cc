...
static ModelTrainingInstance *unwrap_opaque(flexflow_model_training_instance_t opaque) {
  return opaque.ptr;
}
...
char *flexflow_runtime_get_error_string(flexflow_runtime_error_t e) {
  ...
  switch (e) {
    ...
    case FLEXFLOW_RUNTIME_ERROR_UNEXPECTED_EMPTY_HANDLE:
      ...
      break;
    default:
      msg = fmt("Unhandled case for value {}. Please report to the FlexFlow developers.", e);
  }
  return const_cast<char *>(msg);
}

flexflow_runtime_error_t flexflow_model_training_instance_forward(flexflow_model_training_instance_t opaque,
                                                                  flexflow_void_future_t *out) {
  ModelTrainingInstance *training_instance = unwrap_opaque(opaque);
  CHECK_NONNULL(training_instance);

  return R::output_stored(
    forward(*training_instance);
    out
  );
}

flexflow_runtime_error_t flexflow_void_future_destroy(flexflow_void_future_t opaque) {
  return R::deallocate_opaque(opaque);
}

flexflow_runtime_error_t flexflow_wait_on_void_future(flexflow_void_future_t opaque) {
