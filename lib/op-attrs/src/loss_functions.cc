...
LossFunction get_loss_type(OtherLossAttrs const &attrs) { return attrs.loss_type; }
LossFunction get_loss_type(SparseCategoricalCrossEntropyLossAttrs const &attrs) { return LossFunction::SPARSE_CATEGORICAL_CROSSENTROPY; }
...
LossFunction parse_loss_name(std::string const &raw_name) {
  std::string name = transform(raw_name, [](unsigned char c) { return std::tolower(c); });
  ...
  if (name == "categorical_crossentropy") {
    ...
    throw mk_runtime_error("Unknown loss type {}. Please report this as an issue.", name);
  }
}
