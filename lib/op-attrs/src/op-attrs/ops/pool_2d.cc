...
tl::expected<Pool2DAttrs, std::string> make_adaptive_pool2d_attrs(TensorDims const &input_dims,
  ...
  if (input_h % output_h != 0) {
    return tl::unexpected(fmt::format("Currently make_adaptive_pool2d_attrs only supports input_h % output_h == 0, but received input_h={} and output_h={} (input_dims={}). If you need input_h % output_h != 0 supported, please create an issue.", input_h, output_h, input_dims));
  }

  if (input_w % output_w != 0) {
    return tl::unexpected(fmt::format("Currently make_adaptive_pool2d_attrs only supports input_w % output_w == 0, but received input_w={} and output_w={} (input_dims={}). If you need input_w % output_w != 0 supported, please create an issue.", input_w, output_w, input_dims));
  }
  ...
  TensorShape output_shape = ({
    tl::expected<TensorShape, std::string> result = get_output_shape(attrs, TensorShape{input_dims, DataType::FLOAT});
    if (!result.has_value()) {
