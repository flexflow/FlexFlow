...
int get_order_of_replica_type(ReplicaParallelDimSet const &s, ReplicaType replica_type) {
  switch (replica_type) {
    ...
    default:
      throw mk_runtime_error(fmt::format("Unexpected ReplicaType value: {}", static_cast<int>(replica_type)));
  }
}

std::unordered_set<ReplicaParallelDim> get_replica_dims(ReplicaParallelDimSet const &s) {
