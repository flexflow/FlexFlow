...
std::vector<ParallelTensorShape> get_output_shapes(PCGOperatorAttrs const &pcg_op_attrs,
  ...
  return pcg_op_attrs.visit<std::vector<ParallelTensorShape>>(overload {
    ...
    [&](ElementBinaryAttrs const &attrs) -> std::vector<ParallelTensorShape> {
      return {throw_if_unexpected(get_output_shape(attrs, inputs.at(0), inputs.at(1)))};
    },
