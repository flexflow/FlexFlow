...
public:
  Pool2DAttrs() = delete;
  Pool2DAttrs(int kernel_h, int kernel_w, int stride_h, int stride_w, int padding_h, int padding_w, PoolOp pool_type, Activation activation);
public:
...
struct formatter<::FlexFlow::PoolOp> : formatter<string_view> {
  ...
  auto format(::FlexFlow::PoolOp o, FormatContext& ctx) const -> decltype(ctx.out()) {
    using namespace FlexFlow;

    string_view name = "unknown";
    switch (o) {
      case PoolOp::AVG: name = "Avg"; break;
      case PoolOp::MAX: name = "Max"; break;
    }
