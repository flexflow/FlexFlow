...
TEST_CASE("ParallelTensorDims Tests") {
    ...
    SUBCASE("Size and Accessors ") {
        ...
        CHECK(parallel_tensor_dims.at(ff_dim_t(1)).size) = dim1[1];
    }
    ...
    SUBCASE("Iterators begin") {
        ...
        FFOrdered<ParallelDim>::reverse_iterator rit= parallel_tensor_dims.rbegin();
        CHECK((*rit).size == 4);
        ++rit;
        CHECK((*rit).size == 3);

        FFOrdered<ParallelDim>::const_reverse_iterator crit = parallel_tensor_dims.crbegin();
