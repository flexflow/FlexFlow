...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("ReLU shape inference") {
    ...
    auto make_i = [&](SumDegree o_sum, DiscardCopyDegree o_eq, int o_1, int o_2, int o_3) {
      return lift_to_parallel_with_degrees(input, o_sum, o_eq, FFOrdered<int>{o_1, o_2, o_3});
    };
    ...
    SUBCASE("sum degree > 1") {
      ...
      tl::expected<ParallelTensorShape, std::string> result = get_output_shape(attrs, make_i(SumDegree{degree}, 1, 1, 1, 1));
    ...
    SUBCASE("discard copy degree > 1") {
      ...
      tl::expected<ParallelTensorShape, std::string> result = get_output_shape(attrs, make_i(1, DiscardCopyDegree{degree}, 1, 1, 1));
