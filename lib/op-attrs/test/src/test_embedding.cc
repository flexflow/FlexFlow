...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("Sum embedding shape inference") {
    ...
    // get_weights_shape
    {
      tl::expected<TensorShape, std::string> weight_result = get_weights_shape(attrs, input);
      tl::expected<TensorShape, std::string> weight_correct = weights;
      CHECK(weight_result == weight_correct);
    }

    auto make_input = [&](SumDegree o_sum, DiscardCopyDegree o_eq, int o_batch, int o_features) {
      return lift_to_parallel_with_degrees(input, o_sum, o_eq, FFOrdered<int>{o_batch, o_features});
    };

    auto make_output = [&](SumDegree o_sum, DiscardCopyDegree o_eq, int o_batch, int o_outchannels) {
      return lift_to_parallel_with_degrees(output, o_sum, o_eq, FFOrdered<int>{o_batch, o_outchannels});
    };

    auto make_weights = [&](SumDegree o_sum, DiscardCopyDegree o_eq, int o_entries, int o_outchannels) {
      return lift_to_parallel_with_degrees(weights, o_sum, o_eq, FFOrdered<int>{o_entries, o_outchannels});
    };
