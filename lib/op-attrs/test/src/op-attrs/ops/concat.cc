...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("get_output_shape(ConcatAttrs, std::vector<TensorShape>)") {
    ...
    SUBCASE("empty input shapes list passed") {
      ...
      std::optional<TensorShape> result = optional_from_expected(get_output_shape(attrs, input_shapes));
    ...
    SUBCASE("input shapes do not shared the same num_dims") {
      ...
      std::vector<TensorShape> input_shapes = {input_shape1, input_shape2, input_shape3, mismatched_num_dims};
    ...
    SUBCASE("concat axis is out of bounds") {
      ...
      std::vector<TensorShape> input_shapes = {input_shape1, input_shape2, input_shape3};
    ...
    SUBCASE("input shapes are valid") {
      ...
      tl::expected<TensorShape, std::string> result = get_output_shape(attrs, input_shapes);
  ...
  TEST_CASE("get_output_shape(ConcatAttrs, std::vector<ParallelTensorShape>)") {
    ...
    auto lift_input1 = 
      [&](SumDegree o_sum, DiscardCopyDegree o_eq, int o0, int o1, int o2) {
        return lift_to_parallel_with_degrees(input_shape1, o_sum, o_eq, FFOrdered<int>{o0, o1, o2});
      };

    auto lift_input2 = 
      [&](SumDegree o_sum, DiscardCopyDegree o_eq, int o0, int o1, int o2) {
        return lift_to_parallel_with_degrees(input_shape2, o_sum, o_eq, FFOrdered<int>{o0, o1, o2});
      };

    auto lift_input3 = 
      [&](SumDegree o_sum, DiscardCopyDegree o_eq, int o0, int o1, int o2) {
        return lift_to_parallel_with_degrees(input_shape3, o_sum, o_eq, FFOrdered<int>{o0, o1, o2});
      };

    auto lift_output = 
      [&](SumDegree o_sum, DiscardCopyDegree o_eq, int o0, int o1, int o2) {
        return lift_to_parallel_with_degrees(output_shape, o_sum, o_eq, FFOrdered<int>{o0, o1, o2});
      };
    ...
    SUBCASE("sum reduction parallelism") {
      SUBCASE("matching") {
        ...
        tl::expected<ParallelTensorShape, std::string> result = get_output_shape(attrs, inputs);
        tl::expected<ParallelTensorShape, std::string> correct = lift_output(sum_degree, DiscardCopyDegree{1}, 1, 1, 1);
      ...
      SUBCASE("not matching") {
        ...
        std::optional<ParallelTensorShape> result = optional_from_expected(get_output_shape(attrs, inputs));
    ...
    SUBCASE("discard copy reduction parallelism") {
      SUBCASE("matching") {
        ...
        tl::expected<ParallelTensorShape, std::string> result = get_output_shape(attrs, inputs);
        tl::expected<ParallelTensorShape, std::string> correct = lift_output(SumDegree{1}, discard_copy_degree, 1, 1, 1);
    ...
    SUBCASE("parallelism in non-axis shard dims") {
      SUBCASE("matching") {
        ...
        std::vector<ParallelTensorShape> inputs = {
          lift_input1(SumDegree{1}, DiscardCopyDegree{1}, degree0, 1, degree2),
          lift_input2(SumDegree{1}, DiscardCopyDegree{1}, degree0, 1, degree2),
          lift_input3(SumDegree{1}, DiscardCopyDegree{1}, degree0, 1, degree2),
        };

        tl::expected<ParallelTensorShape, std::string> result = get_output_shape(attrs, inputs);
        tl::expected<ParallelTensorShape, std::string> correct = lift_output(SumDegree{1}, DiscardCopyDegree{1}, degree0, 1, degree2);
    ...
    SUBCASE("parallelism degrees are not mutually exclusive") {
      ...
      tl::expected<ParallelTensorShape, std::string> result = get_output_shape(attrs, inputs);
      tl::expected<ParallelTensorShape, std::string> correct = lift_output(sum_degree, discard_copy_degree, degree0, 1, degree2);
