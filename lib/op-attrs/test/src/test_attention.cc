...
TEST_SUITE(FF_TEST_SUITE) {
  ...
  TEST_CASE("get_weights_shape(MultiHeadAttentionAttrs, TensorShape, TensorShape, TensorShape)") {
    int embed_dim = 32;
  ...
  TEST_CASE("parallel shape inference for MultiHeadAttentionAttrs") {
    ...
    TensorShape unpar_o_shape = get_output_shape(attrs, unpar_q_shape, unpar_k_shape, unpar_v_shape);
