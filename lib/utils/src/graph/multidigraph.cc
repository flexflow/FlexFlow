...
to delete the ptr 2 we use this ptr to create a IMultiDiGraphView, this
IMultiDiGraphView is read-only. It creates a MultiDiGraphView object that is not
responsible for ownership management. Set the shared_ptr's destructor to a nop
so that effectively there is no ownership
*/
MultiDiGraphView
