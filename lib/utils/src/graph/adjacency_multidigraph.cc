...
void AdjacencyMultiDiGraph::add_edge(MultiDiEdge const &e) {
  ...
  */
  std::cout<<"e.dst:"<<e.dst<<",e.dst_idx:"<<e.dst_idx <<", e.src:"<<e.src<<",e.src_idx"<<e.src_idx<<std::endl;
...
for (auto const &entry1 : this->adjacency) {
    ...
    const auto &innerMap1 = entry1.second;
    if(innerMap1.size() > 0)
    std::cout << "Node1: " << node1 << ", InnerMap1 Size: " << innerMap1.size() << std::endl;

    for (auto const &entry2 : innerMap1) {
        Node node2 = entry2.first;
        const auto &innerMap2 = entry2.second;
        if(innerMap2.size() >0) 
        std::cout << "  Node2: " << node2 << ", InnerMap2 Size: " << innerMap2.size() << std::endl;

        for (auto const &entry3 : innerMap2) {
            NodePort nodePort = entry3.first;
            const auto &nodePortSet = entry3.second;
            if(nodePortSet.size() > 0)
            std::cout << "    NodePort: " << nodePort << ", NodePortSet Size: " << nodePortSet.size() << std::endl;
...
//this has some bug, for example, for q, we only has the q.dsts, but don't have q.srcs
//how to handle the case when q doesn't hold src/dst/srcidx/dstidx(q.srcs is null), 
//TODO:fix the corner case(q doesn't hold src/dst/srcidx/dstidx(q.srcs is null))
//q.src is null, we return this->adjacency
std::unordered_set<MultiDiEdge>
    AdjacencyMultiDiGraph::query_edges(MultiDiEdgeQuery const &q) const {
  std::unordered_set<MultiDiEdge> result;
  std::cout<<"1 query_keys(q.srcs, this->adjacency).size():"<<query_keys(q.srcs, this->adjacency).size()<<" and this->adjacency.size():"<<this->adjacency.size()<<std::endl;
  for (auto const &src_kv : query_keys(q.srcs, this->adjacency)) {
      std::cout<<"1.5 query_keys(q.dsts, this->adjacency).size():"<<query_keys(q.dsts, src_kv.second).size()<<" and src_kv.second.size():"<<src_kv.second.size()<<", src_kv.first:"<<src_kv.first.value()<<std::endl;
    for (auto const &dst_kv : query_keys(q.dsts, src_kv.second)) {
  std::cout<<"2 query_keys(q.dsts, this->adjacency).size():"<<query_keys(q.dsts, src_kv.second).size()<<" and src_kv.second.size():"<<src_kv.second.size()<<std::endl;
      for (auto const &srcIdx_kv : query_keys(q.srcIdxs, dst_kv.second)) {
      std::cout<<"3 query_keys(q.srcIdxs, dst_kv.second).size():"<<query_keys(q.srcIdxs,dst_kv.second).size()<<" and dst_kv.second.size():"<<dst_kv.second.size()<<std::endl;
