...
std::unordered_map<Node, int>
    get_unchecked_longest_path_lengths_from_root(DiGraphView const &g) {
...
std::unordered_map<Node, float>
    get_unchecked_weighted_longest_path_lengths_from_root(
        DiGraphView const &g,
...
std::unordered_map<Node, float> get_weighted_longest_path_lengths_from_root(
  ...
  return get_unchecked_weighted_longest_path_lengths_from_root(g, node_costs);
}
...
std::unordered_map<Node, int>
  ...
  assert(is_acyclic(g));
  assert(get_sources(g).size() == 1);
  return get_unchecked_longest_path_lengths_from_root(g);
}
