...
std::optional<Node>
  ...
  std::unordered_map<Node, Node> contraction = generate_map(nodes, [&](Node const &) { return contracted_node; });
  return get_imm_post_dominator(apply_contraction(g, contraction), contracted_node);
}
