...
std::unordered_set<DirectedEdge> get_edges_from_subgraph_to_subgraph(DiGraphView const &g,
  ...
  if (!are_disjoint(src_subgraph, dst_subgraph)) {
    throw mk_runtime_error(fmt::format("get_edges_from_subgraph_to_subgraph(DiGraphView, ...) expected src_subgraph and dst_subgraph to be disjoint, but found src_subgraph={}, dst_subgraph={}", src_subgraph, dst_subgraph));
  }
