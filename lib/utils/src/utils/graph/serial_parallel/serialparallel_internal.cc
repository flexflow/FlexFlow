...
std::optional<std::variant<IntermediateSpDecompositionTree, Node>>
  ...
  MultiDiGraph ttsp = MultiDiGraph::materialize_copy_of<AdjacencyMultiDiGraph>(inverse_line_graph_result.graph);
  std::unordered_map<MultiDiEdge, std::variant<IntermediateSpDecompositionTree, Node>> ttsp_edge_to_sp_tree = map_values(
    inverse_line_graph_result.inverse_edge_to_line_node_bidict.as_unordered_map(),
    [](Node const &n) { return std::variant<IntermediateSpDecompositionTree, Node>{n}; });

  while (true) {
    assert (ttsp_edge_to_sp_tree.size() == get_edges(ttsp).size());
    std::optional<ParallelReduction> maybe_parallel_reduction = find_parallel_reduction(ttsp);
    if (maybe_parallel_reduction.has_value()) {
      ...
      std::variant<IntermediateSpDecompositionTree, Node> new_tree = IntermediateSpDecompositionTree{
        SplitType::PARALLEL,
    ...
    std::optional<SeriesReduction> maybe_series_reduction = find_series_reduction(ttsp);
