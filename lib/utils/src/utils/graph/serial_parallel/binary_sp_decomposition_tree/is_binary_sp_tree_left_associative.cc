...
bool is_binary_sp_tree_left_associative(BinarySPDecompositionTree const &t) {
  return t.visit<bool>(overload {
    [](Node const &n) { return true; },
    [](BinarySeriesSplit const &s) { 
      return !s.right_child().has<BinarySeriesSplit>()
    ...
    [](BinaryParallelSplit const &p) {
      return !p.right_child().has<BinaryParallelSplit>()
