...
void parallel_extend_unsafe(DataflowGraph &g,
  ...
  for (Node const &node : get_nodes(ext)) {
    g.add_node_unsafe(node, 
                      get_inputs(ext, node),
                      get_outputs(ext, node));
  }
}

void serial_extend_unsafe(DataflowGraph &g,
                          DataflowGraphView const &ext) {
  // TODO(@lockshaw): This function signature is impossible to implement in general,
  // as there is no guarantee that the graph view ext actually has source nodes with inputs
  // Either the signature should be changed, or an implementation should be added that throws
  // an error if this problematic case is found
...
DataflowGraph serial_composition(DataflowGraphView const &g1,
  ...
  DataflowGraph g = DataflowGraph::create_copy_of<UnorderedSetDataflowGraph>(g1);
