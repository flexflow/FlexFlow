...
std::unordered_set<DataflowGraphIsomorphism>
  ...
  std::unordered_set<OpenDataflowGraphIsomorphism> open_isomorphisms = 
    find_isomorphisms(view_as_open_dataflow_graph(src), view_as_open_dataflow_graph(dst));

  return transform(open_isomorphisms, [](OpenDataflowGraphIsomorphism const &open) {
