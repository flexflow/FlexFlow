...
AdjacencyMultiDiGraph::AdjacencyMultiDiGraph(NodeSource const &node_source,
                        MultiDiEdgeSource const &edge_source,
                        std::unordered_map<Node, std::unordered_map<Node, std::unordered_set<MultiDiEdge>>> const &adjacency,
...
Node AdjacencyMultiDiGraph::add_node() {
  ...
  std::unordered_set<Node> all_nodes = set_union(keys(this->adjacency), {new_node});
  this->adjacency[new_node] = generate_map(all_nodes, 
                                              [](Node const &) { return std::unordered_set<MultiDiEdge>{}; });
...
void AdjacencyMultiDiGraph::remove_node(Node const &n) {
  ...
  std::unordered_set<MultiDiEdge> outgoing = set_union(values(this->adjacency.at(n)));
...
std::unordered_set<Node> AdjacencyMultiDiGraph::query_nodes(NodeQuery const &q) const {
...
AdjacencyMultiDiGraph *AdjacencyMultiDiGraph::clone() const {
  return new AdjacencyMultiDiGraph(this->node_source,
                                   this->edge_source,
                                   this->adjacency,
                                   this->edge_nodes);
}
