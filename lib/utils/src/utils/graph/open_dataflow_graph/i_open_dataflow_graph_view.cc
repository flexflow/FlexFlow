...
std::unordered_set<DataflowEdge> IOpenDataflowGraphView::query_edges(DataflowEdgeQuery const &q) const {
  OpenDataflowEdgeQuery open_query = OpenDataflowEdgeQuery{ 
    dataflow_input_edge_query_none(),
    q,
  };

  std::unordered_set<OpenDataflowEdge> open_edges = this->query_edges(open_query);
