...
bidict<NewDataflowGraphInput, DataflowGraphInput> generate_new_input_id_permutation(OpenDataflowGraphView const &g) {
  DataflowGraphInputSource input_source;
  return generate_bidict(get_inputs(g),
                         [&](DataflowGraphInput const &) { return NewDataflowGraphInput{input_source.new_dataflow_graph_input()}; }).reversed();
}
