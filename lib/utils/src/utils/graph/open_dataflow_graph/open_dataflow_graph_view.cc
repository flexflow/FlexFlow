...
std::unordered_set<OpenDataflowEdge> OpenDataflowGraphView::query_edges(OpenDataflowEdgeQuery const &q) const {
  return this->get_interface().query_edges(q);
}

IOpenDataflowGraphView const &OpenDataflowGraphView::get_interface() const {
  return *std::dynamic_pointer_cast<IOpenDataflowGraphView const>(GraphView::ptr.get());
}
