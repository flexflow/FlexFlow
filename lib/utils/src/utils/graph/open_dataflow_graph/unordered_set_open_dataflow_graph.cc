...
NodeAddedResult UnorderedSetOpenDataflowGraph::add_node(std::vector<OpenDataflowValue> const &inputs,
                         int num_outputs) {
  NOT_IMPLEMENTED();
}

std::unordered_set<Node> UnorderedSetOpenDataflowGraph::query_nodes(NodeQuery const &q) const {
  return apply_query(q.nodes, this->nodes);
}

std::unordered_set<OpenDataflowEdge> UnorderedSetOpenDataflowGraph::query_edges(OpenDataflowEdgeQuery const &q) const {
  std::unordered_set<DataflowEdge> standard_edges = filter(this->standard_edges, [&](DataflowEdge const &e) {
  ...
  std::unordered_set<DataflowInputEdge> input_edges = filter(this->input_edges, [&](DataflowInputEdge const &e) {
  ...
  return set_union(
    transform(standard_edges, [](DataflowEdge const &e) { return OpenDataflowEdge{e}; }),
    transform(input_edges, [](DataflowInputEdge const &e) { return OpenDataflowEdge{e}; })
  );

}

std::unordered_set<DataflowOutput> UnorderedSetOpenDataflowGraph::query_outputs(DataflowOutputQuery const &q) const {
