...
NodeAddedResult OpenDataflowGraph::add_node(std::vector<OpenDataflowValue> const &inputs,
                                            int num_outputs) {
...
IOpenDataflowGraph &OpenDataflowGraph::get_interface() {
  return *std::dynamic_pointer_cast<IOpenDataflowGraph>(GraphView::ptr.get_mutable());
}

IOpenDataflowGraph const &OpenDataflowGraph::get_interface() const {
  return *std::dynamic_pointer_cast<IOpenDataflowGraph const>(GraphView::ptr.get());
}
