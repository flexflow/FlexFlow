...
TEST_CASE("OutputLabelledOpenMultiDiGraph implementation") {
    OutputLabelledOpenMultiDiGraph g = OutputLabelledOpenMultiDiGraph<std::string, std::string>::create<OutputLabelledOpenMultiDiGraph,UnorderedLabelling<Node,std::string>, UnorderedLabelling<InputMultiDiEdge, std::string>, UnorderedLabelling<MultiDiOutput, std::string>>();

    int num_nodes = 3;
    std::vector<std::string> nodel_labels =
      repeat(num_nodes, [&](int i) { return "nodel_labels_" + std::to_string(i); });
    
    std::vector<std::string> input_edge_labels =  repeat(num_nodes, [&](int i) { return "input_edge_labels_" + std::to_string(i); });
    ...
    std::vector<InputMultiDiEdge> input_multi_diedges = {{nodes[1], node_ports[1]},
                                                  {nodes[2], node_ports[2]},
                                                  {nodes[3], nodde_ports[3]}};

    std::vector<OutputMultiDiEdge> output_multi_diedges = {{nodes[0], node_ports[0]},
    ...
    CHECK(g.query_edges(OpenMultiDiEdgeQuery(MultiDiEdgeQuery::all())) == multi_diedges) ; //this may have some problem
