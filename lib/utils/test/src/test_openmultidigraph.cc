...
TEST_CASE_TEMPLATE("OpenMultiDiGraph implementations",
    ...
    rc::dc_check("Full", [&]() {
        ...
        if(num_nodes > 0) {
            e = *gen::unique<std::vector<MultiDiEdge>>(num_edges, gen::construct<MultiDiEdge>(gen::elementOf(n), gen::elementOf(n)));
        }
        ...
        CHECK(g.query_nodes(NodeQuery{query_set<Node>{subset}}) == subset);
         ...
         CHECK(g.query_edges(OpenMultiDiEdgeQuery::all()) == without_order(open_edges));//this may be problem, because OpenMultiDiEdge is a variant 
    });
