...
TEST_CASE("do not pass the optional argument via command") {
  char const *test_argv[] = {
      "program_name", "--batch-size", "100"} ArgsParser args;
  auto batch_size_ref = add_required_argument(
      args, "--batch-size", optional<int>(32), "batch size for training");
  auto ll_gpus_ref = add_optional_argument<int>(
      args,
  ...
  CHECK_THROWS(parse_args(
      args,
      test_argv_length,
      const_cast<char **>(test_argv))); // throw exception because we don't pass
                                        // -ll:gpus via command
}
  ...
  SUBCASE("./a.out --args1 4  --arg2 4 -args3  ") {
    ...
    auto learning_rate_ref =
        add_required_argument(args,
                              ...
                              "Learning rate for the optimizer");
    auto epoch_ref = add_required_argument(args,
                                           "--epoch",
                                           std::optional<int>(1),
...
TEST_CASE("support action_true") {
  ...
  ArgsParser args;
  auto verbose_ref = add_required_argument(args,
                                           "--verbose",
