...
TEST_CASE("AdjacencyMultiDiGraph:basic_test") {
  ...
  CHECK(g.query_edges(MultiDiEdgeQuery::all().with_src_idx(1)) ==
        std::unordered_set<MultiDiEdge>{e3, e4});
  CHECK(g.query_edges(MultiDiEdgeQuery::all().with_dst_idx(1)) ==
  ...
  CHECK(g.query_edges(MultiDiEdgeQuery::all().with_src_idxs({1, 2})) ==
        std::unordered_set<MultiDiEdge>{e3, e4, e5});
  CHECK(g.query_edges(MultiDiEdgeQuery::all().with_dst_idxs({0, 2})) ==
  ...
  CHECK(g.query_edges(MultiDiEdgeQuery::all().with_dst_idx(3)) ==
