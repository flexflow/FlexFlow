...
TEST_CASE("LabelledMultiDiGraph  implementation") {
    LabelledMultiDiGraph  g = LabelledMultiDiGraph <std::string,std::string>::create<AdjacencyMultiDiGraph, UnorderedLabelling<Node,std::string>,UnorderedLabelling<MultiDiEdge, std::string>>();

    int num_nodes = 3;
    std::vector<std::string> nodel_labels = repeat(num_nodes, [&](int i) { return "nodel_labels_" + std::to_string(i); });
    
    std::vector<NodePort> node_ports = repeat(num_nodes, [&] { return g.add_node_port(); });
    std::vector<Node> nodes = repeat(num_nodes, [&](int i) { return g.add_node(nodel_labels[i]); });

    std::vector<NodePort> get_nodeports = repeat(num_nodes, [&](int i ) {return g.at(nodes[i]);});

    CHECK(get_nodeports == node_ports);

    std::vector<std::string> edge_labels =  repeat(num_nodes, [&](int i) { return "edge_labels_" + std::to_string(i); });
    ...
    std::vector<std::string> expected_edge_labels =  repeat(num_nodes, [&](int i ) {return g.at(e[i]);});
