...
struct default_constructible_example {
  ...
  default_constructible_example() : member(1234) {}
...
TEST_CASE_TEMPLATE("required_wrapper_impl is not default constructible",
                   T,
                   int,
                   float,
                   double,
  ...
  STATIC_CHECK_FALSE(
      std::is_default_constructible<required_wrapper_impl<T>>::value);
}
...
TEST_CASE_TEMPLATE("required_wrapper_impl supports expected operations",
                   T,
                   int,
                   float,
                   double,
  ...
  /*   STATIC_CHECK(std::is_same<decltype(std::declval<required_wrapper_impl<T>>()
   * + std::declval<required_wrapper_impl<T>>()),
   * required_wrapper_impl<decltype(t1 + t2)>>::value); */
...
TEST_CASE_TEMPLATE(
    "required_inheritance_impl supports expected operations (generic)",
    T,
    std::vector<int>,
    std::string) {
  rc::dc_check("copy constructible", [&](T t) {
...
TEST_CASE("required_inheritance_impl supports expected operations "
          "(std::vector<int>)") {}
