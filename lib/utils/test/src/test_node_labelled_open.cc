#include "graph/labelled/node_labelled_open.h"
#include "graph/labelled/unordered_label.h"
...
TEST_CASE("NodeLabelledOpenMultiDiGraph implementations") {
  NodeLabelledOpenMultiDiGraph g =
      NodeLabelledOpenMultiDiGraph<std::string>::create<
          AdjacencyOpenMultiDiGraph,
          UnorderedLabelling<std::string, Node>>(); // this may have some
                                                    // problem, we can fix
  int num_nodes = 3;
