...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("get_transitive_reduced_boundary_nodes_for_split") {
    ...
    TransitiveReducedDataflowGraphView tr_g = get_dataflow_graph_transitive_reduction(g);
    ...
    SplitBoundaryNodes result = get_transitive_reduced_boundary_nodes_for_split(tr_g, split);
    SplitBoundaryNodes correct = SplitBoundaryNodes{
      /*pre_split_boundary=*/{n2},
      /*post_split_boundary=*/{n3},
    };
