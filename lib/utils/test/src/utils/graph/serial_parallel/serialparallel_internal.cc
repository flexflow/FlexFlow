...
TEST_SUITE(FF_TEST_SUITE) {
  ...
  TEST_CASE("from_source_to_sink") {
    ...
    SUBCASE("without settings") {
      std::unordered_set<Node> result =
          from_source_to_sink(g, {n.at(0)}, {n.at(3)});
    ...
    auto get_result_for_settings = [&](SourceSettings const &source_settings,
      ...
      return from_source_to_sink(
          g, {n.at(0)}, {n.at(3)}, source_settings, sink_settings);
    };

    SUBCASE("include src and sink") {
      std::unordered_set<Node> result =
          get_result_for_settings(SourceSettings::INCLUDE_SOURCE_NODES,
    ...
    SUBCASE("exclude src, include sink") {
      std::unordered_set<Node> result =
          get_result_for_settings(SourceSettings::EXCLUDE_SOURCE_NODES,
                                  SinkSettings::INCLUDE_SINK_NODES);
  ...
  TEST_CASE("sp_decomposition (base case)") {
    DiGraph g = DiGraph::create<AdjacencyDiGraph>();
    Node n = g.add_node();
    std::optional<std::variant<IntermediateSpDecompositionTree, Node>> result =
        sp_decomposition(g);
    std::optional<std::variant<IntermediateSpDecompositionTree, Node>> correct =
        n;
    CHECK(result == correct);
  }
  ...
  //   correct = IntermediateSpDecompositionTree{
  //     SplitType::PARALLEL,
  //     {ns.at(0), ns.at(1)},
  //   };
  //   CHECK(result == correct);
  ...
  TEST_CASE("sp_decomposition (composite)") {
    ...
    std::optional<std::variant<IntermediateSpDecompositionTree, Node>> correct =
        IntermediateSpDecompositionTree{
            SplitType::SERIAL,
            {ns.at(0),
             IntermediateSpDecompositionTree{SplitType::PARALLEL,
                                             {
                                                 ns.at(1),
                                                 ns.at(2),
                                             }}}};
  ...
  //   IntermediateSpDecompositionTree correct =
  //   IntermediateSpDecompositionTree{
  //     SplitType::PARALLEL,
  //
  // }
