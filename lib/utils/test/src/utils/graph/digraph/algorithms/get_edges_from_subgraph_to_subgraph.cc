  ...
  TEST_CASE("get_edges_from_subgraph_to_subgraph") {
    ...
    SUBCASE("basic tests") {
      ...
      SUBCASE("returns all edges between subgraphs") {
        ...
        std::unordered_set<DirectedEdge> result = get_edges_from_subgraph_to_subgraph(g, src_subgraph, dst_subgraph);
      ...
      SUBCASE("returns no edges if there are no edges from src_subgraph to dst_subgraph") {
        std::vector<DirectedEdge> e = {
          DirectedEdge{n.at(0), n.at(1)},
    ...
    SUBCASE("empty subgraphs") {
      ...
      SUBCASE("returns no edges if no nodes in src_subgraph") {
        std::unordered_set<DirectedEdge> result = get_edges_from_subgraph_to_subgraph(g, {}, unordered_set_of(n));
        std::unordered_set<DirectedEdge> correct = {};
      ...
      SUBCASE("returns no edges if both subgraphs are empty") {
        std::unordered_set<DirectedEdge> result = get_edges_from_subgraph_to_subgraph(g, {}, {});
        std::unordered_set<DirectedEdge> correct = {};
    ...
    SUBCASE("if subgraphs do not cover graph, then does not return external edges") {
      std::vector<DirectedEdge> e = {
        DirectedEdge{n.at(0), n.at(2)},
        DirectedEdge{n.at(0), n.at(3)},
        DirectedEdge{n.at(1), n.at(3)},
    ...
    SUBCASE("throws an error if subgraphs are not disjoint") {
      ...
      CHECK_THROWS(get_edges_from_subgraph_to_subgraph(g, src_subgraph, dst_subgraph));
    }
  }
}
