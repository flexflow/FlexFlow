...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("get_right_child(GenericBinarySPDecompositionTree<T>)") {
    SUBCASE("leaf") {
      GenericBinarySPDecompositionTree<int> input = make_generic_binary_sp_leaf(5);

      CHECK_THROWS(get_right_child(input));
    }
    ...
    SUBCASE("series split") {
      ...
      GenericBinarySPDecompositionTree<int> result = get_right_child(input);
      GenericBinarySPDecompositionTree<int> correct = make_generic_binary_sp_leaf(3); 
    ...
    SUBCASE("parallel split") {
      ...
      GenericBinarySPDecompositionTree<int> result = get_right_child(input);
      GenericBinarySPDecompositionTree<int> correct = make_generic_binary_sp_leaf(7); 
