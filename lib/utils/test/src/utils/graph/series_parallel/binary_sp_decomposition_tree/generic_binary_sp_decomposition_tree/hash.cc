...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("std::hash<GenericBinarySPDecompositionTree<int>>") {
    SUBCASE("leaf") {
      GenericBinarySPDecompositionTree<int> leaf_5 = make_generic_binary_sp_leaf(5);
      size_t leaf_5_hash = get_std_hash(leaf_5);

      SUBCASE("leaves with same labels hash to the same value") {
        GenericBinarySPDecompositionTree<int> also_leaf_5 = make_generic_binary_sp_leaf(5);
      ...
      SUBCASE("leaves with different labels hash to different values") {
        GenericBinarySPDecompositionTree<int> leaf_6 = make_generic_binary_sp_leaf(6);
