...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("try_merge_nondisjoing_unordered_maps(std::unordered_map<K, V>, std::unordered_map<K, V>)") {
    ...
    SUBCASE("compatible neither superset") {
      std::optional<std::unordered_map<int, std::string>> result = try_merge_nondisjoint_unordered_maps(d1, d2);
    ...
    SUBCASE("mismatched key") {
      d1.insert({2, "three"});
      std::optional<std::unordered_map<int, std::string>> result = try_merge_nondisjoint_unordered_maps(d1, d2);
      std::optional<std::unordered_map<int, std::string>> correct = std::nullopt;
      CHECK(result == correct);
    }
