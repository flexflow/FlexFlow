...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE_TEMPLATE("filter(T, F)", T, std::vector<int>,
                                    std::unordered_set<std::string>,
                                    std::set<std::string>,
                                    std::unordered_map<int, int>,
                                    std::map<int, std::string>) {
    RC_SUBCASE("filter returns empty for predicate always_false", [](T const &t) {
      auto always_false = [](auto const &) { return false; };
      T result = filter(t, always_false);
      return result.size() == 0;
    });

    RC_SUBCASE("filter returns input for predicate always_true", [](T const &t) {
