...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE_TEMPLATE("inplace_filter(T, F)", T, std::vector<int>,
                                    std::unordered_set<std::string>,
                                    std::set<std::string>,
                                    std::unordered_map<int, int>,
                                    std::map<int, std::string>) {
    RC_SUBCASE("inplace_filter returns empty for predicate always_false", [](T t) {
      auto always_false = [](auto const &) { return false; };
      inplace_filter(t, always_false);
      return t.size() == 0;
    });

    RC_SUBCASE("inplace_filter returns input for predicate always_true", [](T t) {
      T input = t;
