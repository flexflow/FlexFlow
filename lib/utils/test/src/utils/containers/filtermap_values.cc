...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("filtermap_values(std::unordered_map<K, V>, F)") {
    ...
    std::unordered_map<int, int> result = filtermap_values(input, [](std::string const &v) -> std::optional<int> {
      if (v == "two") {
        return std::nullopt;
      } else {
        return v.size() + 1;
  ...
  TEST_CASE("filtermap_values(std::map<K, V>, F)") {
    ...
    std::map<int, int> result = filtermap_values(input, [](std::string const &v) -> std::optional<int> {
      if (v == "two") {
        return std::nullopt;
      } else {
        return v.size() + 1;
