...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("filtermap_keys(std::unordered_map<K, V>, F)") {
    ...
    std::unordered_map<std::string, std::string> result = filtermap_keys(input, [](int k) -> std::optional<std::string> {
      if (k == 1) {
        return std::nullopt;
      } else {
  ...
  TEST_CASE("filtermap_keys(std::map<K, V>, F)") {
    ...
    std::map<std::string, std::string> result = filtermap_keys(input, [](int k) -> std::optional<std::string> {
      if (k == 1) {
        return std::nullopt;
      } else {
        std::ostringstream oss;
