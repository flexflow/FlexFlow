...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("transform(std::vector<In>, F)") {
    ...
    std::vector<std::string> result = transform(input, [](int x) {
  ...
  TEST_CASE("transform(std::unordered_set<In>, F)") {
    ...
    std::unordered_set<std::string> result = transform(input, [](int x) {
      return std::to_string(x);
    });
  ...
  TEST_CASE("transform(std::string, F)") {
    ...
    std::string result = transform(input, [](char x) -> char {
      return x + 1;
    });
