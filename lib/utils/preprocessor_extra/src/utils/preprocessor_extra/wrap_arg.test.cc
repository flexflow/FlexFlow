...
#define EXAMPLE_MACRO(ARG1, ARG2, ...) \
  BOOST_PP_STRINGIZE(UNWRAP_ARG(ARG1))
...
TEST_CASE("WRAP_ARG") {
  ...
  CHECK(EXAMPLE_MACRO(WRAP_ARG(std::tuple<A, B, C>), H) == "std::tuple<A, B, C>");
  CHECK(OTHER_EXAMPLE_MACRO(std::tuple<A, B, C>, std::tuple<D, E, F>) == "B");
  CHECK(OTHER_EXAMPLE_MACRO(WRAP_ARG(std::tuple<A, B, C>), WRAP_ARG(std::tuple<D, E, F>)) == "std::tuple<D, E, F>");
}
