...
T construct_visitable_from_tuple_impl(seq<S...>, Tup const &tup) {
  return T{std::get<S>(tup)...};
}

template <typename T, typename... Args>
T construct_visitable_from_tuple(std::tuple<Args...> const &t) {
  return construct_visitable_from_tuple_impl<T>(seq_enumerate_args_t<Args...>{}, t);
};
