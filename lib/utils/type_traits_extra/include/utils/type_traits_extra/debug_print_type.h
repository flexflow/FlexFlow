...
#define DEBUG_PRINT_TYPE(...)                                                  \
  ...
  using debug_print_proxy_type = typename debug_print_proxy_type_2 ::          \
      expected_to_not_exist_to_allow_printing_the_following

#define ERROR_PRINT_TYPE(cond, msg, ...)                                       \
  DEBUG_PRINT_TYPE(print_on_condition_t<UNWRAP_ARG(cond),                      \
                                        ADDITIONAL_ERROR_INFO<__VA_ARGS__>>)
