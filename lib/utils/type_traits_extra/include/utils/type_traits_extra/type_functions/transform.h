...
template <template <typename...> class Func, typename... Args>
struct transform<Func, std::tuple<Args...>, std::enable_if_t<is_nary_metafunction_v<Func, 1>>> : from_type_list<transform_t<Func, type_list<Args...>>> { };

template <template <typename...> class Func, typename... Args>
struct transform<Func, std::variant<Args...>, std::enable_if_t<is_nary_metafunction_v<Func, 1>>> : from_type_list<transform_t<Func, type_list<Args...>>> { };
