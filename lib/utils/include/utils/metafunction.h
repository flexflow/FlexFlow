...
template <template <typename...> class Cond, typename Enable = void, typename... Args>
struct metafunction_num_args {
  static constexpr int value = metafunction_num_args<Cond, Enable, int, Args...>::value;
};

template <template <typename...> class Cond, typename... Args>
struct metafunction_num_args<Cond, void_t<decltype(std::declval<Cond<Args...>>())>, Args...> : std::integral_constant<int, (sizeof...(Args))> { };
...
struct is_nary_metafunction<Func, N, enable_if_t<(metafunction_num_args<Func>::value == N)>> : std::true_type { };

template <template <typename...> class Func, typename Enable, typename ...Args>
struct internal_invoke_metafunction;

template <template <typename...> class Func, typename ...Args>
struct internal_invoke_metafunction<Func, typename std::enable_if<(metafunction_num_args<Func>::value == (sizeof...(Args)))>::type, Args...> : Func<Args...> { };
