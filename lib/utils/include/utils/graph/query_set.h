...
struct query_set {
  query_set() = delete;
  query_set(T const &) { NOT_IMPLEMENTED(); }
  query_set(std::unordered_set<T> const &) { NOT_IMPLEMENTED(); }
  query_set(optional<std::unordered_set<T>> const &) { NOT_IMPLEMENTED(); }
...
template <typename T>
query_set<T> query_intersection(query_set<T> const &lhs, query_set<T> const &rhs) {
  if (is_matchall(lhs)) {
    return rhs;
