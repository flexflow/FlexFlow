...
static_assert(is_rc_copy_virtual_compliant<INodeLabelledMultiDiGraph<int>>::value, RC_COPY_VIRTUAL_MSG);
...
static_assert(is_rc_copy_virtual_compliant<ILabelledMultiDiGraph<int, int>>::value, RC_COPY_VIRTUAL_MSG);
...
public:
  MultiDiInput(Node const & node_, size_t idx_)
  : node(node_), idx(idx_)
  {}
...
struct IOutputLabelledMultiDiGraph : public INodeLabelledMultiDiGraph<NodeLabel>,
                                     public IMultiDiGraphView {
public:
  virtual void add_output(MultiDiOutput const &output, OutputLabel const &label) = 0;
  virtual void add_edge(MultiDiOutput const &output, MultiDiInput const &input) = 0;
