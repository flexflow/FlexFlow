...
std::unordered_multiset<T>
  ...
  return visit<std::unordered_multiset<T>>(tt, overload {
...
std::unordered_multiset<T> get_nodes(GenericBinarySeriesSplit<T> const &s) {
  return multiset_union(get_nodes(get_left_child(s)), get_nodes(get_right_child(s)));
}

template <typename T>
std::unordered_multiset<T> get_nodes(GenericBinaryParallelSplit<T> const &p) {
  return multiset_union(get_nodes(get_left_child(p)), get_nodes(get_right_child(p)));
}
