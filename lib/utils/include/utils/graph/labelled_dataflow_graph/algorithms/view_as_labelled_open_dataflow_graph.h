...
struct LabelledDataflowGraphAsOpenView final : public ILabelledOpenDataflowGraphView<NodeLabel, ValueLabel> {
public:
  LabelledDataflowGraphAsOpenView() = delete;
  LabelledDataflowGraphAsOpenView(LabelledDataflowGraphView<NodeLabel, ValueLabel> const &g)
    : g(g) {}
  ...
  std::unordered_set<OpenDataflowEdge> query_edges(OpenDataflowEdgeQuery const &q) const override {
    return transform(this->g.query_edges(q.standard_edge_query), [](DataflowEdge const &e) { return OpenDataflowEdge{e}; });
  }
...
template <typename NodeLabel, typename ValueLabel>
LabelledOpenDataflowGraphView<NodeLabel, ValueLabel> view_as_labelled_open_dataflow_graph(LabelledDataflowGraphView<NodeLabel, ValueLabel> const &g) {
