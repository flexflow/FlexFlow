...
std::string as_dot(LabelledOpenDataflowGraphView<NodeLabel, ValueLabel> const &g,
  ...
  std::function<std::string(Node const &)> unlabelled_get_node_label = [&](Node const &n) -> std::string {
    return get_node_label(g.at(n));
  };

  std::function<std::string(DataflowGraphInput const &)> unlabelled_get_input_label = [&](DataflowGraphInput const &i) {
    return get_input_label(g.at(OpenDataflowValue{i}));
  };

  return as_dot(static_cast<OpenDataflowGraphView>(g), unlabelled_get_node_label, unlabelled_get_input_label);
}
