...
template <typename ReturnType, typename Tree, typename Series, typename Parallel, typename Leaf>
ReturnType visit(
    Tree const &tree,
    GenericBinarySPDecompositionTreeImplementation<Tree, Series, Parallel, Leaf> const &impl,
    GenericBinarySPDecompositionTreeVisitor<ReturnType, Tree, Series, Parallel, Leaf> const &visitor) {
  ...
  switch (node_type) {
    ...
    default:
      throw mk_runtime_error(fmt::format("Unknown SPDecompositionTreeNodeType value: {}", node_type));
  }
}
