...
bool is_binary_sp_tree_left_associative(
  ...
  auto visitor = GenericBinarySPDecompositionTreeVisitor<bool,
                                                         ...
                                                         Leaf>{
      [&](Series const &split) {
        return impl.get_node_type(impl.series_get_right_child(split)) !=
                   SPDecompositionTreeNodeType::SERIES &&
               is_binary_sp_tree_left_associative(
                   impl.series_get_left_child(split), impl) &&
               is_binary_sp_tree_left_associative(
                   impl.series_get_right_child(split), impl);
      },
      [&](Parallel const &split) {
        return impl.get_node_type(impl.parallel_get_right_child(split)) !=
