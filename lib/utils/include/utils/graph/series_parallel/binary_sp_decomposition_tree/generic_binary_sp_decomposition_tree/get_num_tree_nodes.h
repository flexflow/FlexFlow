...
int get_num_tree_nodes(GenericBinarySPDecompositionTree<SeriesLabel, ParallelLabel, LeafLabel> const &tt) {
  return visit<int>(tt,
                    overload{
...
template <typename SeriesLabel, typename ParallelLabel, typename LeafLabel>
int get_num_tree_nodes(GenericBinarySeriesSplit<SeriesLabel, ParallelLabel, LeafLabel> const &s) {
  return 1 + get_num_tree_nodes(get_left_child(s)) +
         get_num_tree_nodes(get_right_child(s));
}

template <typename SeriesLabel, typename ParallelLabel, typename LeafLabel>
int get_num_tree_nodes(GenericBinaryParallelSplit<SeriesLabel, ParallelLabel, LeafLabel> const &p) {
