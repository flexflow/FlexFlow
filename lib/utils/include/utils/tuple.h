...
void visit_tuple_impl(Visitor &v, std::tuple<Types...> const &tup) {
  ...
  if (Idx >= std::tuple_size<typename std::remove_reference<decltype(tup)>::type>::value) {
    return;
  } else {
...
struct tuple_compare_impl {
  static bool compare(const Tuple1& t1, const Tuple2& t2) {
...
struct tuple_compare_impl<Tuple1, Tuple2, 0> {
  static bool compare(const Tuple1&, const Tuple2&) {
    return true;
  }
};

template <typename Tuple1, typename Tuple2>
bool tuple_compare(const Tuple1& t1, const Tuple2& t2) {
  static_assert(std::tuple_size<Tuple1>::value == std::tuple_size<Tuple2>::value,
                "Tuples must have the same size");
  return tuple_compare_impl<Tuple1, Tuple2, std::tuple_size<Tuple1>::value>::compare(t1, t2);
}
