#ifndef _FLEXFLOW_UTILS_INCLUDE_UTILS_TUPLE_EXTRA_INCLUDE_TUPLE_EXTRA_H
#define _FLEXFLOW_UTILS_INCLUDE_UTILS_TUPLE_EXTRA_INCLUDE_TUPLE_EXTRA_H
...
std::any get(std::tuple<Types...> const &t, int idx) {
  ...
  if (idx < 0 || idx >= tuple_size) {
    ...
    oss << "Error: idx " << idx << " out of bounds for tuple of size " << tuple_size;
