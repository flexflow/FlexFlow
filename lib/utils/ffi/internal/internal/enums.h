...
#define REGISTER_FFI_ENUM(EXTERNAL, INTERNAL, ERROR_CODE, ...)                 \
  ...
  const bidict<EXTERNAL, INTERNAL> enum_mapping<EXTERNAL>::mapping =           \
      __VA_ARGS__;

template <typename ExternalEnum>
external_to_internal_t<ExternalEnum> to_internal_impl(ExternalEnum e) {
  return enum_mapping<ExternalEnum>::mapping.maybe_at_l(e)
      .or_else([] {
        throw make_opattrs_error(enum_mapping<ExternalEnum>::err_code);
      })
      .value();
}
...
internal_to_external_t<InternalEnum> to_external_impl(InternalEnum i) {
  ...
  return Mapping::mapping.maybe_at_r(i)
