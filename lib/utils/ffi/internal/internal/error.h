...
template <typename T>
flexflow_error_t flexflow_error_wrap(flexflow_error_source_t error_source, T const &t) {
...
template <typename T>
flexflow_error_t flexflow_error_unwrap(flexflow_error_t const &err, flexflow_error_source_t error_source, T *out) {
  static_assert(sizeof(T) < (FLEXFLOW_FFI_ERROR_BUF_SIZE * sizeof(char)), ""); 
  ...
  out->impl = reinterpret_cast<decltype(unwrap_opaque(std::declval<T>()))>(err.buf);
