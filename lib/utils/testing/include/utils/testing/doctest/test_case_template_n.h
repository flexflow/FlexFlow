...
#define DOCTEST_TEST_CASE_TEMPLATE_T_DEFINE_IMPL(dec, T_TUP, iter, func)       \
  ...
  struct iter<std::tuple<std::tuple<ArgTypes...>, Rest...>> {                  \
    iter(const char *file, unsigned line, int index) {                         \
      ...
      std::vector<doctest::String> arg_type_names = {                          \
          doctest::toString<ArgTypes>()...};                                   \
...
#define DOCTEST_TEST_CASE_TEMPLATE_T_IMPL(dec, T_TUP, anon, ...)               \
  DOCTEST_TEST_CASE_TEMPLATE_T_DEFINE_IMPL(                                    \
      dec, T_TUP, DOCTEST_CAT(anon, ITERATOR), anon);                          \
...
#define DOCTEST_TEST_CASE_TEMPLATE_T(dec, T_TUP, ...)                          \
  DOCTEST_TEST_CASE_TEMPLATE_T_IMPL(                                           \
      dec, T_TUP, DOCTEST_ANONYMOUS(DOCTEST_ANON_TMP_), __VA_ARGS__)

#define TEST_CASE_TEMPLATE_T(name, T_TUP, ...)                                 \
  DOCTEST_TEST_CASE_TEMPLATE_T(name, T_TUP, __VA_ARGS__)
