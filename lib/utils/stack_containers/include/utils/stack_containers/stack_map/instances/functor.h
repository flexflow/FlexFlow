...
struct keys_functor<stack_map<K, V, MAXSIZE>> { 
  ...
  static F<std::invoke_result_t<Func, A>> fmap(stack_map<A, V> const &m, Func const &f) {
...
struct values_functor<stack_map<K, V, MAXSIZE>> { 
  ...
  template <typename Func>
  static F<std::invoke_result_t<Func, A>> fmap(stack_map<K, A, MAXSIZE> const &m, Func const &f) {
    using B = std::invoke_result_t<Func, A>;
