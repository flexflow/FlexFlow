...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("transform_keys(bidict<K, V> const &, F const &)") {
    SUBCASE("type-preserving") {
      ...
      bidict<int, std::string> result =
          transform_keys(b, [](int k) { return k + 1; });
    ...
    SUBCASE("type-changing") {
      ...
      bidict<std::string, std::string> result =
          transform_keys(b, [](int k) { return std::to_string(k); });
  ...
  TEST_CASE("transform_values(bidict<K, V> const &, F const &)") {
    SUBCASE("type-preserving") {
      ...
      bidict<int, std::string> result = transform_values(
          b, [](std::string const &v) { return v + " plus one"; });
    ...
    SUBCASE("type-changing") {
      ...
      bidict<int, size_t> result =
          transform_values(b, [](std::string const &v) { return v.size(); });
