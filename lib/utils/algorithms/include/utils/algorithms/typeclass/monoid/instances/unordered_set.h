...
struct unordered_set_monoid {
  static std::unordered_set<T> mempty() {
    return {};
  }
  static void mappend_inplace(std::unordered_set<T> &acc,
                              std::unordered_set<T> const &val) {
    for (T const &t : val) {
      acc.insert(t);
    }
  }
...
template <typename T>
struct default_monoid<std::unordered_set<T>>
    : type_identity<unordered_set_monoid<T>> {};
