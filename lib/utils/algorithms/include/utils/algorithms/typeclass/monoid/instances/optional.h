...
struct optional_monoid {
  static std::optional<T> mempty() { return std::nullopt; }
  static void mappend_inplace(std::optional<T> &lhs, std::optional<T> const &rhs) {
    if (lhs.has_value()) {
...
template <typename T>
struct default_monoid<std::optional<T>> : optional_monoid<T, default_monoid_t<T>> {};
