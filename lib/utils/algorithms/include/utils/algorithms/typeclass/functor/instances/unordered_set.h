...
struct unordered_set_functor {
  ...
  template <
      typename Func,
      typename = std::enable_if_t<is_hashable_v<std::invoke_result_t<Func, A>>>>
  ...
  static std::vector<std::invoke_result_t<F, In>> fmap(std::vector<In> const &v,
                                                       F const &f) {
...
template <typename T>
struct default_functor<std::unordered_set<T>,
                       std::enable_if_t<is_hashable_v<T>>>
    : type_identity<unordered_set_functor<T>> {};
