...
/* inline constexpr bool is_valid_functor_instance_v =
 * is_valid_functor_instance<Instance>::value; */

/* /1* template <typename T, typename Instance, typename InputType =
 * other_functor_opaque_t, typename OutputType = functor_opaque_t> *1/ */
/* /1* struct is_valid_functor_instance2 *1/ */
/* /1*   : std::is_same< *1/ */
/* /1*       std::invoke_result_t<decltype(Instance::template fmap<InputType>),
 * T<InputType> const &, std::function<OutputType(InputType const &)>>, *1/ */
/* /1*       T<OutputType> *1/ */
/* /1*     > { }; *1/ */
