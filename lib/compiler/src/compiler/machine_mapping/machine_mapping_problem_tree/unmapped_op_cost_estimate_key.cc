...
UnmappedOpCostEstimateKey get_unmapped_op_cost_estimate_key_for_layer(ParallelComputationGraph const &pcg,
  ...
  return UnmappedOpCostEstimateKey{
    ...
    /*input_shapes=*/transform(get_incoming_inputs(pcg, layer), get_tensor_shape),
    /*weight_shapes=*/transform(get_incoming_weights(pcg, layer), get_tensor_shape),
    /*output_shapes=*/transform(get_layer_outputs(pcg, layer), get_tensor_shape),
  };
}
