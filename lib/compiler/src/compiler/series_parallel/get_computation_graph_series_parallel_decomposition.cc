...
std::string render_preprocessed_computation_graph_for_sp_decomposition(ComputationGraph const &cg) {
  ...
  std::unordered_set<layer_guid_t> weight_and_input_layer_successors = get_subgraph_successors(cg, weight_and_input_layers);

  // dot has is incapable of rendering the number of edges in the all-to-all connection,
  // so for visualization purposes we instead insert a "fake" node to reduce the n^2 edges
  // to 2*n edges
  DiGraph preprocessed_digraph = materialize_digraph_view<AdjacencyDiGraph>(cg.raw_graph);
...
std::optional<SerialParallelDecomposition> get_computation_graph_series_parallel_decomposition(ComputationGraph const &cg) {
  std::unordered_set<layer_guid_t> weight_and_input_layers = filter(get_layers(cg), [&](layer_guid_t const &l) {  
    ComputationGraphOpAttrs op_attrs = get_layer_attrs(cg, l).attrs;
    return op_attrs.has<WeightAttrs>() || op_attrs.has<InputAttrs>();
  });
