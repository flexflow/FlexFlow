...
std::vector<MultiDiEdge> get_sorted_node_input_edges(ParallelComputationGraph const &pcg, Node const &n) {
  std::unordered_map<std::size_t, std::unordered_set<MultiDiEdge>> incoming_edges = utils::get_incoming_edges_by_idx(pcg.g, n);
...
std::unordered_map<utils::MultiDiEdge, opmeta::ParallelTensorShape> infer_tensor_shapes(ParallelComputationGraph const &pcg) {
  std::unordered_map<utils::MultiDiEdge, opmeta::ParallelTensorShape> result;
  for (Node const &n : utils::get_topological_ordering(pcg.g)) {
    OperatorParameters op = pcg.nodeMap.at(n); 

    std::vector<ParallelTensorShape> input_tensor_shapes = utils::vector_transform(
    ...
    for (std::size_t i = 0; i < output_tensor_shapes.size(); i++) {
      if (utils::contains_key(outgoing_edges, i)) {
...
struct OpenSubParallelComputationGraph {
  std::unique_ptr<utils::IDownwardOpenMultiDiGraphView const> g;
  std::unordered_map<utils::Node, opmeta::OperatorParameters> nodeMap;
  std::unordered_map<utils::DownwardOpenMultiDiEdge, MachineView> const &output_machine_views;
};
...
std::pair<
  OpenSubParallelComputationGraph, 
  OpenSubParallelComputationGraph,
> apply_split(OpenSubParallelComputationGraph const &g, utils::GraphSplit const &split) {
...
std::pair<
  OpenSubParallelComputationGraph,
  ParallelComputationGraph
> apply_split(ParallelComputationGraph const &g, utils::GraphSplit const &split) {
