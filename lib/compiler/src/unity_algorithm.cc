...
GraphOptimizeResult graph_optimize(
  ...
  std::vector<Substitution> substitutions = get_all_applicable_substitutions(pcg);
  ...
  GraphOptimizeState initial_state = {GraphOptimizeResult(pcg,original_pcg_cost.machine_mapping), original_pcg_cost.runtime};

  GraphOptimizeState best_state = initial_state;
  candidates.push(initial_state);

  for (int iteration = 0; !candidates.empty() && iteration < opt_config.budget; ++iteration) {
    ...
    if (current_state.runtime < best_state.runtime) {
      best_state = current_state;
               ...
               best_state.runtime * opt_config.alpha) {
      continue;
    }

    for (Substitution const &substitution : substitutions) {
      for (ParallelComputationGraph const &new_pcg : apply_substitution(current_state.graph_optimize_result.pcg, substitution)) {
        OptimalCostResult new_pcg_cost = optimal_cost(new_pcg,
                                          allowed_machine_views,
                                          cost_estimator,
                                          resources,
                                          cached_subgraph_costs);
        GraphOptimizeState new_state{GraphOptimizeResult(new_pcg, new_pcg_cost.machine_mapping), new_pcg_cost.runtime};
