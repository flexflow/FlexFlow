...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("get_machine_resource_splits") {
    ...
    SUBCASE("returns no splits if no splits are possible") {
      ...
      std::unordered_set<std::pair<MachineSpecification, MachineSpecification>> result = get_machine_resource_splits(input);
      std::unordered_set<std::pair<MachineSpecification, MachineSpecification>> correct = {};

      CHECK(result == correct);
    }

    SUBCASE("returns splits in gpu and node dimensions, but not at the same time") {
