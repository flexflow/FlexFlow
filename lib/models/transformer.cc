...
ComputationGraph get_transformer_computation_graph(TransformerConfig const &tfConfig) {
    /**
     * @brief Helper to create an attention encoder layer
     */
    auto create_attention_encoder =
        [&](ComputationGraphBuilder *cgb, tensor_guid_t const &input, int hidden_dim,
            int num_heads, int kdim, int vdim) -> tensor_guid_t {
        tensor_guid_t t = cgb->multihead_attention(input, input, input, hidden_dim,
                                                   num_heads, kdim, vdim);
