...
void top_level_task(Task const *task,
  ...
  ts[0] = flexflow_model_add_conv2d(
      ffmodel, "conv1", t0, 64, 11, 11, 4, 4, 2, 2, AC_MODE_NONE, true);
  ts[1] = flexflow_model_add_conv2d(
      ffmodel, "conv1", t0, 64, 11, 11, 4, 4, 2, 2, AC_MODE_NONE, true);
  flexflow_tensor_t t1 = flexflow_model_add_concat(ffmodel, "concat", 2, ts, 1);
  flexflow_tensor_t t2 = flexflow_model_add_pool2d(
      ffmodel, "pool1", t1, 3, 3, 2, 2, 0, 0, POOL_MAX, AC_MODE_NONE);
  flexflow_tensor_t t3 = flexflow_model_add_conv2d(
      ffmodel, "conv2", t2, 192, 5, 5, 1, 1, 2, 2, AC_MODE_NONE, true);
  flexflow_tensor_t t4 = flexflow_model_add_pool2d(
      ffmodel, "pool2", t3, 3, 3, 2, 2, 0, 0, POOL_MAX, AC_MODE_NONE);
  flexflow_tensor_t t5 = flexflow_model_add_conv2d(
      ffmodel, "conv3", t4, 384, 3, 3, 1, 1, 1, 1, AC_MODE_NONE, true);
  flexflow_tensor_t t6 = flexflow_model_add_conv2d(
      ffmodel, "conv4", t5, 256, 3, 3, 1, 1, 1, 1, AC_MODE_NONE, true);
  flexflow_tensor_t t7 = flexflow_model_add_conv2d(
      ffmodel, "conv5", t6, 256, 3, 3, 1, 1, 1, 1, AC_MODE_NONE, true);
  flexflow_tensor_t t8 = flexflow_model_add_pool2d(
      ffmodel, "pool3", t7, 3, 3, 2, 2, 0, 0, POOL_MAX, AC_MODE_NONE);
  flexflow_tensor_t t9 = flexflow_model_add_flat(ffmodel, "flat", t8);
  flexflow_tensor_t t10 = flexflow_model_add_dense_with_default_initializer(
      ffmodel, "linear1", t9, 4096, AC_MODE_RELU, true);
  flexflow_tensor_t t11 = flexflow_model_add_dense_with_default_initializer(
      ffmodel, "linear2", t10, 4096, AC_MODE_RELU, true);
  flexflow_tensor_t t12 = flexflow_model_add_dense_with_default_initializer(
      ffmodel, "linear3", t11, 1000, AC_MODE_NONE, true);
  flexflow_tensor_t t13 =
      flexflow_model_add_softmax(ffmodel, "softmax", t12, label);
