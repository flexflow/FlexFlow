...
from align_ff_utils import (compile_ffmodel, init_ffmodel, run_fwd_bwd,
                            ...
                            save_tensor_grad_ff)
from align_utils import BATCH_SIZE, parse_create_tensor_args, gen_tensor
INPUT_SIZE = 512
SEQ_LENGTH = 5

#set of operaters that have weight/bias
param_weight_op = {'conv2d': Conv2D, 'embedding': Embedding, 'layernorm': LayerNorm, 'view_embedding': Embedding}
param_bias_op = {'conv2d': Conv2D, 'layernorm': LayerNorm}
...
def create_single_operator_ff():
    ...
    if operator_name == 'add':
        input_tensors, label, output_tensor = create_tensors_for_add_ff(ffmodel)
    elif operator_name == 'concat':
        input_tensors, label, output_tensor = create_tensors_for_concat_ff(ffmodel)
    elif operator_name == 'conv2d':
        input_tensors, label, output_tensor = create_tensors_for_conv2d_ff(ffmodel)
    elif operator_name == 'cos':
        input_tensors, label, output_tensor = create_tensors_for_cos_ff(ffmodel)
    elif operator_name == 'embedding':
        input_tensors, label, output_tensor = create_tensors_for_embedding_ff(ffmodel)
    elif operator_name == 'exp':
        input_tensors, label, output_tensor = create_tensors_for_exp_ff(ffmodel)
    elif operator_name == 'flat':
        input_tensors, label, output_tensor = create_tensors_for_flat_ff(ffmodel)
    elif operator_name == 'getitem':
        input_tensors, label, output_tensor = create_tensors_for_getitem_ff(ffmodel)
    elif operator_name == 'identity':
        input_tensors, label, output_tensor = create_tensors_for_identity_ff(ffmodel)
    elif operator_name == 'multiply':
        input_tensors, label, output_tensor = create_tensors_for_multiply_ff(ffmodel)
    elif operator_name == 'pool2d':
        input_tensors, label, output_tensor = create_tensors_for_pool2d_ff(ffmodel)
    elif operator_name == 'reducesum':
        input_tensors, label, output_tensor = create_tensors_for_reducesum_ff(ffmodel)
    elif operator_name == 'relu':
        input_tensors, label, output_tensor = create_tensors_for_relu_ff(ffmodel)
    elif operator_name == 'reshape':
        input_tensors, label, output_tensor = create_tensors_for_reshape_ff(ffmodel)
    elif operator_name == 'scalar_add':
        input_tensors, label, output_tensor = create_tensors_for_scalar_add_ff(ffmodel)
    elif operator_name == 'scalar_multiply':
        input_tensors, label, output_tensor = create_tensors_for_scalar_multiply_ff(ffmodel)
    elif operator_name == 'scalar_sub':
        input_tensors, label, output_tensor = create_tensors_for_scalar_sub_ff(ffmodel)
    elif operator_name == 'scalar_truediv':
        input_tensors, label, output_tensor = create_tensors_for_scalar_truediv_ff(ffmodel)
    elif operator_name == 'sigmoid':
        input_tensors, label, output_tensor = create_tensors_for_sigmoid_ff(ffmodel)
    elif operator_name == 'sin':
        input_tensors, label, output_tensor = create_tensors_for_sin_ff(ffmodel)
    elif operator_name == 'subtract':
        input_tensors, label, output_tensor = create_tensors_for_subtract_ff(ffmodel)
    elif operator_name == 'tanh':
        input_tensors, label, output_tensor = create_tensors_for_tanh_ff(ffmodel)
    elif operator_name == 'transpose':
        input_tensors, label, output_tensor = create_tensors_for_transpose_ff(ffmodel)
    elif operator_name == 'view_embedding':
        input_tensors, label, output_tensor = create_tensors_for_view_embedding_ff(ffmodel)
    else:
        raise ValueError('Not include such Operator in Aligment Test', operator_name)
        
    compile_ffmodel(ffmodel)
    dls = init_ffmodel(ffmodel, input_tensors, label)
    assert len(dls) == len(input_tensors) + 1
    
    input_dls = dls[0 : len(dls) - 1]
    label_dl = dls[len(dls) - 1]
    # forward/backward pass
    run_fwd_bwd(ffmodel, ffconfig, input_dls, label_dl)
    # save data
    ...
    if(operator_name in param_weight_op):
        ...
        save_param_grad_ff(weight, ffmodel, os.path.join(OUT_DIR, "ff_weight_grad.pt"))
    if(operator_name in param_bias_op):
        ...
        save_param_grad_ff(bias, ffmodel, os.path.join(OUT_DIR, "ff_bias_grad.pt"))
