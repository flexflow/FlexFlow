...
def BottleneckBlock(ff, input, out_channels, stride):
  t = ff.conv2d(input, out_channels, 1, 1, 1, 1, 0, 0, ActiMode.AC_MODE_RELU)
  t = ff.conv2d(t, out_channels, 3, 3, stride, stride, 1, 1, ActiMode.AC_MODE_RELU)
  ...
  if ((stride > 1) or (input.dims[3] != out_channels * 4)):
    ...
    input = ff.conv2d(input, 4*out_channels, 1, 1, stride, stride, 0, 0, ActiMode.AC_MODE_RELU)
