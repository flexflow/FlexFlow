                    ...
                    Context ctx, Runtime* runtime)
{
  ...
  Tensor input;
  {
    const int dims[] = {ffConfig.batchSize, 3, 3, 3};
...
//-----------------------------------------------------------------
  ...
  Tensor gate_preds = ff.dense(flat_samples, num_exp, AC_MODE_RELU);

  Tensor topK_output[2];
  ff.top_k(gate_preds, topK_output, num_select, false);

  Tensor exp_tensors[num_exp];
  ff.group_by(flat_samples, topK_output[1], exp_tensors, num_exp, 1.5f);
