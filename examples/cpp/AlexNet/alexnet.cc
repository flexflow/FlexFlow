...
void FlexFlow::top_level_task(Task const *task,
  ...
  //  Add layers
  Tensor t = input, ts[2];
  ts[0] = ff.conv2d(input, 64, 11, 11, 4, 4, 2, 2, AC_MODE_RELU);
  ts[1] = ff.conv2d(input, 64, 11, 11, 4, 4, 2, 2, AC_MODE_RELU);
  t = ff.concat(2, ts, 1 /*axis*/);
    ...
    for (int iter = 0; iter < iterations; iter++) {
      ...
      for (int iter_inner = 0; iter_inner < ff.iter_perbatch; iter_inner++) {
        if (std::strlen(alexnetConfig.dataset_path) == 0) {
          ...
          // shicao pipeline
          for (int i = 0;
               i < data_loader.batch_input->parallel_tensor->owner_op->nFnB;
               i++) {
...
void DataLoader::next_input_ubatch(FFModel &ff) {
  ...
  // Load input
  {
    // IndexSpaceT<4> task_is = (IndexSpaceT<4>)
    ...
    for (Domain::DomainPointIterator it(domain); it; it++) {
      ...
      meta.num_samples =
          ubSize / batch_input->parallel_tensor->dims[ndims - 2].degree;
      for (int i = 0; i < meta.num_samples; i++)
        meta.idxs[i] = idx++;
      argmap.set_point(*it, TaskArgument(&meta, sizeof(SampleIdxs)));
    }
    ...
    launcher.add_field(0, FID_DATA);
    launcher.add_region_requirement(
        RegionRequirement(batch_input->parallel_tensor->out_pipepart[input_idx],
                          0 /*projection id*/,
                          WRITE_ONLY,
                          EXCLUSIVE,
                          batch_input->parallel_tensor->region));
    launcher.add_field(1, FID_DATA);
    input_idx =
        (input_idx + 1) % batch_input->parallel_tensor->pipe_num_part_out;
    runtime->execute_index_space(ctx, launcher);
  }
