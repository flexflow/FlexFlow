...
LegionRuntime::Logger::Category log_app("SimpleNet");

void parse_input_args(char **argv, int argc, SimpleNetConfig& config)
{
...
{
  FFConfig ffConfig;
  SimpleNetConfig simplenetConfig;
  {
    const InputArgs &command_args = HighLevelRuntime::get_input_args();
    char **argv = command_args.argv;
    int argc = command_args.argc;
    parse_input_args(argv, argc, simplenetConfig);
...
//-----------------------------------------------------------------
  ...
  // Data Loader
  DataLoader data_loader(ff, simplenetConfig, input, ff.label_tensor);
  ff.init_layers();
  //Start timer
  {
  ...
  for (int epoch = 0; epoch < ffConfig.epochs; epoch++) {
    ...
    for (int iter = 0; iter < iterations; iter++) {
      if (simplenetConfig.dataset_path.length() == 0) {
...
DataLoader::DataLoader(FFModel& ff, const SimpleNetConfig& simplenet,
                       Tensor input, Tensor label)
{
  Context ctx = ff.config.lg_ctx;
  Runtime* runtime = ff.config.lg_hlr;
  num_samples = 0;
  if (simplenet.dataset_path == "") {
    log_app.print("Use random dataset...");
    num_samples = 256 * 10 * ff.config.workersPerNode * ff.config.numNodes;
    log_app.print("Number of random samples = %d\n", num_samples);
  } else {
    log_app.print("Start loading dataset from %s", simplenet.dataset_path.c_str());
    size_t filesize = get_file_size(simplenet.dataset_path);
  ...
  const SimpleNetConfig* ptr = &simplenet;
  TaskLauncher launcher(CUSTOM_CPU_TASK_ID_1,
      TaskArgument(&ptr, sizeof(SimpleNetConfig*)));
...
void DataLoader::load_entire_dataset(const Task *task,
  ...
  Context ctx, Runtime* runtime)
{
  const SimpleNetConfig* alexnet = *((SimpleNetConfig**)task->args);
