...
int main(int argc, char *argv[]) {
  ...
  std::ifstream infile(
      "/Users/goliaro/Desktop/wikitext-103-raw/wiki.valid.raw");
  if (!infile) {
    std::cout << "Error opening input file" << std::endl;
    return -1;
  }
  std::ofstream outfile(
      "/Users/goliaro/Desktop/wikitext-103-raw/wiki.valid.bpe3",
      std::ofstream::out);
  if (!outfile) {
  ...
  for (auto l = lines.begin(); l != lines.end(); ++l) {
    std::string stripped_line =
        (line.length() > 0) ? tokenizer.strip(*l) : (*l);
    if (stripped_line.length() == 0) {
      outfile << *l << std::endl;
    } else {
      std::cout << "stripped_line: '" << stripped_line
                << "' len: " << stripped_line.length() << std::endl;
