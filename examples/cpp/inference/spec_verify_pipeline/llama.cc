    ...
    if (beam_future_handlers.find(bid) == beam_future_handlers.end()) {
      ...
      if (depth >= llama_config.max_beam_depth) {
        ...
        for (int i = 0; i < tree_bc.num_tokens; i++) {
          ...
          printf("treebc.tokensInfo[%d].abs_depth_in_request: %d\n", i, tree_bc.tokensInfo[i].abs_depth_in_request);
          printf("treebc.tokensInfo[%d].request_index: %d\n", i, tree_bc.tokensInfo[i].request_index);
        }
