...
DataLoaderV2::DataLoaderV2(FFModel &ff,
...
void DataLoaderV2::load_entire_dataset(
    Task const *task,
...
void DataLoaderV2::next_batch(FFModel &ff,
                              BatchConfigV2 *bc,
  ...
  // Load Input
  {
    ...
    DataLoaderNextBatchInputV2 next_batch_input = {bc, batch_predictions};
    DataLoaderNextBatchInputV2 const *ptr = &next_batch_input;
...
template <typename T>
void DataLoaderV2::load_from_file(T *ptr, size_t size, std::string filename) {
...
template <typename T>
void DataLoaderV2::load_attention_weights(T *ptr,
                                          size_t size,
...
void DataLoaderV2::store_outputs(BatchConfigV2 *bc,
...
template void DataLoaderV2::load_attention_weights<float>(
    float *ptr, size_t size, std::string layer_name, std::string weight_path);
template void DataLoaderV2::load_from_file<long>(long *ptr,
                                                 size_t size,
                                                 std::string filename);
template void DataLoaderV2::load_from_file<float>(float *ptr,
                                                  size_t size,
                                                  std::string filename);
...
void FlexFlow::register_custom_tasks() {
  // Load entire dataset
  {
    ...
    registrar.set_leaf();
    Runtime::preregister_task_variant<DataLoaderV2::load_entire_dataset>(
  ...
  // Load input
  {
    ...
    registrar.set_leaf();
    Runtime::preregister_task_variant<DataLoaderV2::load_input>(
