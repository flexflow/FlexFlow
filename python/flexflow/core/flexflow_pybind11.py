...
from .flexflow_pybind11_internal import (
    ActiMode,
    CompMode,
    DataType,
    LossType,
...
def get_weights(self, ffmodel):
    ...
    fflogger.debug(
        "get weights raw_ptr: %s, %s, %s"
        % (str(np_raw_ptr[0]), hex(np_raw_ptr[0]), str(shape))
    )
...
def set_weights(self, ffmodel, np_array):
    ...
    assert num_dims == self.num_dims, "please check dims (%d == %d)" % (
        num_dims,
        self.parameter.num_dims,
    )
    for i in range(0, num_dims):
        assert np_shape[i] == self.dims[i], "please check shape dim %d (%d == %d)" % (
            i,
            np_shape[i],
            self.dims[i],
        )
    np_raw_ptr = np_array.__array_interface__["data"]
    fflogger.debug(
        "set weights raw_ptr: %s, %s, %s"
        % (str(np_raw_ptr[0]), hex(np_raw_ptr[0]), str(np_shape))
    )
...
class FFModel(_FFModel):
    ...
    def compile(
        self, optimizer=None, loss_type=None, metrics=None, comp_mode=CompMode.TRAINING
    ):
        if optimizer != None:
