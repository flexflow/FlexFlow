  ...
  int num_samples)
{
  ...
  Tensor *input = FFCObjectWrapper::unwrap(input_);
  Tensor *full_input = FFCObjectWrapper::unwrap(full_input_);
  SingleDataLoader4DFloat *dataloader = new SingleDataLoader4DFloat(*ffmodel, *input, *full_input, num_samples);
  return FFCObjectWrapper::wrap(dataloader);  
}

void  
flexflow_single_dataloader_4d_float_destroy(
  flexflow_single_dataloader_4d_float_t handle_)
{
  SingleDataLoader4DFloat *handle = FFCObjectWrapper::unwrap(handle_);
  delete handle;
}
  ...
  int num_samples)
{
  ...
  Tensor *input = FFCObjectWrapper::unwrap(input_);
  Tensor *full_input = FFCObjectWrapper::unwrap(full_input_);
  SingleDataLoader2DFloat *dataloader = new SingleDataLoader2DFloat(*ffmodel, *input, *full_input, num_samples);
  return FFCObjectWrapper::wrap(dataloader);  
}

void  
flexflow_single_dataloader_2d_float_destroy(
  flexflow_single_dataloader_2d_float_t handle_)
{
  SingleDataLoader2DFloat *handle = FFCObjectWrapper::unwrap(handle_);
  delete handle;
}
  ...
  int num_samples)
{
  ...
  Tensor *input = FFCObjectWrapper::unwrap(input_);
  Tensor *full_input = FFCObjectWrapper::unwrap(full_input_);
  SingleDataLoader2DInt *dataloader = new SingleDataLoader2DInt(*ffmodel, *input, *full_input, num_samples);
  return FFCObjectWrapper::wrap(dataloader);  
}

void  
flexflow_single_dataloader_2d_int_destroy(
  flexflow_single_dataloader_2d_int_t handle_)
{
  SingleDataLoader2DInt *handle = FFCObjectWrapper::unwrap(handle_);
  delete handle;
}
...
void register_c_custom_tasks()
{
  ...
  // 4D float Load entire dataset from numpy
  {
    ...
    registrar.add_constraint(ProcessorConstraint(Processor::LOC_PROC));
    registrar.set_leaf();
    Runtime::preregister_task_variant<SingleDataLoader4DFloat::load_entire_dataset_from_numpy>(
  ...
  // 2D float Load entire dataset from numpy
  {
    ...
    registrar.add_constraint(ProcessorConstraint(Processor::LOC_PROC));
    registrar.set_leaf();
    Runtime::preregister_task_variant<SingleDataLoader2DFloat::load_entire_dataset_from_numpy>(
  ...
  // 4D float load input
  {
    ...
    registrar.add_constraint(ProcessorConstraint(Processor::TOC_PROC));
    registrar.set_leaf();
    Runtime::preregister_task_variant<SingleDataLoader4DFloat::load_input>(
  ...
  // 2D float load input
  {
    ...
    registrar.add_constraint(ProcessorConstraint(Processor::TOC_PROC));
    registrar.set_leaf();
    Runtime::preregister_task_variant<SingleDataLoader2DFloat::load_input>(
