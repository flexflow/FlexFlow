                               ...
                               Runtime* runtime)
{
  ...
  hipLaunchKernelGGL(copy_kernel, GET_BLOCKS(acc_batch_label.rect.volume()), CUDA_NUM_THREADS, 0, stream,
                                 ...
                                 Runtime* runtime)
{
  ...
  hipStream_t stream;
  checkCUDA(get_legion_stream(&stream));
  hipLaunchKernelGGL(copy_kernel, GET_BLOCKS(acc_batch_input.rect.volume()), CUDA_NUM_THREADS, 0, stream,
                                  ...
                                  Runtime* runtime)
{
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(copy_kernel<DT>), GET_BLOCKS(batch_input_domain.get_volume()), CUDA_NUM_THREADS, 0, stream,
                                     ...
                                     Runtime* runtime)
{
  ...
  checkCUDA(get_legion_stream(&stream));
  //printf("ptr(%p, %p), idx0 %d nb_elements_per_batch %d, batch_size %d, %d\n", acc_full_input.ptr, input_zc, start_idx, num_elements_per_batch, batch_size, start_idx * num_elements_per_batch);
  hipLaunchKernelGGL(HIP_KERNEL_NAME(copy_kernel<DT>), GET_BLOCKS(acc_batch_input.rect.volume()), CUDA_NUM_THREADS, 0, stream,
      acc_batch_input.ptr, input_zc, acc_batch_input.rect.volume());
  checkCUDA(hipDeviceSynchronize());
}
#endif
