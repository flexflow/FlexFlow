...
FFHandler
  ...
  if (handle.batch_config_metadata_size + handle.attention_metadata_size > 0) {
    // allocate memory for offload reserve space
    Memory gpu_mem = Machine::MemoryQuery(Machine::get_machine())
                         .only_kind(Memory::GPU_FB_MEM)
                         .best_affinity_to(task->target_proc)
    ...
    Realm::Rect<1, coord_t> bounds(
        Realm::Point<1, coord_t>(0),
        Realm::Point<1, coord_t>(handle.batch_config_metadata_size + handle.attention_metadata_size - 1));
    std::vector<size_t> field_sizes;
    field_sizes.push_back(sizeof(char));
    Realm::RegionInstance workspaceInst;
    Realm::RegionInstance::create_instance(workspaceInst,
    ...
    handle.attention_metadata = nullptr;
  }
...
void UtilityTasks::init_images_task(Task const *task,
  ...
  hipStream_t stream;
  checkCUDA(get_legion_stream(&stream));
  hipLaunchKernelGGL(init_image_kernel,
                     num_blocks,
                     BLKSIZE,
                     0,
                     stream,
