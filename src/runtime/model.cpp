         ...
         Context ctx, Runtime *runtime)
{
  ...
  hipStream_t stream;
  checkCUDA(get_legion_stream(&stream));
  hipLaunchKernelGGL(apply_normalize, GET_BLOCKS(rect_tensor.volume()), CUDA_NUM_THREADS, 0, stream,
                                    ...
                                    Context ctx, Runtime *runtime)
{
  ...
  hipStream_t stream;
  checkCUDA(get_legion_stream(&stream));
  hipLaunchKernelGGL(init_image_kernel, num_blocks, BLKSIZE, 0, stream, image_ptr, rect_image.volume());
}
                                    ...
                                    Context ctx, Runtime *runtime)
{
  ...
  hipStream_t stream;
  checkCUDA(get_legion_stream(&stream));
  hipLaunchKernelGGL(init_label_kernel, num_blocks, BLKSIZE, 0, stream, label_ptr, rect_label.volume());
}
