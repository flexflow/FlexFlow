...
BatchConfig RequestManager::prepare_next_batch(BatchConfig const &old_bc,
  ...
  BeamSearchBatchConfig RequestManager::prepare_next_batch_init(
      ...
      if (new_bc.request_completed[i]) {
        if (!pending_request_queue.empty() &&
          ...
          std::cout << "LLM KV Cache Size init: "
                    << all_requests[new_request.guid].llm_cache_size
                    << std::endl;
  ...
  BeamSearchBatchConfig RequestManager::prepare_next_batch_beam(
      BeamSearchBatchConfig const &old_bc, BeamInferenceResult const &result) {
    const std::lock_guard<std::mutex> lock(request_queue_mutex);
    if (verbose) {
      std::cout
    ...
    for (int i = 0; i < BatchConfig::max_requests_per_batch(); i++) {
      ...
      {
        log_req_mgr.debug()
            << "num tokens: " << old_bc.num_tokens << ", " << new_bc.num_tokens;
        new_bc.request_completed[i] = false;
      ...
      if (request.status == Request::RUNNING) {
        ...
        for (int j = 0; j < old_batches.size(); j++) {
          std::vector<std::pair<BatchConfig::TokenId, int>> new_tree =
              traverse_beam_tree(
                  old_batches.at(j), i, request.tokens.size() - 1);
        ...
        if (request.llm_cache_size < request.initial_len) {
          ...
          if (new_bc.requestsInfo[i].num_tokens_in_batch +
            ...
            dfs_tree_inputs[guid] =
                std::vector<std::pair<BatchConfig::TokenId, int>>{
                    std::make_pair(request.tokens.back(),
                                   request.tokens.size() - 1)};
          }
      ...
      if (i == old_bc.num_tokens ||
        ...
        if (verbose) {
          std::cout << "i = " << i << ", result index = " << result_index
                    ...
                    << depth << ", beam size: " << beam_size << "\n";
        }

        Request &request =
            all_requests[old_bc.requestsInfo[index].request_guid];
        ...
        if (depth == 1) {
          ...
          request.beam_trees.at(old_bc.model_id).treeLayers[0].parent_ids[0] =
              -1;
  ...
  void RequestManager::update_beam_metadata(BeamSearchBatchConfig & new_bc,
    ...
    if (verbose) {
      std::cout << "-----------after parent id exchange-----------"
                << std::endl;
      for (int j = 0; j < beam_size; j++) {
  ...
  // prepare next init
  void RequestManager::updateBitMask(
      BatchConfig::BitMask & bitmask, int initLength, int non_tree_size) {
    // assert(initLength == 1);
    // eg. 4 tokens: t1: 0000000..1111, t2: 0000000..1110, t3: 0000000..1100,
    // t4: 0000000..1000
  ...
  std::vector<std::pair<BatchConfig::TokenId, int>>
    ...
    for (int i = 0; i <= old_bc.beamRequestsInfo[request_index].max_depth;
         i++) {
      ...
      for (int j = 0; j < tree.treeLayers[i].nodes_num_this_layer; j++) {
        ...
        serializedTree.push_back(
            std::make_pair(tree.treeLayers[i].tokens[j], i));
      }
    }
    // token, index
