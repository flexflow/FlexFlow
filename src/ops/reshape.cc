                           ...
                           Context ctx, Runtime *runtime)
{
  ...
  if (m->data_type == DT_FLOAT) {
    ...
    Reshape::forward_kernel_wrapper<float>(in_ptr, out_ptr, in_domain.get_volume());
    ...
    double* out_ptr = helperGetTensorPointerWO<double>(
      regions[1], task->regions[1], FID_DATA, ctx, runtime);
    Reshape::forward_kernel_wrapper<double>(in_ptr, out_ptr, in_domain.get_volume());
    ...
    int32_t* out_ptr = helperGetTensorPointerWO<int32_t>(
      regions[1], task->regions[1], FID_DATA, ctx, runtime);
    Reshape::forward_kernel_wrapper<int32_t>(in_ptr, out_ptr, in_domain.get_volume());
    ...
    int64_t* out_ptr = helperGetTensorPointerWO<int64_t>(
      regions[1], task->regions[1], FID_DATA, ctx, runtime);
    Reshape::forward_kernel_wrapper<int64_t>(in_ptr, out_ptr, in_domain.get_volume());
  } else {
                            ...
                            Context ctx, Runtime *runtime)
{
  ...
  if (m->data_type == DT_FLOAT) {
    ...
    float* in_grad_ptr = helperGetTensorPointerRW<float>(
        regions[1], task->regions[1], FID_DATA, ctx, runtime);
    Reshape::backward_kernel_wrapper<float>(in_grad_ptr, out_grad_ptr, in_grad_domain.get_volume());
  } else if (m->data_type == DT_DOUBLE) {
    const double* out_grad_ptr = helperGetTensorPointerRO<double>(
        regions[0], task->regions[0], FID_DATA, ctx, runtime);
    double* in_grad_ptr = helperGetTensorPointerRW<double>(
        regions[1], task->regions[1], FID_DATA, ctx, runtime);
    Reshape::backward_kernel_wrapper<double>(in_grad_ptr, out_grad_ptr, in_grad_domain.get_volume());
  } else if (m->data_type == DT_INT32) {
    const int32_t* out_grad_ptr = helperGetTensorPointerRO<int32_t>(
        regions[0], task->regions[0], FID_DATA, ctx, runtime);
    int32_t* in_grad_ptr = helperGetTensorPointerRW<int32_t>(
        regions[1], task->regions[1], FID_DATA, ctx, runtime);
    Reshape::backward_kernel_wrapper<int32_t>(in_grad_ptr, out_grad_ptr, in_grad_domain.get_volume());
  } else if (m->data_type == DT_INT64) {
    const int64_t* out_grad_ptr = helperGetTensorPointerRO<int64_t>(
        regions[0], task->regions[0], FID_DATA, ctx, runtime);
    int64_t* in_grad_ptr = helperGetTensorPointerRW<int64_t>(
        regions[1], task->regions[1], FID_DATA, ctx, runtime);
    Reshape::backward_kernel_wrapper<int64_t>(in_grad_ptr, out_grad_ptr, in_grad_domain.get_volume());
  } else {
    assert(false && "Unsupported data type in Reshape backward");
  }
}

bool Reshape::measure_operator_cost(
    Simulator* sim,
