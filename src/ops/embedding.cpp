                               ...
                               hipStream_t stream)
{
  if (aggr == AGGR_MODE_NONE) {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(embed_forward_no_aggr<TI>), GET_BLOCKS(outputSize), CUDA_NUM_THREADS, 0, stream, 
        input_ptr, output_ptr, weight_ptr,out_dim, batch_size);
  } else {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(embed_forward_with_aggr<TI>), GET_BLOCKS(outputSize), CUDA_NUM_THREADS, 0, stream, 
...
{
  if (aggr == AGGR_MODE_NONE) {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(embed_backward_no_aggr<TI>), GET_BLOCKS(outputSize), CUDA_NUM_THREADS, 0, stream, 
        input_ptr, output_ptr, weight_grad_ptr, out_dim, batch_size);
  } else {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(embed_backward_with_aggr<TI>), GET_BLOCKS(outputSize), CUDA_NUM_THREADS, 0, stream, 
                                            ...
                                            int64_t p) const
{
  ...
  hipLaunchKernelGGL(rand_generate_int64, GET_BLOCKS(size), CUDA_NUM_THREADS, 0, stream,
      ptr, size, p);
}
