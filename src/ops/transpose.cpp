                              ...
                              hipStream_t stream)
{
  ...
  hipLaunchKernelGGL(transpose_simple_kernel, GET_BLOCKS(out_domain.get_volume()), CUDA_NUM_THREADS, 0, stream,
                                       ...
                                       Domain out_domain)
{
  ...
  Transpose::forward_kernel(m, input_ptr, output_ptr, in_domain, out_domain, stream);
}
                                ...
                                hipStream_t stream)
{
  ...
  hipLaunchKernelGGL(transpose_simple_kernel, GET_BLOCKS(in_grad_domain.get_volume()), CUDA_NUM_THREADS, 0, stream,
                                        ...
                                        Domain out_grad_domain)
{
  hipStream_t stream;
  checkCUDA(get_legion_stream(&stream));
  Transpose::backward_kernel(m, input_grad_ptr, output_grad_ptr, in_grad_domain, out_grad_domain, stream);
}
