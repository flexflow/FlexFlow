                                    ...
                                    const Domain* in_domain)
{
  ...
  Concat::forward_kernel(output, inputs, num_inputs, axis, out_domain, in_domain, stream);
  if (m->profiling) {
                                     ...
                                     const Domain* in_grad_domain)
{
  ...
  Concat::backward_kernel(output_grad, input_grads, num_inputs, axis,
                          out_grad_domain, in_grad_domain, stream);
  if (m->profiling) {
    hipEventRecord(t_end, stream);
    checkCUDA(hipEventSynchronize(t_end));
    float elapsed = 0;
