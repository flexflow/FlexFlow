...
void Pool2D::reset_idx(FFModel const &ff) {
  fwd_input_idx[0] = 0;
  fwd_output_idx = 0;
  bwd_input_idx[0] = 0;
  bwd_output_idx = 0;
...
void Pool2D::pipeforward(FFModel const &ff) {
  ArgumentMap argmap;
  Context ctx = ff.config.lg_ctx;
  Runtime *runtime = ff.config.lg_hlr;
  set_argumentmap_for_forward(ff, argmap);
...
void Pool2D::pipebackward(FFModel const &ff) {
  ArgumentMap argmap;
  Context ctx = ff.config.lg_ctx;
  Runtime *runtime = ff.config.lg_hlr;
  set_argumentmap_for_backward(ff, argmap);
  ...
  // regions[2](I): output
  launcher.add_region_requirement(
      RegionRequirement(outputs[0]->out_pipepart[bwd_output_idx],
                        0 /*projection id*/,
                        READ_ONLY,
                        EXCLUSIVE,
                        outputs[0]->region));
