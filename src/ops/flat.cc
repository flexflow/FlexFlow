...
void Flat::reset_idx(const FFModel& ff){
  fwd_input_idx = 0;
  fwd_output_idx = 0;
  bwd_input_idx = 0;
  bwd_output_idx = 0;
...
void Flat::pipeinit(const FFModel& ff)
{
  assert(check_output_input_weight_same_parallel_is());
  parallel_is = outputs[0]->parallel_is;
  ArgumentMap argmap;
...
void Flat::pipeforward(const FFModel& ff)
{
  ArgumentMap argmap;
  Context ctx = ff.config.lg_ctx;
  Runtime* runtime = ff.config.lg_hlr;
...
void Flat::pipebackward(const FFModel& ff)
{
  ArgumentMap argmap;
  Context ctx = ff.config.lg_ctx;
  Runtime* runtime = ff.config.lg_hlr;
...
bool Flat::measure_operator_cost(
    Simulator* sim,
