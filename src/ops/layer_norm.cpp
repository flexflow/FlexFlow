...
void LayerNorm::forward_kernel(const LayerNormMeta* m,
                               ...
                               hipStream_t stream)
{
  hipLaunchKernelGGL(HIP_KERNEL_NAME(RowwiseMomentsCUDAKernel<float>), m->effective_batch_size, kCUDABlockReduceNumThreads, 0, stream, 
          m->effective_num_elements, m->eps, in_ptr, m->mean_ptr, m->rstd_ptr);
  hipLaunchKernelGGL(HIP_KERNEL_NAME(LayerNormForwardCUDAKernel<float>), m->effective_batch_size, kCUDANumThreads, 0, stream, 
                                ...
                                hipStream_t stream)
{
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(ComputeInternalGradientsCUDAKernel<T>), M, kCUDABlockReduceNumThreads, 0, stream, 
          N, output_grad_ptr, input_ptr, gamma_ptr, m->ds_ptr, m->db_ptr);
  const int64_t B = (M + kCUDANumThreads - 1) / kCUDANumThreads;
  hipLaunchKernelGGL(HIP_KERNEL_NAME(ComputeGradientFusedParamsCUDAKernel<T>), B, kCUDANumThreads, 0, stream, 
          M,
          N,
          m->mean_ptr,
          m->rstd_ptr,
  ...
  if (gamma_grad_ptr != NULL || beta_grad_ptr != NULL) {
    if (M < 512) {
      ...
      hipLaunchKernelGGL(HIP_KERNEL_NAME(GammaBetaBackwardSimpleCUDAKernel<T>), B, kCUDANumThreads, 0, stream, 
              M,
              N,
              output_grad_ptr,
              input_ptr,
      ...
      hipLaunchKernelGGL(HIP_KERNEL_NAME(GammaBetaBackwardCUDAKernel<T>), B, dim3(kThreadX, kThreadY), 0, stream, 
              M,
              N,
              output_grad_ptr,
              input_ptr,
