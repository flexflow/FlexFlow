...
void Linear::backward_kernel_wrapper(LinearMeta const *m,
  ...
  if (m->profiling) {
    ...
    // print_tensor<float>((float*)output_grad_ptr, 10,
    // "[Linear:backward:output_grad]");
    // print_tensor<float>((float*)kernel_grad_ptr, 10,
    // "[Linear:backward:kernel_grad]"); print_tensor<1,
    // float>(acc_bias_grad.ptr, acc_bias_grad.rect,
    // "[Linear:backward:bias_grad]");
    // print_tensor<float>((float*)input_grad_ptr, 10,
    // "[Linear:backward:input_grad]");
  }
}
