...
void Linear::forward_kernel_wrapper(LinearMeta const *m,
  ...
  if (m->profiling) {
    ...
    //print_tensor<float>((float*)input_ptr, in_dim * batch_size, "[Linear:forward:input]");
    //print_tensor<float>((float*)weight_ptr, in_dim * out_dim, "[Linear:forward:kernel]");
    //print_tensor<float>((float*)output_ptr, out_dim * batch_size, "[Linear:forward:output]");
  }
}
