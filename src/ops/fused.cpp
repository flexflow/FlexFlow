    ...
    */
    ...
    __host__ void FusedOp::backward_task(
      ...
      for (int op = fused->numOperators - 1; op >= 0; op--) {
        ...
        for (int i = 0; i < fused->op_num_outputs[op]; i++) {
          assert(fused->op_output_source[i + ooff] == SOURCE_OUTPUT);
          my_output_accessor[i] =
              output_accessor[fused->op_output_idx[i + ooff]];
        ...
        switch (fused->op_op_type[op]) {
          ...
          case OP_EMBEDDING: {
            ...
            Kernels::Embedding::backward_kernel_wrapper(
                m,
                my_input_accessor[0],
                my_output_grad_accessor[0],
