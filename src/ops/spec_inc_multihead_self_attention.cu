...
__global__ void spec_store_kv_cache(float const *devQKVProjArray,
  ...
  CUDA_KERNEL_LOOP(i,
    ...
    // new token 
    int new_token_cache_idx = (req_id * max_beam_width + sub_req_id) * (num_heads * max_seq_len * proj_size) +
    ...
    if (sub_req_id != parent_id) {
      for (int depth = 0; depth < beam_depth; depth++) {
        ...
        int steal_from_idx = (req_id * max_beam_width + parent_id) * (num_heads * max_seq_len * proj_size) +
            head_idx * (max_seq_len * proj_size) + 
            steal_token_idx * proj_size +
            data_idx;
        int steal_to_idx = (req_id * max_beam_width + sub_req_id) * (num_heads * max_seq_len * proj_size) +
            head_idx * (max_seq_len * proj_size) + 
            steal_token_idx * proj_size +
            data_idx;
...
void inference_kernel3(SpecIncMultiHeadSelfAttentionMeta const *m,
  ...
  for (int i = 0; i < bc->MAX_NUM_REQUESTS; i++) {
    ...
    for (int sub_req_id = 0; sub_req_id < bc->sub_requests[i]; sub_req_id++) {
      ...
      // padding)
      void const *B = (void const *)(m->keyCache + (i * bc->MAX_NUM_BEAMS + sub_req_id) * kt_req_block_size);
      ...
      // padding)
      B = (void const *)(m->valueCache + (i * bc->MAX_NUM_BEAMS + sub_req_id) * vt_req_block_size);
  ...
  *apply_rotary_embedding = attn->apply_rotary_embedding;
  ...
  // allocate memory for the seqArray and reserve space
  {
    ...
    size_t key_cache_size =
        num_heads * kProjSize * BatchConfig::MAX_NUM_REQUESTS * MAX_SEQ_LEN * BatchConfig::MAX_NUM_BEAMS;
    size_t value_cache_size =
        num_heads * vProjSize * BatchConfig::MAX_NUM_REQUESTS * MAX_SEQ_LEN * BatchConfig::MAX_NUM_BEAMS;
