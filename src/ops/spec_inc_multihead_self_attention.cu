...
__global__ void spec_store_kv_cache(
  ...
  CUDA_KERNEL_LOOP(i, num_tokens * (kProjSize + vProjSize) * num_kv_heads) {
    ...
    if (real_token_idx[token_idx] >= 0) {
      ...
      //   // printf("token idx %d\n", token_idx);
      //   printf("data idx: %d, tok_id %d, new_token_cache_idx %d, parent_id
      //   %d,
      //   "
...
void update_kv_cache_kernel(SpecIncMultiHeadSelfAttentionMeta const *m,
  ...
  if (num_tokens > 0) {
    ...
    // TODO from here, the code doesn't consider the case BEAM_WIDTH > 1,
...
void compute_attention_kernel(SpecIncMultiHeadSelfAttentionMeta const *m,
  ...
  int num_activate_req = 0;
  // all sub request is 1 case
  if (BeamSearchBatchConfig::MAX_BEAM_WIDTH == 1) {
    ...
    num_activate_req = bc->num_active_requests();
...
void SpecIncMultiHeadSelfAttention::inference_kernel_wrapper(
  ...
  // save_tensor<float>(output.get_float_ptr(), 768 * 10,
  // "/home/xinhaoc/FlexFlow/inference/op.txt");
}
