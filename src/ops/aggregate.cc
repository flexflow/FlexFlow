  ...
  n(_n), lambda_bal(_lambda_bal)
{
  ...
  assert(inputs[0]->dims[0].size <= AGGREGATE_MAX_K && "Increase AGGREGATE_MAX_K in #define");
  assert(inputs[0]->dims[1].size <= AGGREGATE_MAX_BATCH_SIZE && "Increase AGGREGATE_MAX_BATCH_SIZE in #define");
...
bool Aggregate::measure_operator_cost(
    Simulator* sim,
