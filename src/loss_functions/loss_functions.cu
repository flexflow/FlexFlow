                                                                        ...
                                                                        float scale_factor)
{
  ...
  checkCUDA(cudaMemcpy(logit_grad_ptr, logit_ptr,
                                                                 ...
                                                                 float scale_factor)
{
  cudaStream_t stream;
  checkCUDA(get_legion_stream(&stream));
  categorical_crossentropy_loss_backward<<<GET_BLOCKS(logit_volume), CUDA_NUM_THREADS, 0, stream>>>(
                                                               ...
                                                               float scale_factor)
{
  cudaStream_t stream;
  checkCUDA(get_legion_stream(&stream));
  mean_squared_error_avg_loss_backward<<<GET_BLOCKS(logit_volume), CUDA_NUM_THREADS, 0, stream>>>(
    logit_grad_ptr, logit_ptr, label_ptr,
    logit_volume);
  // Scale logit gradients by loss->scale_factor
  scale_kernel<<<GET_BLOCKS(logit_grad_volume), CUDA_NUM_THREADS, 0, stream>>>(
