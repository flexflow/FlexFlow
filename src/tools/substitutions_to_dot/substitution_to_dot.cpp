...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " <json-file> <rule-name>" << std::endl;
        return 1;
    }
    ...
    for (MapOutput const &mo : r.mappedOutput) {
        ...
        dot.add_edge({NodeType::SRC, mo.srcOpId, 0}, {NodeType::SRC_OUTPUT_TENSOR, mo.srcOpId, mo.srcTsId});
        {
                dot.add_node(dstOutputNode, label_map("OUTPUT", dstOutputNode));
                dot.add_node_to_subgraph(dstOutputNode, dst_output_subgraph);
        }
        dot.add_edge({NodeType::DST, mo.dstOpId, 0}, {NodeType::DST_OUTPUT_TENSOR, mo.dstOpId, mo.dstTsId});
        dot.add_edge({NodeType::SRC_OUTPUT_TENSOR, mo.srcOpId, mo.srcTsId}, {NodeType::DST_OUTPUT_TENSOR, mo.dstOpId, mo.dstTsId});
    }
    dot.close();
    std::cout << std::endl;
}
