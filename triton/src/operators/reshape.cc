...
Domain Reshape::GetInputBounds(Processor proc) {
  ...
  DomainPoint lo, hi;
  int const input_dims = inputs[0]->bounds.size();
  lo.dim = input_dims;
  hi.dim = input_dims;
...
void Reshape::Load(Processor proc) {
  ...
  assert(proc_args.input_bounds.get_volume() ==
...
void Reshape::initialize(LegionModelInstance *instance,
  ...
  launcher = IndexTaskLauncher(RESHAPE_TASK_ID,
                               launch_space,
                               TaskArgument(NULL, 0),
...
/*static*/ void Reshape::forward_cpu(Task const *task,
  ...
  ReshapeArgs const *args = (ReshapeArgs const *)task->local_args;
...
#undef DIMFUNC
  ...
  if (args->profiling) {
    ...
    printf("%s [Reshape] forward time (CF) = %.2fms\n",
