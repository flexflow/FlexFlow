...
template <>
__global__ void gpu_forward_cast<__half, int8_t>(__half const *input,
                                                 int8_t *output,
                                                 const size_t volume) {
  const size_t offset = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
  if (offset >= volume) {
...
template <>
__global__ void gpu_forward_cast<__half, int64_t>(__half const *input,
                                                  int64_t *output,
                                                  const size_t volume) {
  const size_t offset = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
  if (offset >= volume) {
...
template <>
__global__ void gpu_forward_cast<int64_t, __half>(int64_t const *input,
                                                  __half *output,
                                                  const size_t volume) {
  const size_t offset = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
  if (offset >= volume) {
...
template <>
__global__ void gpu_forward_cast<__half, uint8_t>(__half const *input,
                                                  uint8_t *output,
                                                  const size_t volume) {
  const size_t offset = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
  if (offset >= volume) {
...
template <>
__global__ void gpu_forward_cast<__half, uint64_t>(__half const *input,
                                                   uint64_t *output,
                                                   const size_t volume) {
  const size_t offset = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
  if (offset >= volume) {
...
template <>
__global__ void gpu_forward_cast<uint64_t, __half>(uint64_t const *input,
                                                   __half *output,
                                                   const size_t volume) {
  const size_t offset = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
  if (offset >= volume) {
...
__global__ static void unary_forward_half(__half const *input,
                                          __half *output,
                                          const __half alpha,
                                          const __half beta,
                                          const __half scalar,
  ...
  switch (optype) {
    ...
    case OP_GELU: {
      output[offset] = __hmul(__hmul(input[offset], 0.5f),
...
__global__ static void unary_forward_float(float const *input,
                                           float *output,
                                           float const alpha,
                                           float const beta,
                                           float const scalar,
...
__global__ static void unary_forward_double(double const *input,
                                            double *output,
                                            double const alpha,
                                            double const beta,
                                            double const scalar,
...
template <typename T>
__host__ static void forward_cast(DataType output_type,
                                  ::cudaStream_t stream,
...
__host__
    /*static*/ void
    UnaryOperator::forward_kernel(UnaryArgs const *args,
                                  ::cudaStream_t stream,
                                  void const *input_ptr,
                                  void *output_ptr,
                                  size_t num_elements) {
  if (args->op_type == OP_CAST) {
    ...
    const size_t blocks =
        (num_elements + (THREADS_PER_BLOCK - 1)) / THREADS_PER_BLOCK;
    assert((args->op_type == OP_EXP) || (args->op_type == OP_LOG) ||
           (args->op_type == OP_SQRT) || (args->op_type == OP_IDENTITY) ||
           (args->op_type == OP_SCALAR_MULTIPLY) ||
           (args->op_type == OP_SCALAR_ADD) ||
           (args->op_type == OP_SCALAR_SUB) ||
           (args->op_type == OP_SCALAR_TRUE_DIV) ||
           (args->op_type == OP_GELU) || (OP_RECIPROCAL));
    switch (args->datatype) {
      case DT_HALF: {
        __half alpha = 1.f, beta = 0.f;
        unary_forward_half<<<blocks, THREADS_PER_BLOCK, 0, stream>>>(
            (__half const *)input_ptr,
            (__half *)output_ptr,
            alpha,
            beta,
            args->scalar.half_value,
            args->op_type,
            num_elements);
        break;
      }
