...
Rect<4> Conv2D::GetInputBounds(Processor proc) {
  Point<4> const point = strategy->find_local_point(proc);
  Transform<4, 4> const transform = input_transform;
  Point<4> const offset = transform * point;
  Rect<4> const extent = input_extent;
  Rect<4> const result(extent.lo + offset, extent.hi + offset);
  Rect<4> bounds;
  for (int d = 0; d < 4; d++) {
    bounds.lo[d] = 0;
...
Rect<4> Conv2D::GetOutputBounds(Processor proc) {
  DomainPoint lo, hi;
  lo.dim = 4;
  hi.dim = 4;
  for (int d = 0; d < 4; d++) {
  ...
  Rect<4> const result = strategy->find_local_domain(proc, global);
  return result;
}
...
void Conv2D::Load(Processor proc) {
  ...
  Rect<4> const input = GetInputBounds(proc);
  Rect<4> const output = GetOutputBounds(proc);
  ...
  if (proc.kind() == Processor::TOC_PROC) {
    ...
    CHECK_CUDNN(cudnnSetTensor4dDescriptor(proc_args.inputTensor,
                                           CUDNN_TENSOR_NCHW,
                                           to_cudnn_datatype(inputs[0]->type),
                                           input_n,
                                           input_c,
                                           input_h,
                                           input_w));
    ...
    if (use_bias) {
      ...
      CHECK_CUDNN(
          cudnnSetTensor4dDescriptor(proc_args.biasTensor,
                                     CUDNN_TENSOR_NCHW,
                                     to_cudnn_datatype(weights[0]->type),
                                     1,
                                     output_c,
                                     1,
                                     1));
    ...
    CHECK_CUDNN(cudnnSetFilter4dDescriptor(proc_args.filterDesc,
                                           to_cudnn_datatype(outputs[0]->type),
                                           CUDNN_TENSOR_NCHW,
                                           output_c,
                                           input_c / groups,
                                           kernel_h,
    ...
    CHECK_CUDNN(
        cudnnSetConvolution2dDescriptor(proc_args.convDesc,
                                        pad_h, // padding_h,
                                        pad_w, // padding_w,
                                        stride_h,
                                        stride_w,
    ...
    if (model->runtime_->allowTensorOpMathConversion_) {
      ...
      CHECK_CUDNN(cudnnSetConvolutionMathType(proc_args.convDesc,
                                              CUDNN_TENSOR_OP_MATH));
    }

    int n, c, h, w;
    CHECK_CUDNN(cudnnGetConvolution2dForwardOutputDim(proc_args.convDesc,
                                                      proc_args.inputTensor,
                                                      proc_args.filterDesc,
                                                      &n,
                                                      &c,
                                                      &h,
                                                      &w));
    ...
    CHECK_CUDNN(cudnnSetTensor4dDescriptor(proc_args.outputTensor,
                                           CUDNN_TENSOR_NCHW,
                                           to_cudnn_datatype(outputs[0]->type),
                                           n,
                                           c,
    ...
    CHECK_CUDNN(cudnnFindConvolutionForwardAlgorithm(proc_args.cudnn,
                                                     proc_args.inputTensor,
                                                     proc_args.filterDesc,
                                                     proc_args.convDesc,
                                                     proc_args.outputTensor,
                                                     reqAlgCnt,
                                                     &cnt,
                                                     perfResults));
    assert(cnt > 0);
    ...
    CHECK_CUDNN(
        cudnnGetConvolutionForwardWorkspaceSize(proc_args.cudnn,
                                                proc_args.inputTensor,
                                                proc_args.filterDesc,
                                                proc_args.convDesc,
                                                proc_args.outputTensor,
    ...
    if ((wts->local_memory[local_index].kind() != Memory::GPU_FB_MEM) ||
      ...
      CHECK_CUDA(cudaMemcpy(device_ptr,
                            wts->local_allocation[local_index],
                            weights_size,
...
void Conv2D::initialize(LegionModelInstance *instance,
  ...
  for (Domain::DomainPointIterator itr(launch_domain); itr; itr++) {
    ...
    weight_attach_launcher.attach_array_soa(
        weight_lr,
        weights[0]->local_allocation[local_index],
        false /*column major*/,
        attach_field,
  ...
  if (use_bias) {
    ...
    AttachLauncher bias_attach_launcher(LEGION_EXTERNAL_INSTANCE,
                                        bias_region,
                                        bias_region,
                                        false /*restricted*/,
                                        false /*mapped*/);
    bias_attach_launcher.attach_array_soa(weights[1]->local_allocation[0],
                                          false /*column major*/,
                                          attach_field,
  ...
  launcher = IndexTaskLauncher(CONV2D_TASK_ID,
                               launch_space,
                               TaskArgument(NULL, 0),
  ...
  launcher.add_region_requirement(RegionRequirement(weight_part,
                                                    0 /*projection id*/,
                                                    LEGION_READ_ONLY,
                                                    LEGION_EXCLUSIVE,
                                                    weight_region));
  ...
  if (use_bias) {
    launcher.add_region_requirement(
        RegionRequirement(weights[1]->region[instance_index],
                          0 /*projection id*/,
                          LEGION_READ_ONLY,
                          LEGION_EXCLUSIVE,
...
void Conv2D::Free(Processor proc) {
  assert(proc.kind() == strategy->kind);
  // If this processor is not used for this layer there is nothing to do
  if (!strategy->is_local_processor(proc)) {
    return;
...
/*static*/ void Conv2D::PreregisterTaskVariants(void) {
  {
...
/*static*/ void Conv2D::forward_cpu(Task const *task,
...
#ifdef LEGION_USE_CUDA
/*static*/ void Conv2D::forward_gpu(Task const *task,
                                    std::vector<PhysicalRegion> const &regions,
                                    Context ctx,
                                    Runtime *runtime) {
  assert(task->local_arglen == sizeof(Conv2DArgs));
  Conv2DArgs const *args = (Conv2DArgs const *)task->local_args;
  assert(regions.size() == (3 + int(args->use_bias)));
  assert(task->regions.size() == (3 + int(args->use_bias)));

  void const *input_ptr = TensorAccessor<LEGION_READ_ONLY, 4>::access(
  ...
  void const *filter_ptr = TensorAccessor<LEGION_READ_ONLY, 4>::access(
      args->filter_datatype, args->local_bounds, regions[2]);
  void const *bias_ptr = NULL;
  if (args->use_bias) {
...
/*static*/ void Conv2D::forward_kernel(Conv2DArgs const *args,
                                       void const *input_ptr,
                                       void *output_ptr,
                                       void const *filter_ptr,
                                       void const *bias_ptr) {
  float alpha = 1.0f, beta = 0.0f;
  CHECK_CUDNN(cudnnConvolutionForward(args->cudnn,
                                      &alpha,
                                      args->inputTensor,
                                      input_ptr,
                                      args->filterDesc,
  ...
  if (bias_ptr != NULL) {
    CHECK_CUDNN(cudnnAddTensor(args->cudnn,
                               &alpha,
                               args->biasTensor,
                               bias_ptr,
                               &alpha,
  ...
  if (args->relu) {
    CHECK_CUDNN(cudnnActivationForward(args->cudnn,
                                       args->actiDesc,
                                       &alpha,
                                       args->outputTensor,
                                       output_ptr,
