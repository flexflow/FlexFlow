...
TRITONSERVER_Error *TRITONBACKEND_Finalize(TRITONBACKEND_Backend *backend) {
  ...
  if (Legion::Runtime::wait_for_shutdown()) {
    return TRITONSERVER_ErrorNew(TRITONSERVER_ERROR_UNKNOWN,
                                 "failed to shutdown Legion runtime");
  }
...
TRITONBACKEND_ISPEC TRITONSERVER_Error *
  ...
  LOG_MESSAGE(TRITONSERVER_LOG_VERBOSE,
              (std::string("model ") + instance_state->Model()->Name() +
               ", instance " + instance_state->Name() + ", executing " +
