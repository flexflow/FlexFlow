...
#define CHECK_GENERAL_OPERATOR_ATTRIBUTES(op__,                                \
                                          op_type__,                           \
                                          model__,                             \
                                          strategy__,                          \
                                          num_input__,                         \
  ...
  do {                                                                         \
    ...
    EXPECT_TRUE(op__->model ==                                                 \
                reinterpret_cast<tbl::LegionModelState *>(model__))            \
...
TEST_F(OnnxParserSingleNodeSingleProcessorTest, ParseAdd) {
  ...
  tbl::PartitionStrategy strategy(
      ...
      {reinterpret_cast<tbl::LayerStrategy const *>(&layer_strategy_)});
  ...
  CHECK_GENERAL_OPERATOR_ATTRIBUTES(generated_op,
                                    tbl::OperatorType::OP_EW_ADD,
                                    &model_stub,
                                    &layer_strategy_,
                                    2,
                                    0,
  ...
  for (size_t i = 0; i <= 1; i++) {
    ...
    CHECK_GENERAL_TENSOR_ATTRIBUTES(inputs[i].second,
                                    nullptr,
                                    false,
                                    tbl::DataType::DT_FLOAT,
                                    std::vector<size_t>({4, 2}));
  }

  auto output = model_stub[2];
  CHECK_GENERAL_TENSOR_ATTRIBUTES(output,
                                  generated_op,
                                  false,
                                  tbl::DataType::DT_FLOAT,
                                  std::vector<size_t>({4, 2}));
...
TEST_F(OnnxParserSingleNodeSingleProcessorTest, ParseAvgPool) {
  ...
  CHECK_GENERAL_TENSOR_ATTRIBUTES(inputs[0].second,
                                  nullptr,
                                  false,
                                  tbl::DataType::DT_FLOAT,
...
TEST_F(OnnxParserSingleNodeSingleProcessorTest, ParseAvgPoolCountIncludePad) {
  ...
  auto err = tbl::OnnxParser::LoadModel(
      ...
      reinterpret_cast<tbl::LegionModelState *>(&model_stub),
      &strategy,
      "data/avg_pool_count_include_pad.onnx",
      &inputs,
      &outputs,
      &layers);
  auto expected_err = std::string(
...
TEST_F(OnnxParserSingleNodeSingleProcessorTest, ParseConv2D) {
...

  {
    ...
    CHECK_GENERAL_TENSOR_ATTRIBUTES(weight,
                                    generated_op,
                                    true,
                                    tbl::DataType::DT_FLOAT,
    ...
    float const *data_allocation = reinterpret_cast<float const *>(
...

  {
    ...
    CHECK_GENERAL_TENSOR_ATTRIBUTES(bias,
                                    generated_op,
                                    true,
                                    tbl::DataType::DT_FLOAT,
                                    std::vector<size_t>({2}));
...
TEST_F(OnnxParserSingleNodeSingleProcessorTest, ParseReciprocal) {
  ...
  CHECK_GENERAL_OPERATOR_ATTRIBUTES(generated_op,
                                    tbl::OperatorType::OP_RECIPROCAL,
                                    &model_stub,
                                    &layer_strategy_,
                                    1,
                                    0,
...
TEST_F(OnnxParserSingleNodeSingleProcessorTest, ParseSoftmaxDefaultAxis) {
  ...
  auto err = tbl::OnnxParser::LoadModel(
      ...
      reinterpret_cast<tbl::LegionModelState *>(&model_stub),
      &strategy,
      "data/softmax_default_axis.onnx",
      &inputs,
      &outputs,
      &layers);
  ASSERT_TRUE(err == nullptr) << TRITONSERVER_ErrorMessage(err);
...
TEST_F(OnnxParserSingleNodeSingleProcessorTest, ParseSoftmaxNegativeAxis) {
  ...
  auto err = tbl::OnnxParser::LoadModel(
      ...
      reinterpret_cast<tbl::LegionModelState *>(&model_stub),
      &strategy,
      "data/softmax_negative_axis.onnx",
      &inputs,
      &outputs,
      &layers);
  ASSERT_TRUE(err == nullptr) << TRITONSERVER_ErrorMessage(err);
