...
def find_compile_commands(project: Project) -> Union[AbsolutePath, ErrorResponse]:
    ...
    if len(compile_command_paths) == 1:
        ...
        return ErrorResponse(
            f"Found multiple compile_commands.json, disambiguation required: {list(sorted(compile_command_paths))}"
        )
...
def get_clang_tidy_check_rules(
    project: Project, config: ClangToolsConfig, compile_commands_path: AbsolutePath
) -> FrozenSet[Rule]:
...
def run(
    settings: Settings, project: Project, method: Method, compile_commands_path: Optional[AbsolutePath] = None
) -> Response:
    config = ClangToolsConfig(
...
def main_tidy(raw_args: Any) -> Dict[str, Response]:
    ...
    return {
        raw_args.linter_name: run(
            settings=raw_args.settings,
            project=raw_args.project,
            method=raw_args.method,
            compile_commands_path=raw_args.compile_commands,
        )
    }
...
spec = Specification.create(name="clang_tidy", func=run, supported_methods=[Method.CHECK, Method.FIX])
