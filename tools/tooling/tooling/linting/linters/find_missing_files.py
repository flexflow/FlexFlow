...
def get_fix_missing_files_rules(project: Project) -> FrozenSet[Rule]:
    def _create_missing_source_file(
        p: AbsolutePath, attrs: Attrs, extra: ExprExtra, project: Project = project
    ) -> bool:
...
def check_missing_files(project: Project) -> CheckResponse:
    ...
    return CheckResponse(num_errors=num_errors, json_data=json_data)
...
def fix_missing_files(project: Project) -> FixResponse:
    ...
    return FixResponse(did_succeed=(num_errors == 0), num_fixes=len(fixed), json_data=json_data)
...
spec = Specification.create(name="find_missing_files", func=run, supported_methods={Method.CHECK, Method.FIX})
