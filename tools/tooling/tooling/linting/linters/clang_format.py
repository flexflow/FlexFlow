...
def run_clang_format(
    project: Project, config: ClangToolsConfig, args: Sequence[str], files: Sequence[PathLike[str]]
) -> str:
...
def get_clang_check_rules(project: Project, config: ClangToolsConfig) -> FrozenSet[Rule]:
    def check_if_path_is_formatted(
        p: AbsolutePath, attrs: Attrs, extra: ExprExtra, project: Project = project, config: ClangToolsConfig = config
    ) -> bool:
    ...
    check_formatting_rule = Rule(
        "clang_format.check",
        OpaqueFunction(
            precondition=HasAttribute(FileAttribute.SUPPORTED_BY_CLANG_FORMAT), func=check_if_path_is_formatted
        ),
        FileAttribute.WAS_PROPERLY_CLANG_FORMATTED,
    )

    return update_rules.union([supported_by_clang_format, check_formatting_rule])
...
def get_clang_fix_rules(project: Project, config: ClangToolsConfig) -> FrozenSet[Rule]:
    ...
    run_formatter = Rule(
        "clang_format.fix",
        OpaqueFunction(precondition=HasAttribute(FileAttribute.SUPPORTED_BY_CLANG_FORMAT), func=format_path),
        FileAttribute.DID_FIX_CLANG_FORMATTING,
    )

    return get_clang_check_rules(project, config).union([supported_by_clang_format, run_formatter])
...
def fix_format(project: Project, config: ClangToolsConfig) -> FixResponse:
    ...
    return FixResponse(
        ...
        json_data={"failed": failed_files, "fixed": fixed_files},
    )
...
def main_format(raw_args: Any) -> Dict[str, Response]:
    ...
    return {raw_args.linter_name: run(settings=raw_args.settings, project=raw_args.project, method=raw_args.method)}


def get_parser(p: argparse.ArgumentParser) -> argparse.ArgumentParser:
    return instantiate(p, func=main_format)


spec = Specification.create(name="clang_format", func=run, supported_methods=[Method.CHECK, Method.FIX])
