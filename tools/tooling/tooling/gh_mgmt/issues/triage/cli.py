...
from tooling.gh_mgmt.issues.triage.config import (
    get_default_cache_dir,
    get_auth_token_from_gh_cli,
    init_config,
    get_cache_path,
)
import re
...
def main(raw_args: Any) -> Optional[Json]:
    ...
    init_config(max_lookback=raw_args.max_lookback, cache_dir=raw_args.cache_dir, auth_token=auth_token)
...
def get_triage_parser(p: argparse.ArgumentParser) -> argparse.ArgumentParser:
    p.add_argument(
        "--max-lookback", type=parse_timedelta, action=EnvStore, env_sources=["FFGH_MAX_LOOKBACK"], default="1y"
    )
    p.add_argument(
        "--cache-dir", type=Path, action=EnvStore, env_sources=["FFGH_CACHE_DIR"], default=get_default_cache_dir()
    )
