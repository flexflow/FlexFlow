...
if [[ "${FF_GPU_BACKEND}" != @(cuda|hip_cuda|hip_rocm|intel) ]]; then
  ...
  exit 1
    ...
    sudo apt-get install -y --no-install-recommends hipblas miopen-hip rocm-hip-sdk
    sudo apt-get install -y --no-install-recommends miopen-hip rocm-hip-sdk
    sudo apt-get install -y --no-install-recommends rocm-hip-sdk
else
